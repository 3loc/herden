...
import {
  EMPTY_LISTEN, PREROLL_MS, SILENCE_END_MS, SPEECH_RMS, TRAILING_SILENCE_KEEP_MS, UTTERANCE_MAX_MS,
...
test("three seconds of silence ends the utterance, and not a frame sooner", () => {
...
test("a monologue is cut at the hard cap rather than growing forever", () => {
  ...
  const talked = feed(EMPTY_LISTEN, repeat(speech, frames));
