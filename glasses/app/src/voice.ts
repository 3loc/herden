...
/** The persistent bottom row reports the interpreted command, not a touch hint. */
export function voiceFooter(state: VoiceState, micOpen: boolean): string {
  if (!micOpen) return "MIC OFF";
  switch (state.phase) {
    case "transcribing": return "MIC ON · transcribing";
    case "result": return state.outcome === "not a command"
      ? `MIC ON · heard ${state.transcript ?? "?"}`
      : `MIC ON · ${state.outcome ?? "ready"}`;
    case "error": return `MIC ON · ${state.message ?? "error"}`;
    default: return "MIC ON · listening";
  }
}
