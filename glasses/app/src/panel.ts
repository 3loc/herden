...
const CSS = `
  ...
  .host { grid-template-columns:8px minmax(0,1fr) auto; } .host button { margin:0; padding:2px 5px; background:transparent; color:inherit; border:0; opacity:.48; font-size:11px; }
...
export interface PanelOptions { hosts: HostSettings[]; onSaveHosts: (hosts: HostSettings[]) => void; }

export class Panel {
  #hosts: HTMLElement; #agents: HTMLElement; #dot: HTMLElement; #label: HTMLElement; #settings: HostSettings[]; #onSaveHosts: PanelOptions["onSaveHosts"];
  ...
  constructor(root: HTMLElement, options: PanelOptions) {
    ...
    root.innerHTML = `
      ...
      <h2>Spaces</h2><ul id="agents"><li class="empty">waiting for Herden…</li></ul>
      <details><summary>Add a Host</summary><p class="help">On that machine, run <code>herden glasses enable</code>, then enter its private HUD address and token here.</p>
        <label for="name">Host name</label><input id="name" placeholder="fansvine" autocapitalize="off" spellcheck="false">
    ...
    this.#settings = options.hosts; this.#onSaveHosts = options.onSaveHosts; this.#renderHosts(new Set());
  ...
  render(snapshot: Snapshot, hosts: HostSettings[], online: Set<string>): void {
    ...
    this.#agents.replaceChildren(...snapshot.agents.map((agent) => {
      const li = document.createElement("li"); const mark = document.createElement("span"); mark.className = "mark"; mark.textContent = MARKS[agent.status] ?? "?";
      const space = document.createElement("span"); space.className = "space"; space.textContent = agent.hostName ? `${agent.space} · ${agent.hostName}` : agent.space || "Unnamed Space";
      const state = document.createElement("span"); state.className = "state"; state.textContent = agent.status; li.append(mark, space, state); return li;
    }));
  }
}
