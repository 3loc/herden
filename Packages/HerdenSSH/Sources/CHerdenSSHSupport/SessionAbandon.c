#include "CHerdenSSHSupport.h"

#include <errno.h>

static ssize_t herden_discarded_send(
...
static ssize_t herden_abandoned_receive(
    libssh2_socket_t socket,
    void *buffer,
    size_t length,
    int flags,
...
void herden_libssh2_prepare_session_abandonment(LIBSSH2_SESSION *session) {
    ...
    libssh2_session_callback_set2(
        ...
        (libssh2_cb_generic *)herden_discarded_send
    );
    libssh2_session_callback_set2(
        session,
        LIBSSH2_CALLBACK_RECV,
        (libssh2_cb_generic *)herden_abandoned_receive
    );
}
...
int herden_libssh2_abandon_session(LIBSSH2_SESSION *session) {
    ...
    herden_libssh2_prepare_session_abandonment(session);
