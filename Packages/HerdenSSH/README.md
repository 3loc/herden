...
`HerdenSSH` is Herden's repository-local Swift package for the native SSH
implementation accepted in ADR 0011. The package owns libssh2 and OpenSSL so
the app target consumes only the `HerdenSSH` product and never imports native
...
SHA-256 checksums, and verifies the result. The static XCFrameworks are
deliberately unsigned: application signing belongs to the developer building
Herden, and the repository must not inherit a particular Apple team identity.
The pinned source hashes and committed file-level checksums provide artifact
integrity. Exact byte-for-byte output requires the Xcode, SDK, compiler, and
configuration recorded in `Artifacts/PROVENANCE.md`.
...
`make test` runs the app suites and then `scripts/run-herdenssh-package-tests.sh`
...
`HerdenSSHDirectStreamLocalE2ETests` suite.
...
The same CI fixture also runs `HerdenSSHJumpHostGateE2ETests` against two
