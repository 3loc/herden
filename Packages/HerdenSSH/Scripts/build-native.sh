...
SOURCE_LOCK="${HERDEN_SSH_SOURCE_LOCK:-${PACKAGE_DIR}/Sources.lock}"
DEPLOYMENT_TARGET="18.0"
FIXED_PREFIX="/usr/local/herden-ssh/openssl-3.6.3"
JOBS="${HERDEN_SSH_JOBS:-4}"

# shellcheck source=../Sources.lock
source "${SOURCE_LOCK}"

for command in curl shasum tar perl make cmake xcodebuild xcrun; do
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/herden-ssh-native.XXXXXX")"
...
<dict>
    ...
    <string>com.3loc.herden.ssh.${name}</string>
...
echo "Built and verified HerdenSSH native artifacts."
