...
grep -Fq ".iOS(.v${EXPECTED_MAJOR})" "${PACKAGE_DIR}/Package.swift" || {
    echo "error: HerdenSSH Package.swift does not target iOS ${EXPECTED_MAJOR}" >&2
    exit 1
}
...
[[ ! -e "${ARTIFACT_DIR}/COpenSSL.xcframework/_CodeSignature" ]] || {
    echo "error: COpenSSL XCFramework must remain unsigned" >&2
    exit 1
}
...
echo "HerdenSSH artifact checksums, slices, privacy manifest, provenance, and algorithm policy are valid."
