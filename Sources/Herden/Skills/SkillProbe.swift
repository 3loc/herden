...
enum SkillProbe {
    ...
    /// back to its scope and command prefix: `__HERDEN_SKILL_3__=path`.
    static let fileMarkerPrefix = "__HERDEN_SKILL_"
    static let endMarker = "__HERDEN_SKILL_END__"
    ...
    static func command(for sources: [ResolvedSource]) -> String {
        ...
        return "/bin/sh -c '\(script)' herden-skills-probe \(arguments)"
    }
