...
/// agent CLI emits OSC 133, and herden's API exposes no transcript. So the jump
...
///
/// 1. **Prompt lines seen in a frame.** Every agent TUI Herden attaches to
///    draws the user's own turns behind a prompt glyph (`>`, `›`, `❯`). A line
///    carrying one is a user message wherever it came from — a desktop herden,
