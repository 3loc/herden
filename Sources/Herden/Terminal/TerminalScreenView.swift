...
final class TerminalKeyboardControl {
    weak var terminal: HerdenTerminalView? {
        didSet {
            oldValue?.onFirstResponderChange = nil
            oldValue?.onPromptEditing = nil
            terminal?.onFirstResponderChange = { [weak self] in
                self?.syncFirstResponder()
            }
            terminal?.onPromptEditing = { [weak self] in self?.onPromptEditing?() }
            syncFirstResponder()
        }
    }
    ...
    func sendInput(_ data: Data) {
        terminal?.sendReliableExternalInput(data)
    }
...
struct TerminalScreenView: UIViewRepresentable {
    ...
    func updateUIView(_ view: HerdenTerminalView, context: Context) {
        view.onNavigateBack = onNavigateBack
...
final class HerdenTerminalView: UITerminalView, TerminalByteSink {
    ...
    private var pendingRecordedDeletions = 0
    private var horizontalEditsPrompt = false
    private var horizontalCursorSteps = 0

    private lazy var horizontalGesture = UIPanGestureRecognizer(
        target: self, action: #selector(handleHorizontalGesture(_:)))
    ...
    override var selectedTextRange: UITextRange? {
        ...
        set {
            ...
            let requested = min(max(range.location, 0), length)
            let positions = textInputCursorPositions
            let location = positions.last(where: { $0 <= requested }) ?? 0
            ...
            let oldIndex = positions.lastIndex(where: { $0 <= oldLocation }) ?? 0
            let newIndex = positions.firstIndex(of: location) ?? 0
            sendCursorMovement(by: newIndex - oldIndex)
        }
    }
    ...
    override func closestPosition(to point: CGPoint) -> UITextPosition? {
        ...
        guard let current = gridPointMapper.cell(
            at: CGPoint(x: caret.minX, y: caret.minY - terminalCellSize.height / 2))
        else { return nil }
    ...
    /// Dictation hypotheses and explicit deck keys are already serialized by
    /// app-owned state. Dropping one edit while ordinary typing is gated makes
    /// the next Speech correction relative to text the PTY never received,
    /// which presents as duplicated letters or whole words.
    func sendReliableExternalInput(_ data: Data) {
    ...
    private func recordTerminalInput(_ data: Data) {
        ...
        for segment in text.split(separator: "\u{7f}", omittingEmptySubsequences: false).enumerated() {
            if segment.offset > 0 {
                if pendingRecordedDeletions > 0 { pendingRecordedDeletions -= 1 }
                else if let range = textInputDeletionRange() { deleteFromTextInputStorage(in: range) }
            }
            if !segment.element.isEmpty { recordCommittedText(String(segment.element)) }
        }
    }

    private func recordCommittedText(_ text: String) {
        var insertionEnd = textInputSelection.location + text.utf16.count
            ...
            from: .newlines, options: .backwards)
        {
            ...
            insertionEnd = textInputStorage.utf16.count
        }
        textInputSelection = NSRange(
            location: min(insertionEnd, textInputStorage.utf16.count),
            length: 0)
    }

    private var textInputCursorPositions: [Int] {
        var positions = [0]
        for character in textInputStorage {
            positions.append((positions.last ?? 0) + String(character).utf16.count)
        }
        return positions
    }
    ...
    override func gestureRecognizerShouldBegin(
        ...
        if gestureRecognizer === horizontalGesture {
            let velocity = horizontalGesture.velocity(in: self)
            let translation = horizontalGesture.translation(in: self)
            let point = horizontalGesture.location(in: self)
            let start = CGPoint(x: point.x - translation.x, y: point.y - translation.y)
            return abs(velocity.x) > abs(velocity.y) * 1.5
                && (isPromptEditingPoint(start) || (velocity.x > 0 && onNavigateBack != nil))
        }
    ...
    private func installTouchScrolling() {
        ...
        horizontalGesture.allowedTouchTypes = [directTouch]
        horizontalGesture.maximumNumberOfTouches = 1
        horizontalGesture.delegate = self
        addGestureRecognizer(horizontalGesture)
    ...
    func handleTap(at location: CGPoint) {
        ...
        case .report(let raisesKeyboard):
            if isPromptEditingPoint(location) { onPromptEditing?() }
            if keyboardActivationRegion.contains(location),
               !textInputStorage.isEmpty,
    ...
    func isPromptEditingPoint(_ point: CGPoint) -> Bool {
        isLocalInputEnabled && (keyboardActivationRegion.contains(point)
            || (modeTracker.isAlternateScreen
                && TerminalKeyboardTapTarget.alternateScreenBottomRegion(in: bounds).contains(point)))
    }

    @objc private func handleHorizontalGesture(_ gesture: UIPanGestureRecognizer) {
        let translation = gesture.translation(in: self)
        switch gesture.state {
        case .began:
            let point = gesture.location(in: self)
            horizontalEditsPrompt = isPromptEditingPoint(
                CGPoint(x: point.x - translation.x, y: point.y - translation.y))
            horizontalCursorSteps = 0
            stopTouchScrollMomentum()
            if horizontalEditsPrompt { onPromptEditing?() }
        case .changed:
            guard horizontalEditsPrompt else { return }
            let steps = Int(translation.x / max(8, terminalCellSize.width))
            movePromptCursor(by: steps - horizontalCursorSteps)
            horizontalCursorSteps = steps
        case .ended:
            if !horizontalEditsPrompt,
               ConsoleHorizontalNavigation.returnsToPicker(translation: CGSize(
                   width: translation.x,
    ...
    /// Native cursor keys work in both insert and vi normal mode. Moving a
    /// finger never sends text or Return, and never changes the agent's mode.
    func movePromptCursor(by characters: Int) {
        guard isLocalInputEnabled, markedTextRange == nil, characters != 0 else { return }
        if textInputStorage.isEmpty {
            sendCursorMovement(by: characters)
            return
        }
        let positions = textInputCursorPositions
        let current = positions.lastIndex(where: { $0 <= textInputSelection.location }) ?? 0
        let next = min(max(current + characters, 0), positions.count - 1)
        selectedTextRange = TerminalInputTextRange(location: positions[next], length: 0)
    }
...
extension HerdenTerminalView: TerminalSurfaceOpenURLDelegate,
