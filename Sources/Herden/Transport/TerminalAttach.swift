...
///
/// Agent Attach addresses herden's Agent-facing command with a Pane id. An
/// ordinary shell terminal instead addresses `herden terminal attach` with the
...
/// One interactive terminal request: its typed remote target, herden's
...
/// `shell -c command`, even when Herden requests a PTY and an exec together
...
/// this marker and the subsequent `exec herden agent attach`. None of that
...
enum AttachBootstrapHandshake {
    static let marker = Data("\u{1B}_herden-attach\u{1B}\\".utf8)
    ...
    static let markerPrintfFormat = "\\033_herden-attach\\033\\134"
}
