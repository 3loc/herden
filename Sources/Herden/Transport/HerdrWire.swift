...
enum HerdrWire {
    ...
    ///
    /// Each herden API connection serves one request, so the response on a
    /// connection is always for that connection's sole in-flight request.
    /// Success results still require an exact id match. Errors do not: herden
    ...
    /// The empty `params` object herden expects on parameterless methods.
