...
/// exposes no SSH-specific keepalive machinery, so the session pings herden
...
/// socket + herden), so a dead connection is detected within interval +
/// request timeout.
...
enum EventsSessionStatus: Sendable, Equatable {
    ...
    /// snapshot while events continue to be consumed. Herden 0.9 lifecycle
    /// subscriptions do not replay pre-subscription events. A deliberate
    /// reinstall remains `.connected` through its brief stream gap.
    case connected
...
actor EventsSession {
    ...
    /// Successful herden ping round trips on the Host's live SSH connection.
    ...
    /// a herden that stopped while the app was away classifies as the
    ...
    ///
    /// Pane-scoped entries are derived from a snapshot, and herden rejects an
    ...
    /// Measures only the herden `ping` request on an established Transport.
