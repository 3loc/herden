...
struct TransportErrorPresentation: Equatable, Sendable {
    ...
    /// herden's code and message. Never authored instruction.
    let detail: String?
...
extension TransportError {
    var presentation: TransportErrorPresentation {
        ...
        case .socketNotFound(let path):
            TransportErrorPresentation(
                summary: "The herden socket was not found",
                detail: path,
                recoverySuggestion: "Check this Host's session.")
        case .herdenBinaryNotFound:
            TransportErrorPresentation(
                summary: "herden is not on this Host's SSH PATH",
                detail: nil,
                recoverySuggestion:
        ...
        case .streamLocalOpenFailed:
            TransportErrorPresentation(
                summary: "herden is not running on this Host",
                detail: nil,
        ...
        case .protocolVersionMismatch(let server, let supported):
            TransportErrorPresentation(
                summary: "Incompatible herden protocol",
                detail: "herden speaks protocol \(server); this app needs at least \(supported)",
                recoverySuggestion: "Update herden on the Host.")
        ...
        case .malformedResponse(let payload):
            TransportErrorPresentation(
                summary: "herden returned an invalid response",
                detail: payload,
                recoverySuggestion: "Check its version.")
        case .apiRejected(let code, let message):
            TransportErrorPresentation(
                summary: "herden rejected the request",
                detail: "\(message) (\(code))",
                recoverySuggestion: nil)
