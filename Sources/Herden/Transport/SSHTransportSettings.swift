...
/// How to reach one Host, authenticate against it, and find its herden socket.
struct SSHTransportSettings: Sendable {
    static let defaultSessionListCommand = "herden session list --json"
    static let defaultStageDirectoryCommand =
        "/bin/sh -c 'umask 077; "
        + "directory=$(mktemp -d \"${TMPDIR:-/tmp}/herden.XXXXXXXX\") || exit 1; "
    ...
    /// The Herden plugin (ADR 0007/0008) whose config dir holds the
    /// Notification Registration file.
    static let notificationPluginID = "herden"
    ...
    static let notificationPluginIDToken = "__HERDEN_PLUGIN_ID__"

    static let defaultNotificationConfigDirCommand =
        "/bin/sh -c '\(HerdenHostPath.pathExport); "
        + "printf \"__HERDEN_PLUGIN_CONFIG_DIR__=%s\\n\" "
        + "\"$(herden plugin config-dir \(notificationPluginIDToken))\"'"
    ...
    /// Which herden socket to reach on the Host.
    var socket: HerdenSocketLocation
    ...
    /// Command that wakes a stopped herden server, run over a no-PTY exec
    /// channel when a request hits connection-refused (#6). The default is
    /// the strategy from spec #16: `herden remote-client-bridge` ensures the
    ...
    /// Herden plugin being installed and enabled before touching its
    /// config dir — `herden plugin config-dir` happily prints (and creates) a
    ...
    /// Prints the marker-delimited config dir of the Herden plugin;
    /// herden creates the directory if missing. Runs under POSIX sh because
    ...
    /// attach channel (`HerdenSSHTransport.runAttachChannel`).
