...
private struct HerdenSSHSessionListResponse: Decodable {
    let sessions: [HerdrSession]
}

private enum HerdenSSHAttachPumpError: Error, Sendable {
    case input(String)
    case output(String)
    /// Remote process exited nonzero. Distinct from a channel I/O failure so
    /// Attach can map a bare-`herden` exit 127 onto the PATH problem (#206).
...
protocol HerdenSSHAttachChannel: Sendable {
...
extension SSHPTYChannel: HerdenSSHAttachChannel {}

private final class HerdenSSHAttachPumpCancellation: Sendable {
...
private enum HerdenSSHNotificationFileError: Error, Sendable {
    case permissionVerificationFailed
}

#if DEBUG
struct HerdenSSHNotificationFileStateForTesting: Sendable, Equatable {
...
private actor HerdenSSHNotificationFileCompletion {
...
final class HerdenSSHAttachOutputGate: Sendable {
    ...
    static func makeStream() -> (
        ...
        gate: HerdenSSHAttachOutputGate
    ) {
...
/// The libssh2-backed app Transport. Ordinary herden RPCs use fresh
...
actor HerdenSSHTransport: Transport {
    /// Lowest herden protocol this build can drive. Below it, methods this app
    ...
    /// (`scripts/herden-schema.json`) was generated against. A Host above this
    /// is accepted and fully usable; it is merely newer than this build knows.
    ///
    /// The version check is a floor rather than an equality on purpose. herden
    ...
    private let socketLocation: HerdenSocketLocation
    ...
    static func connect(settings: SSHTransportSettings) async throws -> HerdenSSHTransport {
        ...
        do {
            try await HerdenSSHHostKeyVerifier(
                host: settings.host,
                port: settings.port,
                policy: settings.hostKeyPolicy)
    ...
    func listSessions() async throws -> [HerdrSession] {
        ...
        do {
            sessions = try JSONDecoder().decode(
                HerdenSSHSessionListResponse.self,
                from: output).sessions
        } catch {
            throw TransportError.malformedResponse(
                "herden session list returned invalid JSON: \(Self.preview(output))")
        }
    ...
    func createSpace(_ workspace: SpaceCreationRequest) async throws -> CreatedSpace {
        let created = try await request(
    ...
    private func enforceNotificationPermissions(
        ...
        guard attributes.permissions == permissions else {
            throw HerdenSSHNotificationFileError.permissionVerificationFailed
        }
    }
    ...
    private func withNotificationFileRequestDeadline<T: Sendable>(
        ...
        let completion = HerdenSSHNotificationFileCompletion()
        do {
    ...
    func notificationFileStateForTesting() async -> HerdenSSHNotificationFileStateForTesting {
        let admission = await channelAdmission.snapshot()
        return HerdenSSHNotificationFileStateForTesting(
    ...
    /// Finds the enabled Herden plugin and returns its canonical id.
    private func installedNotificationPluginID(in listOutput: Data) throws -> String {
        ...
        let pluginID = SSHTransportSettings.notificationPluginID
        guard enabledIDs.contains(pluginID) else {
    ...
    private func runNotificationPluginProbe(command: String) async throws -> Data {
        do {
            let result = try await runExec(
                Self.cLocaleCommand(HerdenHostPath.wrappingBareHerden(command)))
            if let missing = HerdenHostPath.missingBinaryError(
        ...
        } catch TransportError.herdenBinaryNotFound {
            throw TransportError.herdenBinaryNotFound
    ...
    private func withColdStartWake<T: Sendable>(
        ...
        do {
            ...
            do {
                ...
                // socket. A Host can run herden from a login shell while `herden`
            ...
            try await self.wake.value {
                ...
                guard result.exitStatus == 0, result.reachedEOF else {
                    throw TransportError.channelFailed(
                        detail: "herden wake command failed: \(Self.preview(result.stderr))")
                }
            }
        }
    }
    ...
    static func wakeExecCommand(
        wakeCommand: String, socketPath: String, socketLocation: HerdenSocketLocation
    ) throws -> String {
        ...
        case .namedSession(let sessionName):
            guard HerdrSessionName.isValid(sessionName) else {
                throw TransportError.channelFailed(
                    detail: "The herden session name is invalid.")
            }
    ...
    func attachTerminal(
        ...
        do {
            ...
            let outputGate = HerdenSSHAttachOutputGate()
    ...
    static func attachExecCommand(
        ...
        return "/bin/sh -c '\(HerdenHostPath.pathExport); "
    ...
    /// `herdenBinaryNotFound` only when the exec'd command is still a bare
    /// `herden` word; injectable scripts keep the generic channel failure.
    static func attachChannelFailure(
        exitStatus: Int32, attachCommand: String
    ) -> TransportError {
        HerdenHostPath.missingBinaryError(exitStatus: exitStatus, command: attachCommand)
    ...
    private func runAttachChannel(
        ...
        output: HerdenSSHAttachOutputGate,
        attachCommand: String
    ) async {
        var failure: TransportError?
        var sawCleanEnd = false
        var pumpFailure: HerdenSSHAttachPumpError?
        ...
        do {
            ...
            } catch let error as HerdenSSHAttachPumpError {
                pumpFailure = error
                throw error
            }
            failure = nil
    ...
    static func runAttachPumps(
        channel: any HerdenSSHAttachChannel,
        input: TerminalAttachInputQueue,
        output: HerdenSSHAttachOutputGate,
        requestTimeout: Duration
    ) async throws -> Bool {
        let cancellation = HerdenSSHAttachPumpCancellation()
        return try await withThrowingTaskGroup(of: Bool.self) { group in
            group.addTask {
                do {
                    ...
                    throw HerdenSSHAttachPumpError.input(String(describing: error))
                }
            }
            group.addTask {
                ...
                do {
                    while !Task.isCancelled {
                        ...
                        guard let bytes else {
                            ...
                            guard status == 0 else {
                                ...
                                throw HerdenSSHAttachPumpError.remoteExit(status)
                            }
                            yieldAdmitted(gate.flush())
                            return true
                        }
                    ...
                    yieldAdmitted(gate.flush())
                    throw HerdenSSHAttachPumpError.output(String(describing: error))
                }
            }
            defer {
                input.finish()
