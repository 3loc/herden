...
enum HerdenHostPath: Sendable {
    ...
    ///
    /// mise exposes tools to non-interactive shells through its shims
    /// directory. Follow mise's own resolution order: `MISE_DATA_DIR`, then
    /// `XDG_DATA_HOME/mise`, then `~/.local/share/mise`. Values set only in
    /// interactive startup files are intentionally not visible here.
