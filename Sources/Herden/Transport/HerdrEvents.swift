...
/// Canonical event kind: the dotted name (`pane.created`) as herden's
...
struct HerdrEventKind: Hashable, Sendable {
    ...
    /// canonical kind. Unknown names pass through untouched — herden's API
    ...
    /// All kinds herden 0.8.0's schema declares subscribable.
...
extension HerdrEventKind {
    ...
    /// event buffer shed under overflow. Never sent by herden and not
...
struct HerdrEvent: Sendable, Equatable {
    ...
    /// Raw payload: only 3 of 26 kinds are typed in herden's schema, so the
