...
/// authenticate, and which herden session to reach. Never carries a secret —
...
struct Host: Identifiable, Codable, Hashable, Sendable {
    ...
    /// while this field remains editable for older herden versions. Blank
    /// means the default herden session.
    ...
    init(from decoder: any Decoder) throws {
        ...
        guard trimmedSessionName.isEmpty || HerdrSessionName.isValid(trimmedSessionName) else {
            throw DecodingError.dataCorruptedError(
                forKey: .sessionName, in: container, debugDescription: "Invalid herden session name")
        }
    }
    ...
    var socketLocation: HerdenSocketLocation {
