...
struct HostListView: View {
    ...
    private let onHostAdded: ((Host.ID) -> Void)?
            ...
            Group {
                if store.catalogLoadError != nil {
                    ...
                    ContentUnavailableView {
                        ...
                        Text("Add a machine that runs herden to get started.")
                    } actions: {
    ...
    private func hostWasAdded(_ id: Host.ID) {
        if let onHostAdded {
            onHostAdded(id)
        } else {
            path.append(id)
...
private struct HostConnectionIndicator: View {
    ...
    private var tint: Color {
        ...
        case .connected: Brand.success
        case .pending: .secondary
        case .warning: Brand.attention
        case .unavailable: Brand.fault
        }
    }
}
