...
enum PreflightCheck: CaseIterable, Sendable {
    ...
    /// The herden socket path exists on the Host.
    case herdenInstalled
    ...
    var title: String {
        ...
        case .herdenInstalled: "herden installed"
        case .serverRunning: "herden server running"
...
struct PreflightReport: Equatable, Sendable {
    ...
    static func failure(_ error: TransportError, authMethod: Host.AuthMethod) -> PreflightReport {
        ...
        case .socketNotFound(let path):
            check = .herdenInstalled
            hint =
                "No herden socket at \(path). Install and start herden on the Host, "
                + "or fix the session name."
        case .homeDirectoryUnresolvable(let detail):
            check = .remoteEnvironment
            hint =
                "Could not resolve the remote home directory, so the herden socket "
                + "path is unknown. (\(detail))"
        case .streamLocalOpenFailed(let path):
            check = .serverRunning
            hint =
                "Could not open the herden socket at \(path). Start herden on the Host, "
        ...
        case .protocolVersionMismatch(let server, let supported):
            ...
            hint =
                "The Host speaks herden protocol \(server); this app needs at least "
                + "\(supported). Update herden on the Host."
        case .malformedResponse(let detail):
            check = .protocolCompatible
            hint = "The server's reply did not parse as herden protocol. (\(detail))"
        case .timedOut:
            check = .connection
        ...
        case .apiRejected(let code, let message):
            check = .serverRunning
            hint = "herden answered but rejected the check: \(message) (\(code))"
        ...
        case .herdenBinaryNotFound:
            // Preflight is one connect + ping and never execs herden, so this
            // case cannot fire here. Keep the closed taxonomy total anyway.
            check = .connection
            hint = "The herden CLI was not found on the Host."
