...
/// cannot authorize a destructive request because herden may reuse it.
...
enum WorktreeRemovalRefusal {
    ...
    private static func apiMessage(code: String, serverMessage: String) -> String {
        ...
        case "dirty_worktree_requires_force":
            "herden couldn't remove this worktree because it has modified or untracked files. Commit or discard those changes on the Host, then retry."
        case "not_linked_worktree":
            "herden couldn't remove this worktree because the workspace is not a linked worktree."
        case "workspace_not_found":
            "herden couldn't remove this worktree because the workspace is no longer on the Host."
        case "worktree_operation_in_progress":
            "herden is already changing a worktree on this Host. Wait a moment, then retry."
        default:
            "herden couldn't remove this worktree: \(serverMessage)"
        }
    }
}
