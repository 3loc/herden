...
extension ConsoleAgent {
    ...
    /// directory disables Open Terminal instead of letting herden inherit some
...
final class AgentOpenTerminalStore {
    ...
    private static func presentation(for error: any Error) -> OpenTerminalFailure {
        ...
        case let api as HerdrAPIError:
            OpenTerminalFailure(
                kind: .rejected,
                message: "herden couldn't create the terminal: \(api.message)")
        case TransportError.apiRejected(_, let message):
            OpenTerminalFailure(
                kind: .rejected,
                message: "herden couldn't create the terminal: \(message)")
        default:
            OpenTerminalFailure(
                kind: .ambiguous,
                message:
