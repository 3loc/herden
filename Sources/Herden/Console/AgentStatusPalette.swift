...
/// herden's own agent colours, so the phone and the TUI agree on what green
/// means. herden paints agent state from its Catppuccin palette; the two
...
enum AgentStatusPalette {
    ...
    /// herden's muted-text role. Its Mocha `overlay0` is too dim to carry a
