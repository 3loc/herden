...
final class HostConsoleProjection {
    ...
    /// The last herden `ping` round trip measured on the Host's *current*
    ...
    /// who went and restarted herden came back to the same failure until they
    /// found the Retry button.
    ///
    /// This is deliberately not a retry cadence. The classification that
    /// stopped the loop stands — retrying a stopped herden on reconnect timing
    ...
    private func handle(_ update: EventsSessionUpdate) {
        ...
        case .status(let status):
            ...
            if status == .connected {
                // Subscription acknowledgement precedes this signal. Keep
                // consuming events while the snapshot runs: lifecycle events
                // schedule a follow-up and status revisions survive an older
                // snapshot response.
                publish()
                scheduleResync()
            } else {
    ...
    private func apply(
        ...
        workspaces = snapshot.workspaces
            .map { workspace in
                let cwd = snapshot.panes.first {
                    $0.workspaceID == workspace.workspaceID
                        && Self.isUsableDirectory($0.cwd)
                }?.cwd
    ...
    private static func snapshotErrorMessage(_ error: any Error) -> String {
        ...
        case let apiError as HerdrAPIError:
            "herden rejected the Console sync: \(apiError.message). Retrying…"
        default:
