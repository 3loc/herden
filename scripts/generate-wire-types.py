...
Usage:
    scripts/generate-wire-types.py            # runs `herden api schema --json`
...
Output: Sources/Herden/Transport/Generated/HerdrAPITypes.swift
...
- String enums are emitted as raw-string `RawRepresentable` structs, not
  Swift enums: herden's API has no stability guarantee, so unknown values
...
OUTPUT_PATH = REPO_ROOT / "Sources/Herden/Transport/Generated/HerdrAPITypes.swift"
...
def load_schema(path: str | None) -> dict:
    ...
    try:
        completed = subprocess.run(
            ["herden", "api", "schema", "--json"], capture_output=True, text=True, check=True
        )
    except FileNotFoundError:
        fail("herden not found on PATH; pass --schema <file> instead")
    except subprocess.CalledProcessError as error:
        fail(f"`herden api schema --json` failed: {error.stderr.strip()}")
...
def emit_string_wrapper(name: str, doc: str, values: list[str]) -> str:
    lines = [
        f"/// {doc}",
        "///",
        "/// Closed set in the source schema, but herden's API has no stability",
...
def generate(schema: dict) -> str:
    ...
    header = "\n".join(
        [
            ...
            f"// Source: `herden api schema --json` "
            ...
            "// wrappers, because herden's API has no stability guarantee.",
            "",
            "import Foundation",
        ]
    )
