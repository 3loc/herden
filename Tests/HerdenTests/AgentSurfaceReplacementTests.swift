...
struct AgentSurfaceReplacementTests {
    ...
    @Test func aReplacementSurfaceTakesOverTheFeed() async throws {
        ...
        weak var predecessor: HerdenTerminalView?
    ...
    static func terminals(in root: UIView) -> [HerdenTerminalView] {
        var found: [HerdenTerminalView] = []
        func walk(_ view: UIView) {
            if let terminal = view as? HerdenTerminalView { found.append(terminal) }
    ...
    static func detailViewFactory() -> (ConsoleAgent) -> AgentTerminalView {
        ...
        let console = ConsoleStore(snapshotRetryDelay: .seconds(30)) { _, subscriptions in
            EventsSession(
                ...
                connect: {
                    throw TransportError.streamLocalOpenFailed(path: "/tmp/herden.sock")
                },
