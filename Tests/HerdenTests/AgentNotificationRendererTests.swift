...
struct AgentNotificationRendererTests {
    ...
    @Test func doesNotRenderTheTerminalTask() {
        let alert = AgentNotificationRenderer.alert(
            workspace: "Herden", agentKind: "codex", status: .done)

        #expect(alert == AgentNotificationAlert(title: "Herden · Codex", body: "Done"))
    }
    ...
    @Test func rendersAnUnrecognizedStatusFactually() {
        let alert = AgentNotificationRenderer.alert(
            workspace: "herden", agentKind: "claude", status: AgentStatus(rawValue: "exited"))
    ...
    @Test func fallbackCopyIsGeneric() {
        #expect(AgentNotificationRenderer.fallback.title == "Herden")
