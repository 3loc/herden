...
struct SharedTransferTests {
    ...
    @Test func completedDeliveryLeavesNoHistoryAndCannotReplay() async throws {
        ...
        await #expect(throws: (any Error).self) {
            try await SharedTransferDelivery.run(
                id: record.id, store: reopened, stage: stage, insert: insert)
        }
        #expect(insertions == 1)
        #expect(!FileManager.default.fileExists(atPath: store.preparedFile(record).fileURL.path))
    }

    @Test func aNewShareDiscardsOnlyUnlockedLeftovers() throws {
        let (store, record, root) = try fixture()
        defer { try? FileManager.default.removeItem(at: root) }
        let lease = try #require(try store.lease(record.id))
        try store.discardUnlockedRecords()
        #expect(try store.records().map(\.id) == [record.id])
        lease.release()
        try store.discardUnlockedRecords()
