...
@Suite(
    "HerdenSSH Jump Host gate e2e",
    .enabled(
        if: RealSSHFixture.gate(HerdenSSHJumpHostTestEnvironment.current != nil),
        "requires the disposable two-sshd Jump Host fixture"),
    .serialized)
struct HerdenSSHJumpHostGateE2ETests {
    @Test("protocol 17 ping traverses independent SSH hops")
    func protocolPingTraversesBothHops() async throws {
        let environment = try #require(HerdenSSHJumpHostTestEnvironment.current)
    ...
    func trustIsIndependentAtBothHops() async throws {
        ...
        let first = try await HerdenSSHTransport.connect(
            settings: environment.settings(policy: policy))
        _ = try await first.ping()
        try await first.close()
        let second = try await HerdenSSHTransport.connect(
            settings: environment.settings(policy: policy))
        ...
        let jumpError = await #expect(throws: TransportError.self) {
            _ = try await HerdenSSHTransport.connect(
                settings: environment.settings(policy: policy))
        }
...
private struct HerdenSSHJumpHostTestEnvironment: Decodable, Sendable {
    let host: String
    let jumpPort: UInt16
    ...
    static let current: HerdenSSHJumpHostTestEnvironment? = {
        guard
            let encoded = ProcessInfo.processInfo.environment["HERDEN_SSH_JUMP_E2E_CONFIG"],
            let data = Data(base64Encoded: encoded)
        else {
            return nil
        }
        return try? JSONDecoder().decode(HerdenSSHJumpHostTestEnvironment.self, from: data)
    }()
...
private func expectPing(
    _ connection: SSHConnection,
    environment: HerdenSSHJumpHostTestEnvironment,
    id: String
) async throws {
