...
// fixtures mirror herden 0.7.5 (protocol 17) responses, trimmed to
...
@Suite struct GeneratedWireTypesTests {
    @Test func paneSendInputEncodesInsertWithoutSubmit() throws {
        let params = PaneSendInputParams(
            paneID: "w1:pA", text: "/tmp/herden.abc/file.pdf ")
        let object = try #require(
            JSONSerialization.jsonObject(with: JSONEncoder().encode(params))
                as? [String: Any])

        #expect(object["pane_id"] as? String == "w1:pA")
        #expect(object["text"] as? String == "/tmp/herden.abc/file.pdf ")
        #expect(object["keys"] == nil)
    }
    ...
    @Test func pongResponseRoundTripsCommittedProtocolShape() throws {
        let pong = try roundTrip(
            PongResponse.self,
            #"{"type":"pong","version":"0.9.8","protocol":22,"capabilities":{"live_handoff":true,"detached_server_daemon":true}}"#)

        #expect(pong.version == "0.9.8")
        #expect(pong.protocolVersion == 22)
        #expect(pong.protocolVersion == HerdenSSHTransport.generatedProtocolVersion)
    }
    ...
    @Test func workspaceInfoResponseRoundTripsLiveCapture() throws {
        // `workspace.rename` live capture (herden 0.7.5, sanitized).
    ...
    @Test func worktreeCreatedResponseRoundTripsLiveCapture() throws {
        // `worktree.create` live capture (herden 0.7.5, sanitized, #97): the
    ...
    @Test func worktreeListResponseRoundTripsNamedAndDetachedEntries() throws {
        let json = #"{"type":"worktree_list","source":{"repo_key":"/Users/u/Herden/.git","repo_name":"Herden","repo_root":"/Users/u/Herden","source_checkout_path":"/Users/u/Herden","source_workspace_id":"wV"},"worktrees":[{"path":"/Users/u/Herden","branch":"main","is_bare":false,"is_detached":false,"is_prunable":false,"is_linked_worktree":false,"label":"Herden","open_workspace_id":"wV"},{"path":"/Users/u/wt/Herden/feat-x","branch":null,"is_bare":false,"is_detached":true,"is_prunable":false,"is_linked_worktree":true,"label":"Herden","open_workspace_id":"w15"}]}"#

        let response = try roundTrip(WorktreeListResponse.self, json)

        #expect(response.source.repoKey == "/Users/u/Herden/.git")
    ...
    @Test func unknownEnumValuesDecodeLeniently() throws {
        // herden's API has no stability guarantee: new enum values must not
    ...
    @Test func worktreeCreateParamsEncodeOnlyProvidedFields() throws {
        // Nil branch/base/label/path must drop their keys so herden applies
    ...
    @Test func agentRenameParamsOmitNilNameToClearIt() throws {
        // Verified live (herden 0.7.5): omitting `name` clears the custom
