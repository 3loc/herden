...
struct StartAgentStoreTests {
        ...
        #expect(
            command.contains(
                "command -v muse >/dev/null 2>&1"
                    + " && printf \"__HERDEN_AGENT_KIND__=%s\\n\" \"muse\""))
    }
    ...
    @Test func canSubmitRequiresAHostWorkspaceAndDetectedAgent() async {
        ...
        store.name = "Reviewer"
        #expect(store.canSubmit == false)  // uppercase violates herden's rule
        store.name = "reviewer"
    ...
    @Test func newWorkspaceServerRejectionUsesExistingErrorPresentation() async {
        ...
        #expect(store.state == .failed("herden rejected the command: no such directory"))
        let next = makeStore(
            hosts: [host],
            workspaces: { _ in [ConsoleWorkspace(id: "w1", label: "Proj")] },
            recents: recents,
    ...
    /// Without an origin the launch carries no directory, leaving herden to
    ...
    @Test func submitSurfacesAServerRejection() async {
        ...
        #expect(store.state == .failed("herden rejected the command: no such workspace"))
    }

    @Test func unsupportedMuseIncludesHostUpgradeAdvice() async {
        let host = Host.fixture()
        let recorder = StartRecorder()
        ...
        #expect(
            store.state
                == .failed(
                    "herden rejected the command: unsupported interactive agent kind muse. "
                        + "Update Herden Host to v0.9.0 or later for Muse support, "
                        + "or choose another supported Agent."))
        #expect(recorder.params.map(\.kind) == ["muse"])
    }

    @Test func unsupportedNonMuseKeepsGenericServerCopy() async {
        let host = Host.fixture()
        let recorder = StartRecorder()
        recorder.error = HerdrAPIError(
            code: "unsupported_agent_kind",
        ...
        #expect(
            store.state
                == .failed("herden rejected the command: unsupported interactive agent kind qwen"))
    }
