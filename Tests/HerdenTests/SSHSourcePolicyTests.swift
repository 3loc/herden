...
///
/// `Packages/HerdenSSH/Scripts/verify-native.sh` proves the same algorithm
...
struct SSHSourcePolicyTests {
    ...
    func methodPreferencesExcludeObsoleteAlgorithms() throws {
        let driver = try Self.read("Packages/HerdenSSH/Sources/HerdenSSH/SessionDriver.swift")
    ...
    func packageAcceptsNoPrivateKeyMaterial() throws {
        let sources = try Self.swiftAndCSources(under: "Packages/HerdenSSH/Sources")
    ...
    func transportPassesOnlyPublicKeyAndSigner() throws {
        let adapter = try Self.read("Sources/Herden/Transport/HerdenSSHTransport.swift")
