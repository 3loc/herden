...
/// natural recovery (restart herden, come back) does nothing. The window is the
...
/// classification: `herdenStoppedWhileAwayFailsTheHostWithItsSetupGuidance`
/// still pins that a stopped herden fails rather than retries, and the cases
...
struct AppForegroundRecoveryTests {
    ...
    /// herden stopped while the app was away, SSH itself fine: the ping opens a
    /// stream-local channel, herden refuses it, and the transport's probe
    ...
    /// return while herden is down.
    @Test func herdenStoppedWhileAwayFailsTheHostWithItsSetupGuidance() async throws {
        let host = Host.fixture()
        let stale = ScriptedTransport(snapshot: .openSession)
        let socketPath = "/home/dev/.config/herden/herden.sock"
        ...
        try await waitUntil("a stopped herden should fail the Host, not retry it") {
    ...
    /// restarted herden has no reason to expect they need it.
    @Test func aFailedHostIsAskedAgainOnTheNextForegroundReturn() async throws {
        ...
        // herden stops while the app is away. The return's ping opens a
        ...
        activity.didEnterBackground()
        activity.didBecomeActive()
        try await waitUntil("the stopped herden should fail the Host") {
            store.hostStatuses[host.id] == .failed(.streamLocalOpenFailed(path: socketPath))
        }

        // The user does exactly what the guidance asks — restarts herden — and
    ...
    @Test func aStillBrokenHostReturnsToFailedWithoutAFlickerOfRecovery() async throws {
        ...
        // Every dial after the first finds herden still down, however many the
        ...
        #expect(
            failure.presentation.message == """
                herden is not running on this Host. If it is running, check SSH \
    ...
    @Test(arguments: [
        TransportError.socketNotFound(
            path: "/home/dev/.config/herden/sessions/work/herden.sock"),
    ...
    @Test func aFailedHostsRestartHoldsTheForegroundActivation() async throws {
        ...
        let failure = TransportError.streamLocalOpenFailed(
            path: "/home/dev/.config/herden/herden.sock")
        let stopped = ScriptedTransport(snapshot: .openSession)
        let restarted = ScriptedTransport(snapshot: .openSession)
        let store = makeStore(attempts: ConnectionAttemptQueue([
            .success(stopped), .success(restarted),
