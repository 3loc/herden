...
struct RequestFailurePresentationTests {
    @Test func notificationRegistrationKeepsItsContextSpecificArms() {
        #expect(
            NotificationPreferencesStore.message(
                ...
                == "Install the Herden plugin on this Host, then check again.")
        #expect(
            NotificationPreferencesStore.message(
                for: NotificationRegistrationError.pluginProbeFailed(detail: "boom"))
                == "Could not check the Herden plugin on this Host. "
    ...
    @Test func notificationRegistrationReplacedBranchRendersTheSharedMessage() {
        #expect(
            NotificationPreferencesStore.message(for: TransportError.herdenBinaryNotFound)
                == TransportError.herdenBinaryNotFound.presentation.message)
    }
    ...
    @Test func attachReplacedBranchRendersTheSharedMessage() {
        #expect(
            AttachTerminalStore.message(for: TransportError.herdenBinaryNotFound)
                == TransportError.herdenBinaryNotFound.presentation.message)
    }
    ...
    @Test func composerKeepsItsContextSpecificArms() {
        ...
        #expect(
            AgentComposerStore.message(
                ...
                == "herden rejected the message: still working")
        #expect(
    ...
    @Test func composerReplacedBranchRendersTheSharedMessage() {
        ...
        #expect(
            AgentComposerStore.message(for: TransportError.herdenBinaryNotFound)
                == TransportError.herdenBinaryNotFound.presentation.message)
    }
}
