...
struct PreflightReportTests {
    ...
    @Test func failurePassesEarlierChecksAndBlocksLaterOnes() {
        ...
        guard case .failed(let hint) = report[.herdenInstalled] else {
            Issue.record("herden check should fail")
            return
        }
    ...
    /// SSH access plus a running herden are the whole Host contract now
    ...
    @Test func streamLocalFailureGuidesTowardForwardingRatherThanInstallation() {
        let report = PreflightReport.failure(
            .streamLocalOpenFailed(path: "/home/dev/.config/herden/herden.sock"),
    ...
    @Test(arguments: [
        ...
        // Not reachable from connect+ping (preflight never execs herden).
        (.herdenBinaryNotFound, .connection),
        (.jumpHostFailed(.sshUnreachable(detail: "refused")), .connection),
        (.tcpForwardingUnavailable, .connection),
        (.socketNotFound(path: "/home/dev/.config/herden/herden.sock"), .herdenInstalled),
        (.homeDirectoryUnresolvable(detail: "no $HOME"), .remoteEnvironment),
        (.streamLocalOpenFailed(path: "/home/dev/.config/herden/herden.sock"), .serverRunning),
    ...
    @Test func plainFailureAttachesTheGivenHintToTheGivenCheck() {
        ...
        #expect(report[.herdenInstalled] == .blocked)
    }
}
