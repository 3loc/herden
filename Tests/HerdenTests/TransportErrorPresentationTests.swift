...
struct TransportErrorPresentationTests {
    private static let homebrewPATH =
        ...
        + "or symlink herden into ~/.local/bin."

    private static let socketGuidance =
        "herden is not running on this Host. If it is running, check SSH stream-local forwarding."
    ...
    @Test func streamLocalOpenFailureRecomposesByteIdentically() {
        let presentation = TransportError.streamLocalOpenFailed(path: "/tmp/herden.sock")
            .presentation
        #expect(presentation.message == Self.socketGuidance)
        #expect(presentation.summary == "herden is not running on this Host")
    ...
    @Test func theEightUnchangedMessagesRecomposeByteForByte() {
        let cases: [(TransportError, String)] = [
            (
                .streamLocalOpenFailed(path: "/tmp/herden.sock"),
                Self.socketGuidance
            ),
            (
            ...
            (
                ...
                "herden is not on this Host's SSH PATH. \(Self.homebrewPATH)"
            ),
            (
    ...
    @Test func retryableSummariesAndSuggestions() {
        ...
        #expect(rejected.presentation.summary == "herden rejected the request")
    ...
    @Test func jumpHostFallbackDropsAnUnretargetedSuggestion() {
        let fallbacks: [TransportError] = [
            .socketNotFound(path: "/tmp/herden.sock"),
            .streamLocalOpenFailed(path: "/tmp/herden.sock"),
            .herdenBinaryNotFound,
    ...
    @Test func socketNotFoundIncludesThePath() {
        let presentation = TransportError.socketNotFound(path: "/tmp/herden.sock").presentation
        #expect(presentation.summary == "The herden socket was not found")
        #expect(presentation.detail == "/tmp/herden.sock")
        #expect(presentation.recoverySuggestion == "Check this Host's session.")
        #expect(
            presentation.message
                == "The herden socket was not found: /tmp/herden.sock. Check this Host's session.")
    }
    ...
    @Test func protocolVersionMismatchNamesTheUpdate() {
        let presentation = TransportError.protocolVersionMismatch(server: 18, supported: 19)
            .presentation
        #expect(presentation.summary == "Incompatible herden protocol")
        #expect(presentation.detail == "herden speaks protocol 18; this app needs at least 19")
        #expect(presentation.recoverySuggestion == "Update herden on the Host.")
        #expect(
            presentation.message
                == "Incompatible herden protocol: herden speaks protocol 18; this app needs at least 19. "
                + "Update herden on the Host.")
    }
    ...
    @Test func homeDirectoryAndMalformedResponsePreserveDetail() {
        ...
        #expect(malformed.summary == "herden returned an invalid response")
        #expect(malformed.detail == "{not-json}")
        #expect(malformed.recoverySuggestion == "Check its version.")
        #expect(
            malformed.message
                == "herden returned an invalid response: {not-json}. Check its version.")
    }
    ...
    @Test func changedRetryableMessagesMatchTheApprovedWording() {
        ...
        #expect(
            TransportError.apiRejected(code: "pane_not_found", message: "gone").presentation.message
                == "herden rejected the request: gone (pane_not_found).")
    }
}
