...
struct HerdenSSHDirectStreamLocalE2ETests {
    ...
    func missingSocketMapsToSocketNotFound() async throws {
        let environment = try #require(DirectStreamLocalTestEnvironment.current)
        let connection = try await environment.deviceKeyConnection(to: environment.endpoint)
        let transport = HerdenSSHTransport(
    ...
    /// itself failing, which is what a Host whose herden is absent from the
    ...
    func failedWakeKeepsTheForwardingDenial() async throws {
        let environment = try #require(DirectStreamLocalTestEnvironment.current)
        // An unresolvable absolute path: the fixture forces a `herden` stub onto
        // the session PATH, so only bypassing PATH entirely reproduces "herden
        ...
        let transport = try await HerdenSSHTransport.connect(
            settings: environment.settings(
                ...
                wakeCommand: "/nonexistent/herden remote-client-bridge"))
...
private struct DirectStreamLocalTestEnvironment: Decodable, Sendable {
    ...
    static let current: DirectStreamLocalTestEnvironment? = {
        guard
            let encoded = ProcessInfo.processInfo.environment["HERDEN_SSH_E2E_CONFIG"],
