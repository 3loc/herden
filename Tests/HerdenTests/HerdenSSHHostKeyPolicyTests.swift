...
@Suite("HerdenSSH Host Key policy")
struct HerdenSSHHostKeyPolicyTests {
    private func hostKey(algorithm: String, marker: String) -> HerdenSSH.SSHHostKey {
        var blob = Data()
        ...
        return HerdenSSH.SSHHostKey(algorithm: algorithm, key: blob)
    }
    ...
    @Test func firstConnectAcceptPersistsAlgorithmAwareFingerprint() async throws {
        ...
        let verifier = HerdenSSHHostKeyVerifier(
            host: "host.example",
            port: 22,
