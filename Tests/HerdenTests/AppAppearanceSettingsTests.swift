...
struct AppAppearanceSettingsTests {
    ...
    @Test func brandSurfacesAndInkResolveDifferentlyForDayAndNight() {
        #expect(hex(Brand.background, style: .light) == 0xFFFCF0)
        #expect(hex(Brand.background, style: .dark) == 0x100F0F)
        #expect(hex(Brand.ink, style: .light) == 0x100F0F)
        #expect(hex(Brand.ink, style: .dark) == 0xF2F0E5)
        #expect(hex(Brand.primary, style: .light) == 0x205EA6)
        #expect(hex(Brand.primary, style: .dark) == 0x4385BE)
    }
