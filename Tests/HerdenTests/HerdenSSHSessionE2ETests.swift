...
@Suite(
    "HerdenSSH session e2e",
    .enabled(
        if: RealSSHFixture.gate(HerdenSSHTestEnvironment.isAvailable),
        "requires the disposable unprivileged sshd fixture"),
    .serialized)
struct HerdenSSHSessionE2ETests {
    ...
    func handshakeReturnsHostKeyBeforeAuthentication() async throws {
        let environment = try #require(HerdenSSHTestEnvironment.current)
    ...
    @Test(
        ...
        .enabled(
            if: HerdenSSHTestEnvironment.hasPasswordFixture,
    ...
    @Test(
        ...
        .enabled(
            if: RealSSHFixture.gate(HerdenSSHTestEnvironment.hasLegacyEndpoint),
    ...
    @Test(
        ...
        .enabled(
            if: RealSSHFixture.gate(HerdenSSHTestEnvironment.hasRestrictedEndpoint),
    ...
    @Test(
        ...
        .enabled(
            if: RealSSHFixture.gate(HerdenSSHTestEnvironment.hasStallEndpoint),
...
private struct HerdenSSHTestEnvironment: Sendable {
    ...
    /// and was unreachable.
    static let current: HerdenSSHTestEnvironment? = {
        let environment = ProcessInfo.processInfo.environment
        guard
            let encoded = environment["HERDEN_SSH_E2E_CONFIG"],
            let data = Data(base64Encoded: encoded),
            let configuration = try? JSONDecoder().decode(
                HerdenSSHTestConfiguration.self,
                from: data),
            let deviceKey = try? RealSSHFixture.deviceKey(seed: configuration.deviceKeySeed)
        else {
            return nil
        }
        return HerdenSSHTestEnvironment(
...
private struct HerdenSSHTestConfiguration: Decodable {
