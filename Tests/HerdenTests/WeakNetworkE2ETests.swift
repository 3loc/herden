...
struct WeakNetworkE2ETests {
    ...
    func concurrentRPCsSurviveADegradedLink() async throws {
        ...
        let transport = try await HerdenSSHTransport.connect(settings: fixture.settings())
    ...
    func abruptLinkLossSurfacesAndRecovers() async throws {
        ...
        let recovered = try await HerdenSSHTransport.connect(settings: settings)
    ...
    func eventsSessionRecoversAcrossBackgroundingOnADegradedLink() async throws {
        ...
        let session = EventsSession(
            ...
            connect: { try await HerdenSSHTransport.connect(settings: capturedSettings) })
...
struct WeakNetworkFixture: Sendable {
    let environment: HerdenSSHTransportBehaviorEnvironment
    let port: UInt16
    let control: WeakNetworkProxyControl

    static var current: WeakNetworkFixture? {
        guard
            let environment = HerdenSSHTransportBehaviorEnvironment.current,
