...
struct LicenseNoticeInventoryTests {
    ...
    @Test func ghosttyStackAndFontNoticesShipVerbatim() throws {
        ...
        for id in ["IBMPlexMono", "InstrumentSerif", "Inter", "JetBrainsMono"] {
            let notice = try #require(byID[id])
    ...
    @Test func everyDiscoveredDependencyDeclarationIsCoveredByInventory() throws {
        ...
        // discover Package.resolved pins, HerdenSSH binary targets, Herden app
        ...
        try assertCoverage(
            discovered: Set(try loadHerdenSSHBinaryTargetNames()),
            mapped: coverage.herdenSSHBinaryTargets,
            inventoryIDs: inventoryIDs,
            source: "HerdenSSH binaryTarget")

        try assertCoverage(
            discovered: Set(try loadHerdenAppProjectPackageNames()),
            mapped: coverage.projectPackages,
            inventoryIDs: inventoryIDs,
            source: "project.yml Herden package dependency")
        ...
        let binaryTargets = try loadHerdenSSHBinaryTargets()
        #expect(!binaryTargets.isEmpty)
        for target in binaryTargets {
            let url = repositoryRoot
                .appendingPathComponent("Packages/HerdenSSH")
            ...
            #expect(
                coverage.herdenSSHBinaryTargets[target.name] != nil,
    ...
    private func artifactNoticeURL(named fileName: String) -> URL {
        repositoryRoot
            .appendingPathComponent("Packages/HerdenSSH/Artifacts/Notices", isDirectory: true)
    ...
    private func loadPackageResolvedIdentities() throws -> [String] {
        let url = repositoryRoot
            .appendingPathComponent("Herden.xcodeproj/project.xcworkspace/xcshareddata/swiftpm")
    ...
    private func loadHerdenSSHBinaryTargetNames() throws -> [String] {
        try loadHerdenSSHBinaryTargets().map(\.name).sorted()
    }

    private func loadHerdenSSHBinaryTargets() throws -> [BinaryTarget] {
        let source = try String(
            contentsOf: repositoryRoot.appendingPathComponent("Packages/HerdenSSH/Package.swift"),
            encoding: .utf8)
    ...
    /// Package names linked from the Herden *application* target in
    /// `project.yml` (not the test or extension targets).
    private func loadHerdenAppProjectPackageNames() throws -> [String] {
        let source = try String(
            contentsOf: repositoryRoot.appendingPathComponent("project.yml"),
            encoding: .utf8)
        let lines = source.split(separator: "\n", omittingEmptySubsequences: false).map(String.init)

        // Locate `targets:` → `Herden:` → its `dependencies:` list; stop at the
        // next top-level target (`HerdenNotificationService:`) or `schemes:`.
        guard let targetsIndex = lines.firstIndex(where: { $0 == "targets:" }) else {
            throw DiscoveryError.projectYML("missing targets:")
        }
        guard
            let herdenIndex = lines[targetsIndex...].firstIndex(where: { $0 == "  Herden:" })
        else {
            throw DiscoveryError.projectYML("missing Herden application target")
        }
        guard
            let depsIndex = lines[herdenIndex...].firstIndex(where: { $0 == "    dependencies:" })
        else {
            throw DiscoveryError.projectYML("Herden target has no dependencies:")
        }
        ...
        for line in lines[(depsIndex + 1)...] {
            ...
            // `- package: Name` under the Herden dependencies list.
    ...
    private func loadBundledFontFamilyStems() throws -> [String] {
        let fonts = repositoryRoot
            .appendingPathComponent("Sources/Herden/Resources/Fonts", isDirectory: true)
    ...
    @Test func settingsViewWiresAcknowledgementsThroughSharedDestination() throws {
        ...
        let source = try String(
            contentsOf: repositoryRoot
                .appendingPathComponent("Sources/Herden/Settings/SettingsView.swift"),
            encoding: .utf8)
