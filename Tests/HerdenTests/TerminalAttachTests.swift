...
/// pin the herden CLI to the Host's socket via `HERDR_SOCKET_PATH` (a
...
struct TerminalAttachTests {
    ...
    @Test func insertingInTheMiddleLeavesTheCursorAfterTheInsertion() async throws {
        ...
        try await waitForEditableText("blue file", in: terminal)
        terminal.movePromptCursor(by: -5)
        terminal.sendReliableExternalInput(Data(" green".utf8))
        try await waitForEditableText("blue green file", in: terminal)
        let selection = try #require(terminal.selectedTextRange)
        #expect(terminal.offset(from: terminal.beginningOfDocument, to: selection.start) == 10)
        terminal.sendReliableExternalInput(Data([0x7F]))
        try await waitForEditableText("blue gree file", in: terminal)
    }

    @MainActor
    @Test func dictationCorrectionsUpdateTheEditableLineBeforeCursorMovement() async throws {
        let terminal = TerminalScreenView.makeConfiguredTerminal()
        terminal.sendReliableExternalInput(Data("open blue".utf8))
        try await waitForEditableText("open blue", in: terminal)
        terminal.sendReliableExternalInput(Data("\u{7f}\u{7f}\u{7f}\u{7f}green".utf8))
        try await waitForEditableText("open green", in: terminal)
        terminal.movePromptCursor(by: -5)
        terminal.sendReliableExternalInput(Data("the ".utf8))
        try await waitForEditableText("open the green", in: terminal)
    }

    @MainActor
    @Test func draggingTheCursorCountsCharactersInsteadOfUTF16CodeUnits() async throws {
        var sent: [Data] = []
        let terminal = TerminalScreenView.makeConfiguredTerminal(onSend: { sent.append($0) })
        terminal.sendReliableExternalInput(Data("a🙂界b".utf8))
        try await waitForEditableText("a🙂界b", in: terminal)
        terminal.movePromptCursor(by: -3)
        let deadline = ContinuousClock.now + .seconds(1)
        while sent.count < 2, ContinuousClock.now < deadline { await Task.yield() }
        #expect(sent.last == Data("\u{1B}[D\u{1B}[D\u{1B}[D".utf8))
        let selection = try #require(terminal.selectedTextRange)
        #expect(terminal.offset(from: terminal.beginningOfDocument, to: selection.start) == 1)
    }

    @MainActor
    private func waitForEditableText(_ expected: String, in terminal: HerdenTerminalView) async throws {
        let deadline = ContinuousClock.now + .seconds(1)
        while ContinuousClock.now < deadline {
            let range = try #require(terminal.textRange(
                from: terminal.beginningOfDocument, to: terminal.endOfDocument))
            if terminal.text(in: range) == expected { return }
            await Task.yield()
        }
        let range = try #require(terminal.textRange(
            from: terminal.beginningOfDocument, to: terminal.endOfDocument))
        #expect(terminal.text(in: range) == expected)
    }
    ...
    @Test func attachOutputPumpWithholdsStartupChatterUntilTheHandshake() async throws {
        ...
        let source = HerdenSSHAttachOutputGate.makeStream()

        let cleanEnd = try await HerdenSSHTransport.runAttachPumps(
            channel: channel,
            input: input,
            output: source.gate,
    ...
    func inputFailureFlushesWithheldStartupDiagnosticExactlyOnce() async throws {
        ...
        let pump = Task {
            do {
                _ = try await HerdenSSHTransport.runAttachPumps(
                    channel: channel,
                    input: input,
                    output: source.gate,
                    requestTimeout: .seconds(1))
    ...
    func anAlreadyCancelledSecondConsumerCannotEndTheFirst() async throws {
        let gate = HerdenSSHAttachOutputGate()
    ...
    private static func waitForParkedConsumer(
        _ gate: HerdenSSHAttachOutputGate,
    ...
    @Test func terminalTouchPanEmitsSemanticRemoteTUIMouseWheelInput() {
        ...
        // The terminal owns one vertical scroll pan and one horizontal
        // navigation/editing pan. Ghostty's built-in direct-touch pans stay
        // disabled so they cannot compete with either app gesture.
        #expect(enabledTouchPans.count == 2)
    ...
    @Test func agentQuickKeysEncodeExpectedBytes() {
        ...
        #expect(AgentQuickKey.controlC.bytes(applicationCursor: false) == [0x03])
    ...
    @Test func agentQuickKeysBypassDisplayOnlyInputWithoutEnablingTyping() async {
        ...
        terminal.sendQuickKey(.controlC)
        terminal.sendReliableExternalInput(Data("dictated ".utf8))
        terminal.receive(Data("\u{1B}[?1h".utf8))
        terminal.sendQuickKey(.up)
        await Task.yield()

        #expect(
            sent == Data([0x1B, 0x5B, 0x5A, 0x03])
                + Data("dictated ".utf8)
                + Data([0x1B, 0x4F, 0x41]))
    }
    ...
    @Test func injectableAttachCommandRidesThrough() throws {
        ...
        #expect(
            command == "/bin/sh -c '\(HerdenHostPath.pathExport); "
    ...
    @Test func execUsesTheSocketScopeAndTakeoverFlag() throws {
        let command = try HerdenSSHTransport.attachExecCommand(
            ...
            request: TerminalAttachRequest(
                ...
                cols: 80,
                rows: 24),
            socketPath: "/home/u/.config/herden/sessions/dev/herden.sock")

        #expect(
            command == "/bin/sh -c '\(HerdenHostPath.pathExport); "
                + "export HERDR_SOCKET_PATH=\"$2\"; "
                + "printf \"\(AttachBootstrapHandshake.markerPrintfFormat)\"; "
                + "exec herden agent attach \"$1\" --takeover' attach "
                + "'w1:p1' '/home/u/.config/herden/sessions/dev/herden.sock'")
    ...
    @Test func terminalTargetSelectsTerminalAttachWithTheSameBootstrapAndSocketScope() throws {
        let command = try HerdenSSHTransport.attachExecCommand(
            agentAttachCommand: "herden agent attach",
            terminalAttachCommand: "herden terminal attach",
            request: TerminalAttachRequest(
        ...
        #expect(command.contains("exec herden terminal attach \"$1\" --takeover"))
    ...
    func unquotableTargetsAreRefused(target: String) {
        ...
        #expect(throws: TransportError.self) {
            _ = try HerdenSSHTransport.attachExecCommand(
                attachCommand: "herden agent attach",
    ...
    /// `herden agent attach`. Without it, the pure gate tests below can pass
    ...
    @Test func attachExecCommandWiresTheBootstrapHandshakeMarker() throws {
        ...
        let execRange = try #require(command.range(of: "exec herden agent attach"))
        #expect(printfRange.upperBound <= execRange.lowerBound)
    }

    @Test func attachExit127OnBareHerdrIsAMissingBinary() {
        #expect(
            HerdenSSHTransport.attachChannelFailure(
                exitStatus: 127, attachCommand: "herden agent attach")
    ...
    @Test func attachExit127OnAnAbsoluteHerdrStaysAChannelFailure() {
        #expect(
            HerdenSSHTransport.attachChannelFailure(
                exitStatus: 127,
                attachCommand: "/nonexistent/herden agent attach")
                == .channelFailed(detail: "attach channel: remote exit status 127"))
    }
    ...
    @Test func attachNonzeroExitBesides127StaysAChannelFailure() {
        #expect(
            HerdenSSHTransport.attachChannelFailure(
                exitStatus: 23, attachCommand: "herden agent attach")
    ...
    @Test func gateWithholdsStartupChatterUntilTheHandshake() {
        ...
        let noise = Data(
            ("ssh rc startup chatter\r\n"
                + #"literal printf "\033_herden-attach\033\134" text"#
                + "\r\n").utf8)
...
private actor FakeAttachPTYChannel: HerdenSSHAttachChannel {
