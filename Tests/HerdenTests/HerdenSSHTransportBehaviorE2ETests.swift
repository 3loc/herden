...
@Suite(
    "HerdenSSH Transport behavior e2e",
    .enabled(
        if: RealSSHFixture.gate(HerdenSSHTransportBehaviorEnvironment.current != nil),
        "requires the disposable direct and Jump Host fixtures"),
    .serialized)
struct HerdenSSHTransportBehaviorE2ETests {
    ...
    func protocolFloorAdmitsNewerAndRefusesOlder() throws {
        #expect(HerdenSSHTransport.minimumProtocolVersion == 17)
        #expect(HerdenSSHTransport.generatedProtocolVersion == 22)
        ...
        #expect(
            ...
            _ = try HerdenSSHTransport.serverInfo(
                from: PongResponse(protocolVersion: 16, version: "ancient"))
        }

        // Floor through generated (17–22): usable, no notice. Older versions stay
        // connectable after the snapshot bump; the live fake-herden fixture
        // still speaks 17 and is not rewritten to require 20 at runtime.
        for version in [
            HerdenSSHTransport.minimumProtocolVersion,
            18,
            19,
            20,
            21,
            HerdenSSHTransport.generatedProtocolVersion,
        ] {
            let info = try HerdenSSHTransport.serverInfo(
        ...
        let newer = try HerdenSSHTransport.serverInfo(
            from: PongResponse(
                protocolVersion: HerdenSSHTransport.generatedProtocolVersion + 1,
                version: "future"))
        #expect(newer.exceedsGeneratedProtocol)
        #expect(newer.version == "future")
    }

    /// The exact reply the user's live herden 0.8.0 returns, captured from
    /// `~/.config/herden/herden.sock`. Pins the regression itself rather than a
    /// synthetic version number.
    @Test("the live herden 0.8.0 pong connects rather than being refused")
    func liveZeroEightZeroPongIsAccepted() throws {
        let pong = try JSONDecoder().decode(
            ...
            from: Data(
                ...
                """#.utf8))
        let info = try HerdenSSHTransport.serverInfo(from: pong)
    ...
    /// Protocol 20 remains compatible with the committed protocol-22 snapshot.
    /// A 0.8.2-shaped pong must not raise the newer-Host notice.
    ...
    /// The ADR 0011 Host contract in one test: SSH access plus a running herden.
    ...
    /// real home probe and reaches herden with no Host-side helper in the path.
    ...
    func notificationPluginProbeFailuresStayTyped() async throws {
        ...
        // Installed but switched off: herden will not run it, so it counts as
        ...
        disabled.pluginListCommand =
            ...
            + "{\"plugin_id\":\"herden\",\"enabled\":false}]}}'"
        #expect(
            try await notificationRegistrationFailure(settings: disabled)
                == .pluginNotInstalled)

        // herden missing from the Host's PATH: the probe exits nonzero.
        var missingBinary = environment.directSettings()
        missingBinary.pluginListCommand = "/nonexistent/herden plugin list --json"
        try await expectPluginProbeFailure(
            settings: missingBinary, "a Host without herden on PATH")

        // The probe runs and succeeds but answers with something that is not
        // a plugin list.
        var unparseable = environment.directSettings()
        unparseable.pluginListCommand = "printf '%s' 'herden: not json'"
        try await expectPluginProbeFailure(
    ...
    func attachIsExclusive() async throws {
        let environment = try #require(HerdenSSHTransportBehaviorEnvironment.current)
        let transport = try await HerdenSSHTransport.connect(settings: environment.directSettings())
    ...
    func missingEventsAckPreservesConnection() async throws {
        ...
        let transport = try await HerdenSSHTransport.connect(settings: settings)
        defer { Task { try? await transport.close() } }
    ...
    func staleSocketWakeIsBounded() async throws {
        ...
        let recovered = try await HerdenSSHTransport.connect(settings: recovering)
        ...
        let failed = try await HerdenSSHTransport.connect(settings: failing)
        ...
        let unrecovered = try await HerdenSSHTransport.connect(settings: ineffective)
    ...
    func hungWakeSurfacesTimedOut() async throws {
        ...
        let reset = try await HerdenSSHTransport.connect(settings: resetting)
    ...
    func cancelledRequestPreservesReuse() async throws {
        ...
        let observer = try await HerdenSSHTransport.connect(settings: observerSettings)
    ...
    /// CLAUDE.md's load-bearing herden 0.7.5 fact: `agent.start` on a pane whose
    /// shell has not reached its prompt is refused with `agent_pane_busy`, and
    /// the Transport waits the shell out rather than surfacing it. herden 0.8.0
    ...
    /// shell must surface herden's refusal rather than retry forever, and the
    /// wait before it does so must be long enough to cover a real shell's boot.
    @Test("a shell that never boots exhausts the retry budget and surfaces herden's refusal")
    ...
    /// Closing the channel without a herden error envelope is ambiguous: the
    ...
    /// directory. Dropping the cwd is invisible in the reply — herden answers
    ...
    @Test("agent start leaves terminal colours local to each client")
    func agentStartDoesNotSendTerminalColors() async throws {
        let environment = try #require(HerdenSSHTransportBehaviorEnvironment.current)
        let transport = try await HerdenSSHTransport.connect(
            settings: environment.directSettings())
        defer { Task { try? await transport.close() } }
        ...
        #expect(
            recorded[1]
                == "agent.start {\"kind\":\"codex\",\"name\":\"fixture\",\"name_is_user_set\":false,\"pane_id\":\"pane:\(token)\"}")
    }
    ...
    /// so discovery output is not trusted to stay inside herden's grammar. One
    ...
    @Test("session discovery refuses a name outside herden's grammar")
    func sessionDiscoveryRefusesNamesOutsideTheGrammar() async throws {
        ...
        await #expect(
            throws: TransportError.malformedResponse(
                "herden session list returned an invalid session name")
        ) {
            _ = try await transport.listSessions()
        }
    }
    ...
    func agentDiscoveryIgnoresLoginNoiseUnknownKindsAndDuplicates() async throws {
        ...
        settings.agentDiscoveryCommand =
            ...
            + "'__HERDEN_AGENT_KIND__=codex' "
            + "'__HERDEN_AGENT_KIND__=notarealagent' "
            + "'__HERDEN_AGENT_KIND__=codex' "
            + "'__HERDEN_AGENT_KIND__=claude' "
            + "'__HERDEN_AGENT_KIND__=qwen' "
            + "'last login: never'"
        let transport = try await HerdenSSHTransport.connect(settings: settings)
        defer { Task { try? await transport.close() } }
    ...
    func firstHomeRelativeRequestSpendsOneTotalDeadline() async throws {
        ...
        settings.homeCommand =
            "/bin/sh -c 'sleep 3; printf \"__HERDEN_HOME__=%s\\n\" \"$1\"' home \(quotedHome)"
        let transport = try await HerdenSSHTransport.connect(settings: settings)
        defer { Task { try? await transport.close() } }
    ...
    /// alone is not evidence that the requested value reached herden. Record the
    ...
    /// Omission clears a custom name in herden 0.7.5. JSON null is not the wire
    ...
    /// `pane.read` has no cursor and the options choose what herden samples.
    /// Assert both directions: exact params into herden, typed result back out.
    ...
    /// A syntactically valid herden error envelope is not a channel failure.
    /// Numeric codes normalize to strings in the Transport's domain error.
    @Test("a herden error envelope surfaces as a typed API rejection")
    func herdenErrorEnvelopeSurfacesAsATypedAPIRejection() async throws {
        let environment = try #require(HerdenSSHTransportBehaviorEnvironment.current)
        let transport = try await HerdenSSHTransport.connect(
            settings: environment.directSettings())
        defer { Task { try? await transport.close() } }
    ...
    /// The session boundary converts the Transport's typed herden rejection to
    /// the public failure taxonomy that drives reconnect and user guidance.
    @Test("the session maps a herden rejection to apiRejected")
    func eventsSessionMapsAHerdrRejectionToAPIRejected() async throws {
        ...
        let session = EventsSession(
            ...
            connect: { try await HerdenSSHTransport.connect(settings: settings) },
    ...
    /// of its own rather than guessing at herden's — 0.7.5's refusal of an
    ...
    /// see `scripts/fixtures/fake-herden-streamlocal.py`.
    ...
    private static func recordedRequests(
        from transport: HerdenSSHTransport,
        token: String
    ) async throws -> [String] {
        let read = try await transport.readPane(
    ...
    private func exerciseProductionConnector(host: Host) async throws {
        ...
        #expect(transport is HerdenSSHTransport)
    ...
    private func exerciseNotificationFiles(
        ...
        let configDirectory =
            "\(homePath)/.herden-ci/notify-\(UUID().uuidString.lowercased())"
        ...
        let missingTransport = try await HerdenSSHTransport.connect(settings: missingSettings)
    ...
    private func exerciseEventsStream(settings: SSHTransportSettings) async throws {
        ...
        try await withThrowingTaskGroup(of: ServerInfo.self) { group in
            for _ in 0..<HerdenSSHTransport.maxConcurrentForwardingChannels {
                group.addTask { try await transport.ping() }
            }
            for try await info in group {
                #expect(info.protocolVersion == 17)
    ...
    private func exerciseAttach(
        ...
        #expect(!output.contains("herden-attach"))
    ...
    private func connectionCount(from transport: HerdenSSHTransport) async throws -> Int {
...
struct HerdenSSHTransportBehaviorEnvironment: Sendable {
    let host: String
    let port: UInt16
    let username: String
    ...
    static let current: HerdenSSHTransportBehaviorEnvironment? = {
        guard
            let directEncoded = ProcessInfo.processInfo.environment["HERDEN_SSH_E2E_CONFIG"],
            let directData = Data(base64Encoded: directEncoded),
            let jumpEncoded = ProcessInfo.processInfo.environment["HERDEN_SSH_JUMP_E2E_CONFIG"],
        ...
        return HerdenSSHTransportBehaviorEnvironment(
            host: direct.host,
            port: direct.port,
    ...
    func directSettings(
        socket: HerdenSocketLocation? = nil
    ...
    /// whole SSH byte stream — RPC, SFTP, PTY and the forwarded herden socket
    ...
    private func settings(
        ...
        settings.agentDiscoveryCommand =
            "printf '__HERDEN_AGENT_KIND__=codex\\n__HERDEN_AGENT_KIND__=gemini\\n'"
        guard let quotedHome = RemoteShellPath.quotedAbsolute(homePath) else {
            return settings
        }
        settings.homeCommand =
            "/bin/sh -c 'printf \"__HERDEN_HOME__=%s\\n\" \"$1\"' home \(quotedHome)"
        settings.attachCommand = "\(homePath)/.herden-ci/fake-attach"
        settings.terminalAttachCommand = "\(homePath)/.herden-ci/fake-attach"
        settings.stageDirectoryCommand =
            "/bin/sh -c 'umask 077; "
            + "directory=$(mktemp -d \"$1/herden.XXXXXXXX\") || exit 1; "
            + "printf \"__HERDEN_STAGE_DIR__=%s\\n\" \"$directory\"' stage \(quotedHome)/.herden-ci"
        return settings
    }
    ...
    func failOnceHomeCommand(_ path: String) -> String {
        ...
        return "/bin/sh -c 'count=$(wc -c < \"$1\" 2>/dev/null || printf 0); "
            ...
            + "printf \"__HERDEN_HOME__=relative\\n\"; else "
