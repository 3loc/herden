...
struct ConsoleSpacePresentationTests {
    @Test func spacesSortByLabelAndCountTheirAgentsPerHost() {
        ...
        let agents = [
            ConsoleAgent(
                hostID: alpha.id,
                hostName: alpha.displayName,
                agent: Agent(.fixture(paneID: "p1", workspaceID: "w2")),
                workspaceLabel: "Zulu",
                repositoryCheckout: nil),
            ConsoleAgent(
                hostID: alpha.id,
                hostName: alpha.displayName,
                agent: Agent(.fixture(paneID: "p2", workspaceID: "w2")),
                workspaceLabel: "Zulu",
                repositoryCheckout: nil),
            ConsoleAgent(
                hostID: beta.id,
                hostName: beta.displayName,
                agent: Agent(.fixture(paneID: "p3", workspaceID: "w2")),
                workspaceLabel: "Zulu",
                repositoryCheckout: nil),
        ]
        ...
        let spaces = ConsoleSpace.project(
            ...
            workspacesByHost: [
                alpha.id: [
                    ConsoleWorkspace(id: "w2", label: "Zulu"),
                    ConsoleWorkspace(id: "w1", label: "Alpha"),
                ],
                beta.id: [ConsoleWorkspace(id: "w2", label: "Zulu")],
            ],
            agents: agents)
    ...
    @Test func hostFilterAppliesToSpacesAsWellAsAgents() {
        ...
        let spaces = ConsoleSpace.project(
            hosts: [alpha, beta],
            workspacesByHost: [
                alpha.id: [ConsoleWorkspace(id: "w1", label: "Alpha")],
                beta.id: [ConsoleWorkspace(id: "w2", label: "Beta")],
            ],
            agents: [],
            filteredHostID: beta.id)

        #expect(spaces.map(\.hostID) == [beta.id])
        #expect(spaces.map(\.workspace.label) == ["Beta"])
    }

    @Test func aKnownSpaceKeepsTheConsoleVisibleWithoutAgents() {
        #expect(
