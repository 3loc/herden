...
// The default wake command is the real herden invocation from spec #16: the
// remote bridge's entry point ensures the server is running before bridging.
@Suite("Wake command")
struct WakeCommandTests {
    @Test func defaultsToHerdenRemoteClientBridge() {
        ...
        #expect(settings.wakeCommand == "herden remote-client-bridge")
    }

    @Test func scopesNamedSessionWakeCommandToSessionState() throws {
        let command = try HerdenSSHTransport.wakeExecCommand(
            wakeCommand: "herden remote-client-bridge",
            socketPath: "/home/u/.config/herden/sessions/testloop/herden.sock",
            socketLocation: .namedSession("testloop"))

        #expect(
            command == "LC_ALL=C /bin/sh -c '\(HerdenHostPath.pathExport); "
                + "export HERDR_SOCKET_PATH=\"$1\"; "
                + "export HERDR_SESSION=\"$2\"; herden remote-client-bridge < /dev/null' wake "
                + "'/home/u/.config/herden/sessions/testloop/herden.sock' testloop")
    }

    @Test func defaultSessionWakeCommandDoesNotOverrideSessionState() throws {
        let command = try HerdenSSHTransport.wakeExecCommand(
            wakeCommand: "/opt/herden-wake --foreground",
            socketPath: "/home/u/.config/herden/herden.sock",
            socketLocation: .defaultSession)

        #expect(
            command == "LC_ALL=C /bin/sh -c '\(HerdenHostPath.pathExport); "
                + "export HERDR_SOCKET_PATH=\"$1\"; "
                + "/opt/herden-wake --foreground < /dev/null' wake "
    ...
    @Test func absolutePathWakeCommandDoesNotOverrideSessionState() throws {
        let command = try HerdenSSHTransport.wakeExecCommand(
            wakeCommand: "/opt/herden-wake --foreground",
            socketPath: "/home/u/My Config/herden.sock",
            socketLocation: .absolutePath("/home/u/My Config/herden.sock"))
    ...
    @Test func refusesInvalidNamedSession() {
        #expect(throws: TransportError.self) {
            _ = try HerdenSSHTransport.wakeExecCommand(
                wakeCommand: "herden remote-client-bridge",
                socketPath: "/home/u/.config/herden/sessions/work session/herden.sock",
