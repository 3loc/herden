...
A plugin is a directory with a `herden-plugin.toml` manifest and commands Herdr
...
installing or linking one, skim the `herden-plugin.toml` manifest and the scripts
...
Start with a directory that contains `herden-plugin.toml` and one executable
script or program:
...
```bash
herdr plugin install ogulcancelik/herden-plugin-examples/agent-telegram-notify
...
The example cookbook repo is `ogulcancelik/herden-plugin-examples`. It contains
...
but changing `herden-plugin.toml` after the install preview aborts install. Build
...
or more `herden-plugin.toml` files whose required metadata can be parsed. Plugins
