...
`herden-plugin.toml` whose required metadata can be parsed. Search by repository
...
`herden-plugin.toml` manifest at its root or in a subdirectory. See
...
`herden-plugin.toml` manifests with parseable required metadata on its default
