...
插件不是 SDK 集成。它是一个带 `herden-plugin.toml` 清单和 Herdr 可启动
...
从你信任的作者和仓库安装插件,并先大致看看新插件做什么: `herden-plugin.toml`
...
从一个包含 `herden-plugin.toml` 和一个可执行脚本或程序的目录开始:
...
示例菜谱仓库是 `ogulcancelik/herden-plugin-examples`。它在子目录中包含
...
安装预览之后修改 `herden-plugin.toml` 会导致安装中止。构建失败时会显示
...
包含一个或多个必需元数据可解析的 `herden-plugin.toml` 的公开 GitHub 仓库自动索引。
