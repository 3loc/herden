...
プラグインは SDK インテグレーションではありません。`herden-plugin.toml`
...
何をするのか先に目を通してください: `herden-plugin.toml` マニフェストと、実行される
...
`herden-plugin.toml` と実行可能なスクリプトまたはプログラムをひとつ含む
ディレクトリから始めます:
...
サンプル集のリポジトリは `ogulcancelik/herden-plugin-examples` です。
...
構いませんが、インストールプレビュー後に `herden-plugin.toml` を変更すると
...
プラグインは普通の GitHub リポジトリのままです: `herden-plugin.toml` を含めて公開し、
