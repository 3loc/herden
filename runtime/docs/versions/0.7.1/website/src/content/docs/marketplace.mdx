...
Publish a normal public GitHub repository with a `herden-plugin.toml` manifest at
...
The index does not parse `herden-plugin.toml` yet, so manifest fields like the
