...
`herden-plugin.toml` manifest and commands Herdr can launch. Herdr validates the
...
does first: the `herden-plugin.toml` manifest and the scripts or binaries it runs.
...
Plugins stay ordinary GitHub repositories: publish one with `herden-plugin.toml`,
