...
插件 API 是面向可执行工作流工具的早期宿主面。插件是带 `herden-plugin.toml` 清单的包。清单声明可分享的动作、事件钩子、终端窗格入口点和链接处理器。动作和窗格仅限清单声明;运行时动作注册和运行时 argv 窗格创建不在 v1 范围内。
...
路径可以是包含 `herden-plugin.toml` 的插件目录,或直接指向清单的路径。清单结构如下:
