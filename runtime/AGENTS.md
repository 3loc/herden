...
| `src/platform/` | Platform-specific shell encoding; never inject client palette/OSC colours into Agent launch commands. Codex-only `FORCE_COLOR=1` selects ANSI colour depth without pinning a client's palette. |
...
The SDK and Zig libc configuration live at `~/.local/share/herden/windows-cross/`,
