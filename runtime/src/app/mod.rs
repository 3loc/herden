...
pub struct App {
    ...
    pub(crate) codex_force_color: bool,
...
impl App {
    pub fn new(
        ...
        let mut app = Self {
            ...
            codex_force_color: config.advanced.codex_force_color,
    ...
    fn apply_live_config(
        ...
        {
            diagnostics.push(
                "terminal.kitty_graphics changes require restarting Herden; kept current setting"
                    .into(),
            );
        }
        ...
        if !invalid_section("advanced") {
            ...
            self.codex_force_color = config.advanced.codex_force_color;
        }
...
mod tests {
    ...
    fn reload_config_invalid_sidebar_bounds_keeps_previous_ui_and_returns_partial() {
        ...
        assert_eq!(
            ...
            Some("config.toml; herden config check")
        );
    ...
    fn reload_config_applies_known_sibling_and_summarizes_unknown_key() {
        ...
        assert_eq!(
            app.state.config_diagnostic.as_deref(),
            Some("config.toml has unknown keys; herden config check")
        );
    ...
    fn reload_config_keeps_current_state_on_invalid_toml() {
        ...
        assert!(app
            ...
            .as_deref()
            .is_some_and(|message| {
                message == "config.toml invalid; keeping current config; herden config check"
            }));
        assert!(app.state.toast.is_none());
