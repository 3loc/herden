...
impl App {
    ...
    fn resolve_worktree_source(
        ...
        if let Some(cwd) = cwd {
            ...
            let space = crate::workspace::git_space_metadata(&path).ok_or_else(|| {
                ApiFailure::new(
                    "not_git_worktree",
                    "Herden worktree actions require a path inside a Git work tree",
                )
            })?;
    ...
    fn worktree_source_from_workspace(&self, ws_idx: usize) -> Result<WorktreeSource, ApiFailure> {
        ...
        let Some(space) = git_space else {
            return Err(ApiFailure::new(
                "not_git_worktree",
                "Herden worktree actions require a workspace inside a Git work tree",
            ));
        };
        if space.is_linked_worktree {
            return Err(ApiFailure::new(
...
mod tests {
    ...
    fn install_event_plugin(app: &mut App, name: &str, event: &str) -> PathBuf {
        ...
        let manifest_path = plugin_root.join("herden-plugin.toml");
