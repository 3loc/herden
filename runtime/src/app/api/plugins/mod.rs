...
mod tests {
    ...
    fn write_manifest(root: &std::path::Path) -> std::path::PathBuf {
        ...
        let manifest = root.join("herden-plugin.toml");
        std::fs::write(
            &manifest,
            r#"
...
"#,
    ...
    fn plugin_link_lists_and_unlinks_manifest() {
        ...
        assert_eq!(
            ...
            canonical_path_string(&root.join("herden-plugin.toml"))
        );
        assert_eq!(
            plugin.plugin_root,
        ...
        let response = app.handle_api_request(Request {
            ...
            method: Method::PluginLink(PluginLinkParams {
                ...
                source: Some(PluginSourceInfo {
                    ...
                    repo: Some("herden-plugin-examples".into()),
...
"#,
    ...
    fn plugin_command_output_reader_caps_and_marks_truncation() {
        ...
        assert_eq!(
            output,
            "abc\n[herden truncated plugin output after 3 bytes]"
        );
    }
...
"#,
            ...
            assert_eq!(
                ...
                ("0", "0"),
                "plugin action and pane must launch Herden after its executable is replaced"
            );
            return;
        }
