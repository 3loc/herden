...
impl App {
    ...
    fn respawn_shell_for_launch_pane(
        ...
        let runtime = match crate::terminal::TerminalRuntime::spawn(
            ...
            terminal
                .query_context
                .and_then(|context| context.appearance),
...
mod tests {
    ...
    #[test]
    fn managed_agent_exit_keeps_its_pane_and_workspace_for_the_shell() {
        let event_hub = crate::api::EventHub::default();
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &crate::config::Config::default(),
        ...
        // The process-exit observation can precede the shell becoming the
        // foreground process. Keep the pane through both observations.
        app.handle_internal_event(AppEvent::StateChanged {
            pane_id,
            agent: None,
            ...
            visible_working: false,
            process_exited: false,
            observed_at: now + std::time::Duration::from_secs(2),
        });
        ...
        assert!(!events.iter().any(|(_, event)| {
            event.event == crate::api::schema::EventKind::PaneClosed
                || event.event == crate::api::schema::EventKind::WorkspaceClosed
        }));
    }
