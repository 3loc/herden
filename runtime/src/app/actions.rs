...
mod tests {
    ...
    fn update_ready_uses_event_install_command_in_toast() {
        ...
        state.handle_app_event(AppEvent::UpdateReady {
            version: "0.5.0".into(),
            install_command: "brew update && brew upgrade herden".into(),
        });

        assert_eq!(
            state.update_install_command,
            "brew update && brew upgrade herden"
        );
        let toast = state.toast.as_ref().expect("update toast");
        assert_eq!(
            toast.context,
            "detach, run `brew update && brew upgrade herden`, then run Herden again to reconnect"
        );
    }
