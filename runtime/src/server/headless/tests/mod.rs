...
#[cfg(unix)]
#[path = "terminal_resume.rs"]
mod terminal_resume_tests;
    ...
    for i in 0..=crate::app::APP_EVENT_DRAIN_LIMIT {
        server
            ...
            .try_send(AppEvent::UpdateReady {
                ...
                install_command: "herden install".into(),
            })
            .unwrap();
    }
...
fn api_window_title_wins_until_it_is_cleared() {
    ...
    server.handle_client_window_title_api("set".into(), Some("herden api".into()));
    assert_eq!(
        next_window_title(&control_rx),
        Some(Some("herden api".to_string()))
    );
...
fn oversized_paste_rejection_notifies_only_the_sending_client() {
    ...
    match read_server_message(
        ...
        ServerMessage::ClientShellError { message } => assert_eq!(
            message,
            "Paste rejected: Input message is 5000012 bytes; Herden's limit is 1048576 bytes"
        ),
    ...
    match read_server_message(
        ...
        ServerMessage::ClientShellError { message } => assert_eq!(
            message,
            "Paste rejected: Input message is 7000000 bytes; Herden's limit is 1048576 bytes"
        ),
        other => panic!("expected client shell paste error, got {other:?}"),
    }
    assert!(
...
fn update_notification_reaches_client_shell_independent_of_delivery() {
    ...
    let changed = server.handle_internal_event_with_forwarding(AppEvent::UpdateReady {
        ...
        install_command: "herden update".into(),
    });
...
fn update_notification_is_semantic_for_system_delivery() {
    ...
    match read_server_message(
        ...
        ServerMessage::SemanticNotification(notification) => {
            ...
            assert_eq!(notification.title, "Herden v9.9.9 available");
            assert_eq!(
                notification.body.as_deref(),
                Some("detach, run `herden update`, then run Herden again to reconnect")
            );
        }
