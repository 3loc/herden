...
#[test]
fn foreground_client_without_theme_keeps_last_reported_host_theme() {
    with_terminal_session_test_server(|server, _, _, _| {
        let _reported = connect_test_shell(server, 21, 80, 24);
        report_background(server, 21, 10);
        assert_eq!(
        ...
        let _unreported = connect_test_shell(server, 22, 80, 24);
...
#[test]
fn unreported_desktop_controller_keeps_terminal_query_context() {
    with_terminal_session_test_server(|server, terminal_id, _, _| {
        server.app.state.active = Some(0);
        server.app.state.mode = crate::app::Mode::Terminal;
        let tab_id = server.app.public_tab_id(0, 0).expect("tab");
        let _reported = connect_test_shell(server, 21, 80, 24);
        ...
        server.tab_geometry_controllers.insert(tab_id, 22);
        assert_eq!(
            background(server.app.terminal_runtimes.get(&terminal_id).unwrap()).r,
            10
        );
        assert!(!server.apply_shell_client_terminal_appearance(22));
...
#[test]
fn direct_appearance_batch_cannot_recolour_after_child_observes_defaults() {
    with_terminal_session_test_server(|server, terminal_id, terminal_name, _| {
        attach(server, 7, &terminal_name, false);
        assert!(report_light(server, 7));
...
#[test]
fn direct_detach_cannot_recolour_after_child_observes_direct_defaults() {
