//! Auto-detect launch behavior for the `herden` command.
//!
//! When the user runs `herden` with no subcommand:
...
/// directory where the user ran `herden`.
...
/// Checks whether a herden server is currently listening on the client socket.
///
...
/// Checks whether a herden server is listening at a specific socket path.
...
fn validate_running_server_compatibility(saved_federation: bool) -> io::Result<()> {
    let Some(status) = read_server_status()? else {
        return Err(io::Error::other(format!(
            "a herden server is listening, but its status API is unavailable.\n\n{}\nIf that fails, stop the old server process manually.",
    ...
    Err(io::Error::other(format!(
        "This session needs one final server update before Herden can attach ({requirement}).\n\nserver: v{} endpoint generation {}\nclient: v{} endpoint generation {}\n\n{}",
...
/// Spawns the herden server as a background daemon process.
///
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    let exe = std::env::current_exe().map_err(|err| {
        io::Error::new(
            err.kind(),
            format!("failed to determine herden executable path: {err}"),
        )
    })?;
    ...
    let pid =
        crate::platform::launch_server_daemon_command(&mut command).map_err(|err: io::Error| {
            io::Error::new(err.kind(), format!("failed to spawn herden server: {err}"))
        })?;
...
pub fn wait_for_server_socket(socket_path: &Path, timeout: Duration) -> io::Result<()> {
    ...
    Err(io::Error::new(
        io::ErrorKind::TimedOut,
        format!(
            "server did not become ready within {}s (socket: {}). The background server may still be starting; try `herden` again, or check {}",
            timeout.as_secs(),
            socket_path.display(),
            crate::session::data_dir().join("herden-server.log").display()
        ),
    ))
}
...
///
/// This is the entry point called from `main.rs` when the user runs `herden`
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/herden-test"));
...
"#,
    ...
    fn validate_running_server_compatibility_names_session_commands_for_protocol_mismatch() {
        ...
        assert!(
            message.contains("Run `herden session stop work`"),
            "unexpected error: {message}"
        );
        assert!(
            message.contains("then run `herden session attach work` again"),
            "unexpected error: {message}"
        );
