//! Headless server mode — runs the herden event loop without a real terminal.
//!
//! The server:
//! - Does not enter raw mode or read stdin
//! - Creates and listens on both `herden.sock` (existing JSON API) and
//!   `herden-client.sock` (new binary protocol)
...
/// The headless server — runs the herden event loop without a real terminal.
    ...
    fn handle_server_event(&mut self, ev: ServerEvent) -> bool {
        ...
        match ev {
            ...
            ServerEvent::ClientPasteRejected {
                ...
                let detail =
                    format!("Input message is {size} bytes; Herden's limit is {max} bytes");
