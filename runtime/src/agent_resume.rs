...
pub fn is_reserved_native_state_source(source: &str, agent: &str) -> bool {
    matches!(
        (agent_source_kind(source), agent),
        (Some("claude"), "claude")
            | (Some("codex"), "codex")
            | (Some("copilot"), "copilot")
            | (Some("devin"), "devin")
            | (Some("droid"), "droid")
            | (Some("qodercli"), "qodercli")
            | (Some("qwen"), "qwen")
            | (Some("cursor"), "cursor")
            | (Some("grok"), "grok")
    )
...
pub fn plan(source: &str, agent: &str, session_ref: &AgentSessionRef) -> Option<AgentResumePlan> {
    ...
    let argv = match (agent_source_kind(source), agent, session_ref.kind) {
        (Some("claude"), "claude", AgentSessionRefKind::Id) => {
            vec![
                "claude".into(),
                "--resume".into(),
        ...
        (Some("pi"), "pi", AgentSessionRefKind::Path | AgentSessionRefKind::Id) => {
            vec!["pi".into(), "--session".into(), session_ref.value.clone()]
        }
        (Some("omp"), "omp", AgentSessionRefKind::Path | AgentSessionRefKind::Id) => {
        ...
        (Some("hermes"), "hermes", AgentSessionRefKind::Id) => {
            vec![
                "hermes".into(),
                "--resume".into(),
                session_ref.value.clone(),
        ...
        (Some("opencode"), "opencode", AgentSessionRefKind::Id) => {
            vec![
                "opencode".into(),
                "--session".into(),
                session_ref.value.clone(),
        ...
        (Some("qodercli"), "qodercli", AgentSessionRefKind::Id) => {
            vec![
                "qodercli".into(),
                "--resume".into(),
                session_ref.value.clone(),
        ...
        (Some("qwen"), "qwen", AgentSessionRefKind::Id) => {
            vec!["qwen".into(), "--resume".into(), session_ref.value.clone()]
        }
        (Some("kilo"), "kilo", AgentSessionRefKind::Id) => {
            vec!["kilo".into(), "--session".into(), session_ref.value.clone()]
        }
        (Some("cursor"), "cursor", AgentSessionRefKind::Id) => {
            vec![
                if cfg!(windows) {
                    "cursor-agent.cmd"
                } else {
        ...
        (Some("antigravity_cli"), "agy", AgentSessionRefKind::Id) => {
            vec![
                "agy".into(),
                "--conversation".into(),
                session_ref.value.clone(),
        ...
        (Some("grok"), "grok", AgentSessionRefKind::Id) => {
            vec!["grok".into(), "--resume".into(), session_ref.value.clone()]
        }
        (Some("letta"), "letta", AgentSessionRefKind::Id) => {
...
pub(crate) fn is_official_agent_source(source: &str, agent: &str) -> bool {
    matches!(
        ...
        (Some("claude"), "claude")
            ...
            | (Some("kilo"), "kilo")
            | (Some("cursor"), "cursor")
            | (Some("antigravity_cli"), "agy")
            | (Some("grok"), "grok")
            | (Some("letta"), "letta")
    )
}

fn agent_source_kind(source: &str) -> Option<&str> {
    source
        .strip_prefix("herden:")
