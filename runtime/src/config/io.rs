...
pub fn config_diagnostic_summary(diagnostics: &[String]) -> Option<String> {
    ...
    Some(format!("{target}{impact}; herden config check"))
}
...
mod tests {
    ...
    fn config_diagnostic_summary_reports_default_fallback() {
        ...
        assert_eq!(
            config_diagnostic_summary(&diagnostics).as_deref(),
            Some("config.toml invalid; using defaults; herden config check")
        );
    }
    ...
    fn config_diagnostic_summary_reports_unreadable_config_impact() {
        ...
        assert_eq!(
            ...
            Some("config.toml unreadable; using defaults; herden config check")
        );
        ...
        assert_eq!(
            ...
            Some("config.toml unreadable; keeping current config; herden config check")
        );
    }
    ...
    fn config_diagnostic_summary_reports_retained_live_config() {
        ...
        assert_eq!(
            config_diagnostic_summary(&diagnostics).as_deref(),
            Some("config.toml invalid; keeping current config; herden config check")
        );
    }
