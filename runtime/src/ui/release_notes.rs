...
mod tests {
    ...
    fn release_notes_inline_code_spans_are_styled_without_backticks() {
        ...
        let lines = release_notes_lines("- `herden pane run ...` now works", &palette);

        assert_eq!(lines.len(), 1);
        assert_eq!(line_text(&lines[0].1), " • herden pane run ... now works");
        assert_eq!(lines[0].1.spans[1].content.as_ref(), "herden pane run ...");
    ...
    fn release_notes_preview_lines_show_update_steps() {
        let palette = Palette::catppuccin();
        let lines = release_notes_preview_line_entries("herden update", &palette)
            .into_iter()
            .map(|(_, line)| line)
            .collect::<Vec<_>>();
        ...
        assert_eq!(
            line_text(&lines[1]),
            " detach, run herden update, then run Herden again to reconnect"
        );
        assert_eq!(lines[0].spans[1].style.fg, Some(palette.accent));
        assert_eq!(lines[0].spans[2].style.fg, Some(palette.text));
        assert_eq!(lines[1].spans[2].content.as_ref(), "herden update");
    ...
    fn release_notes_preview_display_is_part_of_the_scrollable_notes_body() {
        ...
        let lines = release_notes_display_lines(&notes, "herden update", &palette);
