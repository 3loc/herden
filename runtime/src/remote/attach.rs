...
pub(crate) fn prepare_saved_ssh(target: &str, session_name: &str) -> io::Result<()> {
    ...
    let output = ssh.sh_output(&format!(
        "{} </dev/null",
        remote_bridge_command(&prepared.remote_herden, session_name)
    ))?;
...
fn remote_binary_on_path_any(
    ...
    let output = ssh.user_shell_output("command -v herden")?;
    if output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
...
fn remote_binary_exists(ssh: &RemoteSsh, remote_herden: &RemoteHerden) -> io::Result<bool> {
    let command = format!("test -x {}", remote_herden.shell_path);
...
fn remote_server_status(
    ...
    let command = remote_session_command(remote_herden, &ssh.session_name, "status server --json");
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
...
fn remote_live_handoff_command(
    ...
    remote_session_command(
        ...
        &format!(
            ...
            remote_herden.shell_path, protocol, version
        ),
    )
}
...
fn live_handoff_remote_server(ssh: &RemoteSsh, remote_herden: &RemoteHerden) -> io::Result<()> {
    ...
    let command = remote_live_handoff_command(remote_herden, &ssh.session_name, protocol, &version);
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
...
fn stop_remote_server(ssh: &RemoteSsh, remote_herden: &RemoteHerden) -> io::Result<()> {
    let command = remote_session_command(remote_herden, &ssh.session_name, "server stop");
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
...
fn confirm_remote_install(
    ...
    eprint!(
        "Install {} to {}? [Y/n] ",
        source_description, remote_herden.shell_path
    );
    io::stderr().flush()?;
...
fn remote_session_command(remote_herden: &RemoteHerden, session_name: &str, args: &str) -> String {
    let mut command = remote_herden.shell_path.clone();
...
fn remote_bridge_command(remote_herden: &RemoteHerden, session_name: &str) -> String {
    format!(
        "exec {}",
        remote_session_command(remote_herden, session_name, "remote-client-bridge")
    )
}
...
fn bridge_connection(
    ...
    command
        .arg("-T")
        .arg(target)
        .arg(remote_bridge_command(remote_herden, session_name))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
...
mod tests {
    ...
    fn saved_machine_server_commands_are_scoped_to_the_explicit_session() {
        ...
        for command in [
            ...
            assert_eq!(
                remote_session_command(&herden, "agents", command),
                format!("{} --session agents {command}", herden.shell_path)
            );
            assert_eq!(
                remote_session_command(&herden, crate::session::DEFAULT_SESSION_NAME, command),
                format!("{} {command}", herden.shell_path)
            );
        }
        assert!(
            remote_live_handoff_command(&herden, "agents", 19, "0.7.9").starts_with(&format!(
    ...
    fn remote_bridge_command_uses_installed_binary() {
        ...
        assert_eq!(
            remote_bridge_command(&remote_herden, crate::session::DEFAULT_SESSION_NAME),
    ...
    fn remote_path_discovery_reads_multiple_absolute_paths() {
        ...
        assert_eq!(candidates[0].shell_path, "/usr/bin/herden");
        assert_eq!(candidates[1].shell_path, "'/opt/herden bin/herden'");
    }
