...
pub(super) fn prepare_remote_herden(
    ...
    if remote_herden.platform.is_windows() {
        return prepare_windows_remote_herden(ssh, remote_herden, require_surface_interest);
    }
...
pub(super) fn find_installed_remote_herden(ssh: &RemoteSsh) -> io::Result<RemoteHerden> {
    ...
    let remote_herden = RemoteHerden::for_platform(platform);
    if remote_herden.platform.is_windows() {
        return prepare_windows_remote_herden(ssh, remote_herden, true)
            .map(|prepared| prepared.remote_herden);
    }
...
fn prepare_windows_remote_herden(
    ...
    if !remote_binary_exists(ssh, &remote_herden)? {
        return Err(io::Error::new(
            io::ErrorKind::NotFound,
            format!(
                "herden.exe is not installed or is not on PATH on Windows host {}; install a compatible Windows package with remote host support and retry",
                ssh.target()
            ),
        ));
    }
    ...
    {
        return Err(io::Error::other(format!(
            "herden.exe on Windows host {} does not support saved SSH endpoint federation; install a compatible Windows package with remote host support and retry",
            ssh.target()
        )));
    }
...
fn remote_binary_supports_endpoint_requirement(
    ...
    Ok(remote_client_status(ssh, remote_herden)?
...
fn bridge_connection(
    ...
    command
        .arg("-T")
        .arg(target)
        .arg(remote_herden.executable.bridge_command(session_name))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
    ...
    fn windows_remote_commands_use_one_encoded_powershell_grammar() {
        ...
        let commands = [
            ...
            (
                ...
                "$process = Start-Process -FilePath herden.exe -ArgumentList '--session agents remote-client-bridge' -NoNewWindow -Wait -PassThru -ErrorAction Stop; exit $process.ExitCode",
            ),
            (
