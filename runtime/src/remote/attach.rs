...
const STABLE_UPDATE_MANIFEST_URL: &str = "https://herden.app/latest.json";
const PREVIEW_UPDATE_MANIFEST_URL: &str = "https://herden.app/preview.json";
const REMOTE_BINARY_ENV_VAR: &str = "HERDR_REMOTE_BINARY";
// Keep the upstream marker stable so mixed-version remote Hosts interoperate.
...
pub(crate) fn run_remote(remote: RemoteLaunch) -> io::Result<()> {
    ...
    let program = std::env::args()
        .next()
        .unwrap_or_else(|| "herden".to_string());
    ...
    let prepared_remote =
        prepare_remote_herden(&remote_ssh, remote.live_handoff, require_surface_interest)?;
    ensure_remote_server_ready(
        &remote_ssh,
        &prepared_remote.remote_herden,
...
pub(crate) fn prepare_saved_ssh(target: &str, session_name: &str) -> io::Result<()> {
    ...
    let prepared = prepare_remote_herden(&ssh, false, true)?;
    ensure_remote_server_ready(
        &ssh,
        &prepared.remote_herden,
    ...
    let output = ssh.shell_output(&prepared.remote_herden.platform, &command)?;
    if !output.status.success() {
        return Err(command_failed("remote server startup failed", &output));
    }
    match remote_server_status(&ssh, &prepared.remote_herden, true)? {
...
impl RemoteHerden {
    fn for_platform(platform: RemotePlatform) -> Self {
        let (install_suffix, executable) = if platform.is_windows() {
            (
                String::new(),
                RemoteExecutable::WindowsPath("herden.exe".to_string()),
            )
        } else {
            let install_suffix = ".local/bin/herden".to_string();
...
pub(super) struct PreparedRemoteHerden {
    pub(super) remote_herden: RemoteHerden,
...
impl RemoteSsh {
    ...
    fn install_herden(&self, remote_herden: &RemoteHerden, source_path: &Path) -> io::Result<()> {
        let output = self.sh_output(&remote_install_prepare_script(remote_herden))?;
        if !output.status.success() {
    ...
    fn install_windows_herden(
        &self,
        remote_herden: &RemoteHerden,
        source_path: &Path,
        identity: &str,
        sha256: &str,
    ) -> io::Result<RemoteHerden> {
        ...
        let result = (|| {
            ...
            let install_result = (|| {
                ...
                self.copy_windows_file(
                    source_path,
                    &format!(r"{remote_dir}\herden-windows-x86_64.zip"),
                )?;
                ...
                .map(|path| remote_herden.clone().with_windows_path(path))
            })();
...
fn remote_install_prepare_script(remote_herden: &RemoteHerden) -> String {
    format!(
        r#"set -eu
...
"#,
        install_suffix = remote_herden.install_suffix
    )
}
...
fn windows_remote_install_command(remote_dir: &str, identity: &str, sha256: &str) -> String {
    ...
    let package =
        crate::platform::quote_powershell_arg(&format!(r"{remote_dir}\herden-windows-x86_64.zip"));
    windows_powershell_script_command(&format!(
        r#"$herdenInstaller = {installer}; $herdenPackage = {package}; & powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $herdenInstaller -Channel {channel} -LocalPackagePath $herdenPackage -LocalPackageFormat zip -LocalPackageIdentity {identity} -LocalPackageSha256 {sha256}; if ($LASTEXITCODE -ne 0) {{ exit $LASTEXITCODE }}; $herdenHome = if ([string]::IsNullOrWhiteSpace($env:HERDR_HOME)) {{ Join-Path $env:USERPROFILE '.herden' }} else {{ $env:HERDR_HOME }}; $activeJunction = Join-Path $herdenHome 'packages\standalone\current'; $activeTarget = [string](Get-Item -LiteralPath $activeJunction -Force -ErrorAction Stop).Target; if ([string]::IsNullOrWhiteSpace($activeTarget)) {{ throw 'Herden installer did not activate a concrete release.' }}; $installedHerden = Join-Path $activeTarget 'herden.exe'; if (-not (Test-Path -LiteralPath $installedHerden -PathType Leaf)) {{ throw 'Herden installer result does not contain herden.exe.' }}; $encodedResult = [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes([System.IO.Path]::GetFullPath($installedHerden))); [Console]::Out.WriteLine('{WINDOWS_REMOTE_INSTALL_RESULT_MARKER}' + $encodedResult); exit 0"#,
...
pub(super) fn prepare_remote_herden(
    ssh: &RemoteSsh,
    live_handoff_enabled: bool,
    require_surface_interest: bool,
) -> io::Result<PreparedRemoteHerden> {
    let platform = detect_remote_platform(ssh)?;
    let remote_herden = RemoteHerden::for_platform(platform);
    if remote_herden.platform.is_windows() {
        return prepare_windows_remote_herden(
            ssh,
            remote_herden,
    ...
    let remote_binary_candidates = remote_binary_candidates(ssh, &remote_herden)?;
    ...
    if override_binary.is_none() {
        for candidate in &remote_binary_candidates {
            if remote_binary_supports_endpoint_requirement(ssh, candidate, require_surface_interest)
                .unwrap_or(false)
            {
                return Ok(PreparedRemoteHerden {
                    remote_herden: candidate.clone(),
    ...
    if let Some(status_probe_herden) = remote_binary_candidates.first().or_else(|| {
        remote_binary_exists(ssh, &remote_herden)
            .ok()
            .and_then(|exists| exists.then_some(&remote_herden))
    }) {
    ...
    if !stop_after_install_approved {
        confirm_remote_install(
            &ssh.destination(),
            &remote_herden,
            &install_source_description(&remote_herden.platform, override_binary.as_deref()),
        )?;
    }
    let source = resolve_install_source(&remote_herden.platform, override_binary)?;
    let install_result = ssh.install_herden(&remote_herden, &source.path);
    source.cleanup();
    install_result?;

    if !remote_binary_supports_endpoint_requirement(ssh, &remote_herden, require_surface_interest)?
    {
        return Err(io::Error::other(format!(
            "installed remote herden at {}, but it does not support saved SSH endpoint federation",
            remote_herden.executable.display()
        )));
    }
    warn_if_remote_bin_not_on_path(ssh)?;

    Ok(PreparedRemoteHerden {
        remote_herden,
        stop_after_install_approved,
    })
}

pub(super) fn find_installed_remote_herden(ssh: &RemoteSsh) -> io::Result<RemoteHerden> {
    let platform = detect_remote_platform(ssh)?;
    let remote_herden = RemoteHerden::for_platform(platform);
    let candidates = remote_binary_candidates(ssh, &remote_herden)?;
    ...
    Err(io::Error::new(
        ...
        format!(
            "matching Herden is not ready on {}; run `herden --remote {}` interactively to install or update it",
            ssh.target(),
            ssh.target()
        ),
    ))
...
fn prepare_windows_remote_herden(
    ssh: &RemoteSsh,
    remote_herden: RemoteHerden,
    live_handoff_enabled: bool,
    require_surface_interest: bool,
    ...
    if !stop_after_install_approved {
        confirm_remote_install(
            &ssh.destination(),
            &remote_herden,
            &install_source_description(&remote_herden.platform, override_package.as_deref()),
        )?;
    }
    ...
    let source = match override_package {
        ...
        None => download_release_asset(&remote_herden.platform)?,
    };
    let install_result = (|| {
        let sha256 = crate::checksum::file_sha256(&source.path)?;
        ssh.install_windows_herden(
            &remote_herden,
            &source.path,
    ...
    source.cleanup();
    let remote_herden = install_result?;
    if !remote_binary_supports_endpoint_requirement(ssh, &remote_herden, require_surface_interest)?
    {
        return Err(io::Error::other(format!(
            "installed remote herden at {}, but it does not support the required remote hosting capabilities",
            remote_herden.executable.display()
        )));
    }
    Ok(PreparedRemoteHerden {
...
pub(super) fn find_installed_remote_api_herden(
    ssh: &RemoteSsh,
    session: &str,
) -> io::Result<RemoteHerden> {
    let platform = detect_remote_platform(ssh)?;
    ...
    Err(io::Error::new(
        io::ErrorKind::Unsupported,
        "remote Herden does not support machine API forwarding; update Herden on this machine",
    ))
}
...
fn remote_binary_candidates(
    ...
    if let Some(path_candidate) = remote_binary_on_path_any(ssh, remote_herden)? {
    ...
    let output = ssh.sh_output(&known_remote_binary_candidate_script(
        &remote_herden.platform,
    ))?;
    if !output.status.success() {
        return Err(command_failed("remote binary discovery failed", &output));
    }
    let stdout = String::from_utf8_lossy(&output.stdout);
    for candidate in remote_herdens_from_path_discovery(remote_herden, &stdout) {
...
fn windows_remote_binary_candidate_command() -> String {
    windows_powershell_script_command(&format!(
        r#"function Emit-HerdenPath([string]$CandidatePath) {{ if ([string]::IsNullOrWhiteSpace($CandidatePath) -or -not (Test-Path -LiteralPath $CandidatePath -PathType Leaf)) {{ return }}; $candidateFullPath = [System.IO.Path]::GetFullPath($CandidatePath); $encodedCandidate = [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($candidateFullPath)); [Console]::Out.WriteLine('{WINDOWS_REMOTE_PATH_MARKER}' + $encodedCandidate) }}; $pathCommand = Get-Command herden.exe -CommandType Application -ErrorAction SilentlyContinue | Select-Object -First 1; if ($null -ne $pathCommand) {{ Emit-HerdenPath $pathCommand.Source }}; $herdenHome = if ([string]::IsNullOrWhiteSpace($env:HERDR_HOME)) {{ Join-Path $env:USERPROFILE '.herden' }} else {{ $env:HERDR_HOME }}; $activeJunction = Get-Item -LiteralPath (Join-Path $herdenHome 'packages\standalone\current') -Force -ErrorAction SilentlyContinue; if ($null -ne $activeJunction -and -not [string]::IsNullOrWhiteSpace([string]$activeJunction.Target)) {{ Emit-HerdenPath (Join-Path ([string]$activeJunction.Target) 'herden.exe') }}; exit 0"#
    ))
}

fn windows_remote_binary_candidates(
    remote_herden: &RemoteHerden,
    stdout: &str,
) -> Vec<RemoteHerden> {
    let mut candidates = Vec::new();
    for path in stdout.lines().filter_map(|line| {
        ...
        push_if_new_remote_binary_candidate(
            &mut candidates,
            remote_herden.clone().with_windows_path(path),
        );
    }
    candidates
}
...
fn push_if_new_remote_binary_candidate(
    candidates: &mut Vec<RemoteHerden>,
    candidate: RemoteHerden,
) {
    if !candidates
        .iter()
...
if [ -n "$home" ]; then
    emit "$home/.local/bin/herden"
fi
"#,
    );
    if platform.os == "macos" {
        script.push_str(
            r#"    emit "/opt/homebrew/bin/herden"
    emit "/usr/local/bin/herden"
"#,
        );
    } else if platform.os == "linux" {
        script.push_str(
            r#"    emit "/home/linuxbrew/.linuxbrew/bin/herden"
"#,
        );
    }
    script.push_str(
        r#"if [ -n "$home" ]; then
    emit "$home/.local/share/mise/installs/herden/$version/bin/herden"
    emit "$home/.local/share/mise/installs/herden/$version/herden"
    emit "$home/.local/share/mise/installs/github-ogulcancelik-herden/$version/herden"
    emit "$home/.nix-profile/bin/herden"
fi
if [ -n "$user" ]; then
    emit "/etc/profiles/per-user/$user/bin/herden"
fi
emit "/nix/var/nix/profiles/default/bin/herden"
emit "/run/current-system/sw/bin/herden"
"#,
    );
...
fn remote_binary_on_path_any(
    ssh: &RemoteSsh,
    remote_herden: &RemoteHerden,
) -> io::Result<Option<RemoteHerden>> {
    let output = ssh.posix_user_shell_output("command -v herden")?;
    if output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        if let Some(candidate) = remote_herden_from_path_discovery(remote_herden, &stdout) {
    ...
    let output = ssh.sh_output("command -v herden\n")?;
    if !output.status.success() {
        return Ok(None);
    }

    let stdout = String::from_utf8_lossy(&output.stdout);
    Ok(remote_herden_from_path_discovery(remote_herden, &stdout))
}

fn remote_herdens_from_path_discovery(
    remote_herden: &RemoteHerden,
    stdout: &str,
) -> Vec<RemoteHerden> {
    stdout
        .lines()
        .filter_map(|path| remote_herden_from_path(remote_herden, path))
        .collect()
}

fn remote_herden_from_path_discovery(
    remote_herden: &RemoteHerden,
    stdout: &str,
) -> Option<RemoteHerden> {
    stdout
        .lines()
        .find_map(|path| remote_herden_from_path(remote_herden, path))
}

fn remote_herden_from_path(remote_herden: &RemoteHerden, path: &str) -> Option<RemoteHerden> {
    let path = path.trim();
    if !path.starts_with('/') {
        return None;
    }
    ...
    Some(remote_herden.clone().with_shell_path(shell_quote(path)))
}

fn is_mise_shim_path(path: &str) -> bool {
    path.ends_with("/mise/shims/herden")
}
...
fn remote_client_status(
    ...
    let command = remote_herden.executable.status_client_command();
    let output = ssh.shell_output(&remote_herden.platform, &command)?;
    if !output.status.success() {
...
fn remote_binary_supports_endpoint_requirement(
    ...
    Ok(
        remote_client_status(ssh, remote_herden)?.is_some_and(|status| {
            status.supports_endpoint_requirement(&remote_herden.platform, require_surface_interest)
        }),
    )
}

fn remote_binary_exists(ssh: &RemoteSsh, remote_herden: &RemoteHerden) -> io::Result<bool> {
    let command = remote_herden.executable.exists_command();
    Ok(ssh
        .shell_output(&remote_herden.platform, &command)?
        .status
        .success())
}
...
fn install_source_description_for(
    ...
    if local_binary_can_seed_remote {
        "the current local herden binary".to_string()
    } else {
        format!(
            "the {} {} asset for {}",
            current_version(),
...
fn ensure_remote_server_ready(
    ...
    let status = remote_server_status(ssh, remote_herden, require_surface_interest)?;
    ...
    if live_handoff_enabled && live_handoff {
        match live_handoff_remote_server(ssh, remote_herden) {
            Ok(()) => return Ok(()),
            Err(err) => {
    ...
    if stop_after_install_approved {
        stop_remote_server(ssh, remote_herden)?;
        return Ok(());
    }
...
fn confirm_remote_install_with_running_server(
    ...
    let status = match remote_server_status(ssh, remote_herden, require_surface_interest) {
        Ok(status) => status,
        Err(err) => {
            if !io::stdin().is_terminal() {
                return Err(io::Error::other(format!(
                    "could not inspect the running remote herden server on {target} before installing: {err}; run from an interactive terminal to approve updating the remote binary"
                )));
            }
            eprintln!(
                "could not inspect the running remote herden server on {target} before installing: {err}"
            );
            eprint!("continue installing the remote herden binary? [y/N] ");
            io::stderr().flush()?;
            ...
            if answer != "y" && answer != "yes" {
                return Err(io::Error::new(
                    ...
                    "remote herden install cancelled",
                ));
            }
            return Ok(false);
        }
    ...
    if plan == RemoteInstallRunningServerPlan::KeepRunning {
        if io::stdin().is_terminal() {
            eprintln!("remote herden server on {target} is already compatible:");
            eprintln!("  server: v{}", version_label(version.as_deref()));
            eprintln!(
                "Herden will install {} without stopping the running remote server.",
                current_version()
            );
        }
        return Ok(false);
    ...
    if !io::stdin().is_terminal() {
        match plan {
            ...
            RemoteInstallRunningServerPlan::StopRequired(_) => {
                return Err(io::Error::other(format!(
                    "remote herden server on {target} is running v{}; run from an interactive terminal to approve stopping it for the update",
    ...
    if plan == RemoteInstallRunningServerPlan::LiveHandoff {
        eprintln!("remote herden server on {target} is currently running:");
        eprintln!("  server: v{}", version_label(version.as_deref()));
        eprintln!(
            "Herden will install {} and hand off live pane processes to the prepared server.",
            current_version()
        );
        return Ok(false);
    }
    ...
    eprintln!("  server: v{}", version_label(version.as_deref()));
    eprintln!(
        "To complete the remote update, Herden must stop the running remote server after installing."
    );
...
fn remote_server_status(
        ...
        )
    {
        ...
        let negotiation = probe_remote_endpoint(ssh, remote_herden)?;
...
fn confirm_remote_server_stop(
    ...
    if !io::stdin().is_terminal() {
        if required_upgrade {
            return Err(io::Error::other(format!(
                "remote herden server on {target} needs one final update before this client can attach; run from an interactive terminal to approve updating it"
            )));
        }

        eprintln!(
            "remote herden server on {target} is still running v{}; it will use {} after it restarts.",
    ...
    match reason {
        RemoteServerRestartReason::EndpointProtocol => {
            eprintln!(
                "the remote server predates Herden's stable endpoint protocol and must update before this client can attach."
            );
        }
        RemoteServerRestartReason::SurfaceInterest => {
            eprintln!(
        ...
        RemoteServerRestartReason::DaemonDetach => {
            eprintln!(
                "the remote server was started by a herden build that may not survive SSH connection loss. restart it so network drops disconnect only this client."
            );
        }
    }
    ...
    if required_upgrade {
        return Err(io::Error::new(
            io::ErrorKind::Interrupted,
            "remote herden server stop cancelled",
        ));
    }
...
fn live_handoff_remote_server(ssh: &RemoteSsh, remote_herden: &RemoteHerden) -> io::Result<()> {
    let status = remote_client_status(ssh, remote_herden)?.ok_or_else(|| {
        io::Error::other("could not inspect the prepared remote herden binary before live handoff")
    })?;
    let protocol = status.protocol.ok_or_else(|| {
        io::Error::other("prepared remote herden did not report its private protocol")
    })?;
    let version = status
        .version
        .filter(|version| !version.is_empty())
        .ok_or_else(|| io::Error::other("prepared remote herden did not report its version"))?;
    let command =
        remote_herden
            .executable
    ...
    eprintln!(
        "handed off the remote herden server on {}; reconnecting to the prepared server.",
        ssh.target()
    );
    Ok(())
}

fn stop_remote_server(ssh: &RemoteSsh, remote_herden: &RemoteHerden) -> io::Result<()> {
    let command = remote_herden
        .executable
    ...
    wait_for_remote_server_shutdown(ssh, remote_herden)?;
    eprintln!(
        "stopped the remote herden server on {}; it will restart when the remote client bridge attaches.",
        ssh.target()
    );
    Ok(())
}
...
fn wait_for_remote_server_shutdown(
    ...
    loop {
        if remote_server_status(ssh, remote_herden, false)? == RemoteServerStatus::NotRunning {
            return Ok(());
        }
        if Instant::now() >= deadline {
            return Err(io::Error::new(
                ...
                format!(
                    "shutdown was requested, but the old remote herden server on {target} is still responding after {} seconds",
...
fn warn_if_remote_bin_not_on_path(ssh: &RemoteSsh) -> io::Result<()> {
    ...
    eprintln!(
        "herden: installed remote binary to ~/.local/bin/herden, but the remote shell does not resolve `herden` to that path"
    );
    Ok(())
}
...
fn remote_shell_resolves_managed_install(stdout: &str) -> bool {
    stdout
        ...
        .next()
        .map(str::trim)
        .is_some_and(|path| path.ends_with("/.local/bin/herden"))
}
...
fn preview_assets_for_build<'a>(
    ...
    let build = manifest.builds.get(build_id).ok_or_else(|| {
        io::Error::other(format!(
            "preview manifest no longer includes build {build_id}; run `herden update` locally or set {REMOTE_BINARY_ENV_VAR}=target/release/herden"
        ))
    })?;
...
fn remote_release_asset(asset_key: &str) -> io::Result<RemoteReleaseAsset> {
    if crate::build_info::is_preview() {
        let build_id = crate::build_info::build_id().ok_or_else(|| {
            io::Error::other("preview client has no build id; set HERDR_REMOTE_BINARY or install Herden on the remote manually")
        })?;
        ...
        if protocol != CURRENT_PROTOCOL {
            return Err(io::Error::other(format!(
                "preview manifest has build {build_id} protocol {protocol}, but this client needs protocol {CURRENT_PROTOCOL}; set {REMOTE_BINARY_ENV_VAR}=target/release/herden or install a matching Herden on the remote host manually"
            )));
        }
    ...
    let release = manifest.release_for_version(&current_version).ok_or_else(|| {
        io::Error::other(format!(
            "release manifest does not include herden {current_version}; build herden for {} or install it there manually",
            asset_key
        ))
    })?;
    if let Some(protocol) = release.protocol {
        if protocol != CURRENT_PROTOCOL {
            return Err(io::Error::other(format!(
                "release manifest has herden {current_version} protocol {protocol}, but this client needs protocol {CURRENT_PROTOCOL}; set {REMOTE_BINARY_ENV_VAR}=target/release/herden or install a matching herden on the remote host manually"
            )));
        }
    }
    let asset = release.assets.get(asset_key).ok_or_else(|| {
        io::Error::other(format!(
            "no {asset_key} binary in the release manifest for herden {current_version}"
        ))
    })?;
...
fn private_download_dir(asset_key: &str) -> io::Result<PathBuf> {
    ...
    Err(io::Error::new(
        ...
        "failed to create private herden remote download directory",
    ))
}
...
fn confirm_remote_install(
    ...
    if !io::stdin().is_terminal() {
        return Err(io::Error::other(format!(
            "matching remote herden {} is not installed at {}; run from an interactive terminal to approve installation",
            current_version(),
            remote_herden.executable.display()
        )));
    }
    ...
    eprintln!(
        "matching herden {} is not installed on {target} for {}.",
        current_version(),
        remote_herden.platform.asset_key()
    );
    eprint!(
        "Install {} to {}? [Y/n] ",
        source_description,
    ...
    if !read_remote_confirmation(&mut io::stdin().lock(), true)? {
        return Err(io::Error::new(
            io::ErrorKind::Interrupted,
            "remote herden installation cancelled",
        ));
    }
...
pub(super) fn remote_api_bridge_command(
    ...
    match &remote_herden.executable {
        RemoteExecutable::PosixShellPath(_) => posix_remote_output_command(&format!(
            "exec {}",
            remote_herden.executable.command(&args)
        )),
...
impl SshStdioBridge {
    pub(super) fn start(
        ...
        Self::start_command(
            target,
            if noninteractive && remote_herden.bridge_idle_timeout {
                remote_herden
                    .executable
                    .bridge_command_with_idle_timeout(&session_name, true)
            } else {
                remote_herden.executable.bridge_command(&session_name)
            },
            local_socket,
            ssh_options,
            noninteractive,
                ...
                match listener.accept() {
                    Ok(stream) => {
                        ...
                        if let Err(err) = bridge_connection(
                            ...
                            if noninteractive {
                                ...
                                eprintln!("herden: remote bridge failed: {err}");
                            }
                        }
                    }
                    ...
                    Err(err) => {
                        if noninteractive {
                            ...
                            eprintln!("herden: remote bridge listener failed: {err}");
                        }
                        break;
                    }
                }
...
/// ignored when Herden runs under Git Bash and host aliases stop resolving.
...
fn local_forward_socket_path(target: &str, session_name: &str) -> PathBuf {
    ...
    let readable_name = format!("herden-remote-{pid}-{target_clean}-{session_clean}.sock");
    let target_prefix: String = target_clean.chars().take(8).collect();
    let hash = short_socket_hash(target, session_name);
    let short_name = format!("herden-r-{pid}-{target_prefix}-{hash}.sock");
...
mod tests {
    ...
    fn bridge_socket_is_user_only() {
        ...
        let socket = std::env::temp_dir().join(format!(
            "herden-bridge-permissions-test-{}.sock",
            std::process::id()
        ));
        let remote_herden = RemoteHerden::for_platform(RemotePlatform {
            os: "linux",
            arch: "x86_64",
        });
        let bridge = SshStdioBridge::start(
    ...
    fn accepted_bridge_stream_is_reset_to_blocking() {
        ...
        let socket = std::env::temp_dir().join(format!(
            "herden-bridge-blocking-test-{}.sock",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&socket);
    ...
    fn bridge_stream_delivers_large_frame_before_delayed_reply() {
        ...
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let socket = std::env::temp_dir().join(format!(
            "herden-bridge-large-frame-{}-{nonce}.sock",
            std::process::id()
        ));
        let listener = crate::ipc::bind_private_local_listener(&socket).expect("bind listener");
    ...
    fn managed_ssh_config_includes_user_config_then_fallback() {
        ...
        // herden's fallback transport settings are present...
        assert!(
        ...
        if let Some(home) = std::env::var_os("HOME") {
            ...
            if user_config.is_file() {
                ...
                assert!(
                    ...
                    "user config must be Included before herden's fallback: {contents}"
                );
            }
        }
    ...
    fn saved_machine_server_commands_are_scoped_to_the_explicit_session() {
        let herden =
            RemoteHerden::for_platform(RemotePlatform::from_uname("Linux", "x86_64").unwrap());
        for (args, command) in [
            ...
            assert_eq!(
                herden.executable.session_command("agents", args),
                format!("{} --session agents {command}", herden.executable.display())
            );
            assert_eq!(
                herden
                    .executable
                    .session_command(crate::session::DEFAULT_SESSION_NAME, args),
                format!("{} {command}", herden.executable.display())
            );
        }
        assert!(herden
            .executable
    ...
    fn remote_install_stream_command_avoids_shell_c_wrapper() {
        let command = remote_install_stream_command("/home/a b/.local/bin/herden.tmp.123");

        assert_eq!(command, "tee '/home/a b/.local/bin/herden.tmp.123'");
    }
    ...
    fn remote_install_prepare_and_commit_scripts_quote_paths() {
        ...
        let prepare = remote_install_prepare_script(&remote_herden);

        assert!(prepare.contains("mkdir -p \"$dir\""));
        assert!(prepare.contains("printf '%s\\0%s\\0' \"$tmp\" \"$dest\""));
        assert_eq!(
            parse_remote_install_paths(b"/home/a b/herden.tmp.42\0/home/a b/herden\0").unwrap(),
            (
                "/home/a b/herden.tmp.42".to_string(),
                "/home/a b/herden".to_string()
            )
        );
        assert_eq!(
            parse_remote_install_paths(b"/home/a b\n/herden.tmp.42\0/home/a b\n/herden\0").unwrap(),
            (
                "/home/a b\n/herden.tmp.42".to_string(),
                "/home/a b\n/herden".to_string()
            )
        );
        assert_eq!(
            remote_install_commit_script("/home/a b/herden.tmp.42", "/home/a b/herden"),
            "set -eu\nchmod 755 '/home/a b/herden.tmp.42'\nmv '/home/a b/herden.tmp.42' '/home/a b/herden'\n"
        );
    }
    ...
    fn extract_remote_args_removes_space_form() {
        ...
        assert_eq!(cleaned, vec!["herden", "--help"]);
    ...
    fn extract_remote_args_removes_equals_form() {
        let args = vec!["herden".into(), "--remote=user@host".into()];
        let (cleaned, remote) = extract_remote_args(&args).unwrap();
        assert_eq!(cleaned, vec!["herden"]);
        let remote = remote.unwrap();
    ...
    fn extract_remote_args_accepts_explicit_handoff() {
        let args = vec!["herden".into(), "--remote=dev".into(), "--handoff".into()];
    ...
    fn extract_remote_args_preserves_handoff_without_remote() {
        let args = vec!["herden".into(), "update".into(), "--handoff".into()];
    ...
    fn extract_remote_args_rejects_remote_keybindings_without_remote() {
        let args = vec!["herden".into(), "--remote-keybindings=server".into()];
    ...
    fn extract_remote_args_requires_value() {
        let args = vec!["herden".into(), "--remote".into()];
        let err = extract_remote_args(&args).unwrap_err();
    ...
    fn windows_remote_commands_use_one_encoded_powershell_grammar() {
        let executable = RemoteExecutable::WindowsPath("herden.exe".to_string());
        let commands = [
            (
                "platform probe",
            ...
            (
                ...
                "if ($null -ne (Get-Command herden.exe -CommandType Application -ErrorAction SilentlyContinue)) { exit 0 }; exit 1",
            ),
            (
                "client status",
                executable.status_client_command(),
                "& herden.exe status client '--json'; exit $LASTEXITCODE",
            ),
            (
                "named server status",
                executable.session_command("agents", &["status", "server", "--json"]),
                "& herden.exe '--session' agents status server '--json'; exit $LASTEXITCODE",
            ),
            (
                "server stop",
                executable.session_command("agents", &["server", "stop"]),
                "& herden.exe '--session' agents server stop; exit $LASTEXITCODE",
            ),
            (
                "direct bridge",
                executable.bridge_command("agents"),
                "$process = Start-Process -FilePath herden.exe -ArgumentList '--session agents remote-client-bridge' -NoNewWindow -PassThru -ErrorAction Stop; $null = $process.Handle; $process.WaitForExit(); exit $process.ExitCode",
            ),
            (
                "API bridge with explicit default session",
                remote_api_bridge_command(&RemoteHerden::for_platform(RemotePlatform { os: "windows", arch: "x86_64" }), "default", false),
                "$process = Start-Process -FilePath herden.exe -ArgumentList '--session default remote-api-bridge' -NoNewWindow -PassThru -ErrorAction Stop; $null = $process.Handle; $process.WaitForExit(); exit $process.ExitCode",
            ),
            (
                "API bridge capability probe",
                remote_api_bridge_command(&RemoteHerden::for_platform(RemotePlatform { os: "windows", arch: "x86_64" }), "agents", true),
                "$process = Start-Process -FilePath herden.exe -ArgumentList '--session agents remote-api-bridge --check' -NoNewWindow -PassThru -ErrorAction Stop; $null = $process.Handle; $process.WaitForExit(); exit $process.ExitCode",
            ),
            (
    ...
    fn windows_install_commands_copy_zip_and_return_concrete_path() {
        ...
        let remote_dir = r"C:\Temp\Herden O'Brien\测试";
        assert_eq!(
            windows_scp_target(
                "user@example",
                &format!(r"{remote_dir}\herden-windows-x86_64.zip")
            ),
            "user@example:C:/Temp/Herden O'Brien/测试/herden-windows-x86_64.zip"
        );
        assert_eq!(
        ...
        let installed = r"C:\Users\test\测试\.herden\packages\standalone\releases\0.9.0\herden.exe";
    ...
    fn windows_candidate_discovery_prefers_path_then_active_release() {
        ...
        let path = r"C:\stale\herden.exe";
        let active = r"C:\Users\test\.herden\packages\standalone\releases\current\herden.exe";
        let stdout = format!(
        ...
        let candidates = windows_remote_binary_candidates(&remote_herden, &stdout);
        ...
        assert!(command.contains("Get-Command herden.exe"));
    ...
    fn reattach_command_includes_remote_and_session() {
        assert_eq!(
            reattach_command(
                "target/release/herden",
                "user@host",
                "work",
                RemoteKeybindings::Local,
                false,
            ),
            "target/release/herden --remote user@host --session work"
        );
        assert_eq!(
            reattach_command(
                "herden",
        ...
        assert_eq!(
            ...
            "herden --remote host --remote-keybindings server"
        );
        assert_eq!(
            reattach_command(
                "herden",
    ...
    fn windows_reattach_command_uses_current_executable() {
        ...
        assert_eq!(
            reattach_command(
                r"C:\Program Files\Herden\herden.exe",
                "host'name",
                "work'name",
                RemoteKeybindings::Local,
                false,
    ...
    fn remote_api_bridge_always_selects_the_saved_session() {
        ...
        for session in ["default", "agents"] {
            assert_eq!(
                remote_api_bridge_command(&remote_herden, session, false),
                posix_remote_output_command(&format!(
                    "exec \"$HOME/.local/bin/herden\" --session {session} remote-api-bridge"
                ))
            );
        }
    }
    ...
    fn remote_bridge_idle_timeout_requires_explicit_support_and_opt_in() {
        ...
        let remote = RemoteHerden::for_platform(RemotePlatform {
            os: "linux",
            arch: "x86_64",
        });
        assert!(remote
    ...
    fn remote_bridge_command_uses_installed_binary() {
        ...
        assert_eq!(
            ...
            "printf '\n%s\n' 'herdr-remote-output-ready:1'\nexec \"$HOME/.local/bin/herden\" remote-client-bridge"
        );
        assert_eq!(
            remote_herden.executable.saved_bridge_command("agents"),
            "exec \"$HOME/.local/bin/herden\" --session agents remote-client-bridge </dev/null"
        );
    }
    ...
    fn remote_path_discovery_uses_path_binary() {
        ...
        let remote_herden = remote_herden_from_path_discovery(&remote_herden, "/usr/bin/herden\n")
            .expect("path binary");
        ...
        assert_eq!(
            remote_herden
                .executable
                .bridge_command(crate::session::DEFAULT_SESSION_NAME),
            "printf '\n%s\n' 'herdr-remote-output-ready:1'\nexec /usr/bin/herden remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_quotes_discovered_binary() {
        ...
        let remote_herden =
            remote_herden_from_path_discovery(&remote_herden, "/opt/herden bin/herden\n")
                .expect("path binary");
        ...
        assert_eq!(
            remote_herden
                .executable
                .bridge_command(crate::session::DEFAULT_SESSION_NAME),
            "printf '\n%s\n' 'herdr-remote-output-ready:1'\nexec '/opt/herden bin/herden' remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_uses_macos_path_binary() {
        ...
        let remote_herden =
            remote_herden_from_path_discovery(&remote_herden, "/opt/homebrew/bin/herden\n")
                .expect("path binary");
        ...
        assert_eq!(
            remote_herden
                .executable
                .bridge_command(crate::session::DEFAULT_SESSION_NAME),
            "printf '\n%s\n' 'herdr-remote-output-ready:1'\nexec /opt/homebrew/bin/herden remote-client-bridge"
        );
        assert_eq!(remote_herden.platform.asset_key(), "macos-aarch64");
    }
    ...
    fn remote_path_discovery_reads_multiple_absolute_paths() {
        ...
        let candidates = remote_herdens_from_path_discovery(
            &remote_herden,
            "/usr/bin/herden\nbin/herden\n /opt/herden bin/herden\n",
        );

        assert_eq!(candidates.len(), 2);
        assert_eq!(
            candidates[0].executable,
            RemoteExecutable::PosixShellPath("/usr/bin/herden".to_string())
        );
        assert_eq!(
            candidates[1].executable,
            RemoteExecutable::PosixShellPath("'/opt/herden bin/herden'".to_string())
        );
    }
    ...
    fn remote_path_discovery_ignores_mise_shims() {
        ...
        let candidates = remote_herdens_from_path_discovery(
            &remote_herden,
            "/home/can/.local/share/mise/shims/herden\n/home/can/.local/share/mise/installs/herden/0.7.1/bin/herden\n",
        );
        ...
        assert_eq!(
            ...
            RemoteExecutable::PosixShellPath(
                "/home/can/.local/share/mise/installs/herden/0.7.1/bin/herden".to_string()
            )
        );
    }
    ...
    fn known_remote_binary_candidate_script_includes_mise_and_nix_paths() {
        ...
        assert!(script.contains("emit \"$home/.local/bin/herden\""));
        assert!(!script.contains("mise/shims/herden"));
        assert!(script.contains(&format!("version={}", shell_quote(&current_version()))));
        assert!(
            script.contains("emit \"$home/.local/share/mise/installs/herden/$version/bin/herden\"")
        );
        assert!(script.contains("emit \"$home/.local/share/mise/installs/herden/$version/herden\""));
        assert!(script.contains(
            "emit \"$home/.local/share/mise/installs/github-ogulcancelik-herden/$version/herden\""
        ));
        assert!(script.contains("emit \"$home/.nix-profile/bin/herden\""));
        assert!(script.contains("emit \"/etc/profiles/per-user/$user/bin/herden\""));
        assert!(script.contains("emit \"/run/current-system/sw/bin/herden\""));
        assert!(script.contains("emit \"/home/linuxbrew/.linuxbrew/bin/herden\""));
        assert!(!script.contains("emit \"/opt/homebrew/bin/herden\""));
    }
    ...
    fn known_remote_binary_candidate_script_includes_macos_homebrew_paths() {
        ...
        assert!(script.contains("emit \"/opt/homebrew/bin/herden\""));
        assert!(script.contains("emit \"/usr/local/bin/herden\""));
        assert!(!script.contains("emit \"/home/linuxbrew/.linuxbrew/bin/herden\""));
    }
    ...
    fn remote_path_discovery_quotes_single_quotes_in_discovered_binary() {
        ...
        let remote_herden =
            remote_herden_from_path_discovery(&remote_herden, "/opt/herden's/bin/herden\n")
                .expect("path binary");
        ...
        assert_eq!(
            remote_herden
                .executable
                .bridge_command(crate::session::DEFAULT_SESSION_NAME),
            "printf '\n%s\n' 'herdr-remote-output-ready:1'\nexec '/opt/herden'\\''s/bin/herden' remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_ignores_relative_paths() {
        ...
        let remote_herden = remote_herden_from_path_discovery(&remote_herden, "bin/herden\n");

        assert!(remote_herden.is_none());
    }
    ...
    fn remote_path_discovery_ignores_empty_output() {
        ...
        let remote_herden = remote_herden_from_path_discovery(&remote_herden, "\n");
    ...
    fn remote_shell_path_warning_accepts_managed_install() {
        assert!(remote_shell_resolves_managed_install(
            "/home/can/.local/bin/herden\n"
        ));
        assert!(remote_shell_resolves_managed_install(
            "/Users/can/.local/bin/herden\n"
        ));
        assert!(!remote_shell_resolves_managed_install(
            "/usr/local/bin/herden\n"
        ));
        assert!(!remote_shell_resolves_managed_install(""));
    }
    ...
    fn remote_live_handoff_uses_prepared_binary_identity() {
        ...
        let command = remote_herden.executable.live_handoff_command(
            crate::session::DEFAULT_SESSION_NAME,
            19,
            "0.7.9",
        );
    ...
    fn install_source_description_uses_override_binary() {
        ...
        assert_eq!(
            install_source_description_for(
                &platform,
                Some(Path::new("/tmp/herden-aarch64")),
                false
            ),
            "HERDR_REMOTE_BINARY (/tmp/herden-aarch64)"
        );
    }
    ...
    fn install_source_description_uses_local_binary_when_allowed() {
        ...
        assert_eq!(
            ...
            "the current local herden binary"
        );
    }
    ...
    fn resolve_install_source_uses_override_binary_without_temporary_cleanup() {
        ...
        let source = resolve_install_source(&platform, Some(PathBuf::from("/tmp/herden-aarch64")))
            .expect("override source");
        assert_eq!(source.path, PathBuf::from("/tmp/herden-aarch64"));
    ...
    fn windows_local_forward_endpoint_uses_private_state_dir() {
        ...
        assert!(path
            .file_name()
            .is_some_and(|name| name.to_string_lossy().starts_with("herden-r-")));
    }
    ...
    fn local_forward_socket_path_uses_readable_name_when_it_fits() {
        ...
        let filename = path
            ...
            .to_string();
        assert!(
            filename.starts_with("herden-remote-"),
    ...
    fn install_source_cleanup_removes_temporary_directory() {
        let dir = std::env::temp_dir().join(format!(
            "herden-install-source-cleanup-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
