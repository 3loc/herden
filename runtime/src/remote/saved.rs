...
use super::attach::{find_installed_remote_herden, RemoteSsh, SshStdioBridge};
...
pub(crate) fn connect_saved_ssh(
    ...
    let remote_herden = find_installed_remote_herden(&ssh)?;
...
impl SavedSshApiBridge {
    pub(crate) fn start(profile_id: &str, target: &str, session: &str) -> io::Result<Self> {
        let ssh = validated_saved_ssh(profile_id, target, session)?;
        let remote_herden = super::attach::find_installed_remote_api_herden(&ssh, session)?;
        let command = super::attach::remote_api_bridge_command(&remote_herden, session, false);
        let path = crate::platform::remote_bridge_endpoint_path(
            &format!("herden-api-ssh-{}-{profile_id}.sock", std::process::id()),
            &format!(
                "herden-api-{}-{}.sock",
                std::process::id(),
                &profile_id[..16]
...
fn saved_bridge_path(profile_id: &str) -> PathBuf {
    ...
    let readable = format!("herden-ssh-{pid}-{profile_id}.sock");
    let short = format!("herden-s-{pid}-{}.sock", &profile_id[..16]);
...
mod tests {
    ...
    fn bootstrap_command_preserves_the_explicit_remote_session() {
        assert_eq!(
            ...
            "herden --remote 'build host' --session 'agent work'"
        );
    }
    ...
    fn prompt_and_compatibility_failures_require_attention() {
        for message in [
            ...
            "Host key verification failed",
            "matching Herden is not ready; install or update",
            "handshake rejected",
        ] {
