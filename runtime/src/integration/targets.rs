...
pub(crate) fn install_antigravity_cli() -> io::Result<AntigravityCliInstallPaths> {
    ...
    // The Herden block is Herden-owned, so rewrite it wholesale and leave every
...
/// Builds the Herden-owned `hooks.json` block for Antigravity CLI.
///
/// Every event Herden registers takes a flat handler list; the `matcher`/`hooks`
/// group is only valid for the tool events, which Herden does not use.
...
/// The complete Herden-owned Grok hook config. Installation and status share
...
pub(crate) fn install_grok() -> io::Result<GrokInstallPaths> {
    ...
    // Grok merges every `~/.grok/hooks/*.json`, so herden owns a dedicated
...
pub(crate) fn uninstall_grok() -> io::Result<GrokUninstallResult> {
    ...
    // herden owns both files outright, so removal is a straight delete.
