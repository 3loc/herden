...
mod tests {
    ...
    fn add_and_remove_tui_plugin_preserves_jsonc_config() {
        ...
        add_tui_plugin(&dir, "./herden-tui-state.js").unwrap();
        add_tui_plugin(&dir, "./herden-tui-state.js").unwrap();
        ...
        assert!(remove_tui_plugin(&dir, "./herden-tui-state.js").unwrap());
    ...
    fn managed_jsonc_leaves_opencode_migration_target_absent() {
        ...
        let config_path = add_tui_plugin(&dir, "./herden-tui-state.js").unwrap();
        ...
        assert_eq!(
            ...
            json!({ "plugin": ["./herden-tui-state.js"] })
        );
    ...
    fn configured_tui_plugin_accepts_option_tuple() {
        ...
        fs::write(
            ...
            r#"{"plugin":[["./herden-tui-state.js",{"enabled":true}]]}"#,
        )
        .unwrap();

        assert!(tui_plugin_is_configured(&dir, "./herden-tui-state.js"));
        assert!(remove_tui_plugin(&dir, "./herden-tui-state.js").unwrap());
    ...
    fn cli_registration_preserves_options_and_other_preferences() {
        ...
        fs::write(&path, r#"{"theme":{"name":"catppuccin"},"plugins":[{"package":"./herden-opencode","options":{"custom":true}},"example"]}"#).unwrap();
        add_cli_plugin(&dir, &state, "./herden-opencode").unwrap();
        assert!(cli_plugin_is_configured(&dir, "./herden-opencode"));
        assert_eq!(parse_config(&path)["plugins"].as_array().unwrap().len(), 2);
        assert_eq!(parse_config(&path)["plugins"][0]["options"]["custom"], true);
        assert!(remove_cli_plugin(&dir, "./herden-opencode").unwrap());
    ...
    fn cli_registration_defers_while_migration_pending() {
        ...
        assert!(add_cli_plugin(&dir, &state, "./herden-opencode")
            .unwrap()
            .is_none());
