...
const integrations = [
  { name: "Pi", modulePath: "./pi/herden-agent-state.ts" },
  { name: "Oh My Pi", modulePath: "./omp/herden-agent-state.ts" },
] as const;

const socketPlugins = [
  {
    name: "OpenCode",
    modulePath: "./opencode/herden-agent-state.js",
    sessionID: "opencode-session",
  },
  { name: "Kilo", modulePath: "./kilo/herden-agent-state.js", sessionID: "kilo-session" },
] as const;
...
for (const socketPlugin of socketPlugins) {
  test(`${socketPlugin.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    ...
    const { HerdenAgentStatePlugin } = await importFresh(socketPlugin.modulePath);
    const plugin = await HerdenAgentStatePlugin();
    await plugin.event({
      event: {
...
test("OpenCode stays disabled without the Herden socket environment", async () => {
  process.env.HERDR_ENV = "1";
  process.env.HERDR_PANE_ID = "test:p1";
  delete process.env.HERDR_SOCKET_PATH;

  const { HerdenAgentStatePlugin } = await importFresh("./opencode/herden-agent-state.js");

  expect(await HerdenAgentStatePlugin()).toEqual({});
});
...
test("OMP accepts POSIX and Windows session paths", async () => {
  const { isAbsoluteSessionPath } = await importFresh("./omp/herden-agent-state.ts");
...
test("Pi reports a Windows session path", async () => {
  ...
  const { handlers, pi } = createExtensionHarness();
  const { default: install } = await importFresh("./pi/herden-agent-state.ts");
  install(pi);
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
  ...
  handlers.get("agent_start")?.({}, context);
  await waitFor(() => requestStates(requests).length === 2);
  eventHandlers.get("herden:blocked")?.({ active: true, label: "approval" }, context);
  await waitFor(() => requestStates(requests).length === 3);
  ...
  eventHandlers.get("herden:blocked")?.({ active: false }, context);
...
test("Oh My Pi retries working before a queued idle state", async () => {
  ...
  const { default: install } = await importFresh("./omp/herden-agent-state.ts");
  install(pi);
