...
async function loadPlugin() {
  importCounter += 1;
  const { HerdenAgentStatePlugin } = await import(`./herden-agent-state.js?test=${importCounter}`);
  return HerdenAgentStatePlugin();
}
...
test("dual server entrypoint keeps V1 hooks and never reports from the V2 shared server", async () => {
  const module = await import(`./herden-agent-state.js?test=${++importCounter}`);
  expect(module.default.server).toBe(module.HerdenAgentStatePlugin);
