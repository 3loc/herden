# managed by Herden; reinstalling the integration replaces this file.
...
$herden = if ([string]::IsNullOrWhiteSpace($env:HERDR_BIN_PATH)) { "herden" } else { $env:HERDR_BIN_PATH }
$source = if ($payload.is_new_session -eq $true) { "new" } else { "resume" }
$commandArgs = @(
    "pane", "report-agent-session", $env:HERDR_PANE_ID,
    "--source", "herden:letta", "--agent", "letta",
...
try {
    ...
    } -ArgumentList $herden, (,$commandArgs)
