const API_SCHEMA_JSON: &str = include_str!("../../herden-api.schema.json");
...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: herden api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Herden API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `herden api schema --json` to print the full schema.\nUse `herden api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("herden api commands:");
    eprintln!("  herden api snapshot");
    eprintln!("  herden api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: herden api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Herden API schema"));
        assert!(text.contains("Use `herden api schema --json`"));
        assert!(text.len() < 400);
    }
}
