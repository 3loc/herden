...
pub(super) fn restart_guidance() -> String {
    TARGET.with(|target| match target.borrow().as_ref() {
        Some(target) => format!("Update Herden and restart the server on machine '{}' (session {}). Stopping the server exits its pane processes.", target.profile.label, target.profile.session),
...
fn parse_machine_prefix(args: &[String]) -> Result<Option<(String, Vec<String>)>, String> {
    ...
    if index >= args.len() || args[index] == "--" {
        return Err("usage: herden --machine <label-or-id> <command>".into());
    }
...
fn resolve_machine<'a>(
        ...
        .find(|profile| profile.id.as_str() == selector)
    {
        ...
        let profile = matches
            .next()
            .ok_or_else(|| format!("unknown machine '{selector}'; use `herden machine list`"))?;
...
mod tests {
    ...
    fn machine_prefix_routes_without_consuming_command_payload() {
        for prefix in [args(&["--machine", "mac"]), args(&["--machine=mac"])] {
            ...
            assert_eq!(
                ...
                Some((
                    "mac".into(),
                    args(&["herden", "agent", "prompt", "w4:p1", "--machine"])
                ))
            );
        }
        assert_eq!(
    ...
    fn machine_prefix_rejects_missing_target_and_conflicting_global_options() {
        for input in [
            args(&["herden", "--machine"]),
            args(&["herden", "--machine="]),
            args(&["herden", "--machine", "--help"]),
            args(&["herden", "--machine", "mac"]),
            args(&[
                "herden",
                "--machine",
                "mac",
