...
const HELP: &str = "Usage:
  herden machine list [--json]
  herden machine add <ssh-target> --label <label> [--remote-session <name>]
  herden machine rename <profile-id> --label <label>
  herden machine remove <profile-id>
  herden machine enable <profile-id>
  herden machine disable <profile-id>

Add prepares the remote Herden installation and starts its server before saving.
Missing or incompatible installations require approval in an interactive terminal.
Changes apply automatically to open local Herden clients.
Removing or disabling a machine leaves its remote sessions running.
Saved machines contain only a label, SSH target, explicit Herden session, and enabled state.
...
fn list(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        _ => {
            eprintln!("usage: herden machine list [--json]");
            return Ok(2);
        }
    };
...
fn parse_add_args(args: &[String]) -> Result<AddArgs, String> {
    ...
    let target = target.ok_or_else(|| {
        "usage: herden machine add <ssh-target> --label <label> [--remote-session <name>]"
            .to_owned()
    })?;
...
fn add(args: &[String]) -> std::io::Result<i32> {
    ...
    println!("Open Herden clients connect automatically.");
    Ok(0)
}
...
fn rename(args: &[String]) -> std::io::Result<i32> {
    ...
    let [raw_id, flag, label] = args.as_slice() else {
        eprintln!("usage: herden machine rename <profile-id> --label <label>");
        return Ok(2);
    };
    if flag != "--label" {
        eprintln!("usage: herden machine rename <profile-id> --label <label>");
...
fn remove(args: &[String]) -> std::io::Result<i32> {
    let Some(id) = one_profile_id(args, "usage: herden machine remove <profile-id>")? else {
        return Ok(2);
    };
    let mut catalog = load_catalog()?;
...
fn set_enabled(args: &[String], enabled: bool) -> std::io::Result<i32> {
    ...
    let usage = format!("usage: herden machine {action} <profile-id>");
