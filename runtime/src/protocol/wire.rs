//! Wire protocol for herden server/client communication.
//!
...
pub fn check_client_version(client_version: u32) -> VersionCheck {
    ...
    if client_version == PROTOCOL_VERSION {
        ...
        VersionCheck::Incompatible(format!(
            "client version {client_version} is older than server version {PROTOCOL_VERSION}; please upgrade your herden client"
        ))
    } else {
        VersionCheck::Incompatible(format!(
            "client version {client_version} is newer than server version {PROTOCOL_VERSION}; please upgrade the herden server"
        ))
    }
}
...
mod tests {
    ...
    fn client_shell_snapshot_roundtrip() {
        let msg = ServerMessage::ClientShellSnapshot(Box::new(ClientShellSnapshot {
            ...
            update_install_command: "herden update".into(),
    ...
    fn server_window_title_roundtrip() {
        for title in [Some("herden api".to_owned()), None] {
