...
fn normalise_output(text: &str) -> String {
    ...
    // `agent.read` is a recent-output request. If it exceeds the HUD budget,
    // retain the newest complete lines, never the old head of the transcript.
    let clean = if clean.len() > MAX_OUTPUT_CHARS {
        let mut start = clean.len() - MAX_OUTPUT_CHARS;
        while !clean.is_char_boundary(start) {
            start += 1;
        }
        if let Some(next_line) = clean[start..].find('\n') {
            start += next_line + 1;
        }
        &clean[start..]
    } else {
...
mod tests {
    ...
    fn projection_uses_the_workspace_label_not_cwd_or_terminal_title() {
        let labels = HashMap::from([("w3X".to_string(), "example space".to_string())]);
        let agent = project_agent(
            &serde_json::json!({
                "pane_id": "w3X:p1",
                "workspace_id": "w3X",
                "cwd": "/home/dev/projects/herden",
        ...
        .unwrap();
        assert_eq!(agent.space, "example space");
    ...
    #[test]
    fn output_reader_keeps_the_newest_lines_when_the_budget_is_exceeded() {
        let history = (0..1_000)
            .map(|index| format!("{index:04}:{}", "x".repeat(110)))
            .collect::<Vec<_>>()
            .join("\n");
        let output = normalise_output(&format!("{history}\nlatest answer"));
        assert!(output.len() <= MAX_OUTPUT_CHARS);
        assert!(!output.starts_with("0000:"));
        assert!(output.ends_with("latest answer"));
        assert!(output
            .lines()
            .next()
            .unwrap()
