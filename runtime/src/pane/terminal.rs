...
pub(crate) struct GhosttyPaneCore {
    ...
    /// Once the child has observed terminal colours, changing them would turn
    /// one viewer's palette into shared application state. Keep that context
    /// stable for the rest of this PTY's lifetime.
    pub application_terminal_theme_frozen: bool,
...
impl GhosttyPaneTerminal {
    pub fn new(
        ...
        Ok(Self {
            core: Mutex::new(GhosttyPaneCore {
                ...
                application_terminal_theme_frozen: false,
    ...
    pub fn apply_host_terminal_theme(&self, theme: crate::terminal_theme::TerminalTheme) -> bool {
        if let Ok(mut core) = self.core.lock() {
            if core.application_terminal_theme_frozen {
                return false;
            }
    ...
    pub fn freeze_application_terminal_theme(&self) {
        if let Ok(mut core) = self.core.lock() {
            core.application_terminal_theme_frozen = true;
        }
    }
    ...
    pub fn process_pty_bytes(
        ...
        let queried_terminal_colours = default_color_events.iter().any(|event| {
            matches!(
                event.event,
        ...
        let reports_colour_scheme = core
            .terminal
            .mode_get(crate::ghostty::MODE_COLOR_SCHEME_REPORT)
            .unwrap_or(false);
        let answered_colour_scheme_query = terminal_responses.iter().any(|response| {
            response
                .windows(b"\x1b[?997;".len())
                .any(|window| window == b"\x1b[?997;")
        });
        if (queried_terminal_colours || reports_colour_scheme || answered_colour_scheme_query)
            && !core.application_terminal_theme_frozen
        {
            core.application_terminal_theme_frozen = true;
            debug!(
                pane = pane_id.raw(),
                "froze application terminal theme after child colour observation"
            );
        }
...
mod tests {
    ...
    fn process_pty_bytes_reports_latest_libghostty_pwd_callback() {
        ...
        let partial = pane.process_pty_bytes(pane_id, 0, b"\x1b]7;file:///tmp/herden%20", &tx);
        ...
        assert_eq!(
            ...
            Some(std::path::PathBuf::from("/tmp/herden repo"))
        );
        #[cfg(windows)]
        assert_eq!(
            completed.reported_cwd,
            Some(std::path::PathBuf::from("\\tmp\\herden repo"))
        );
    ...
    #[test]
    fn color_scheme_subscription_freezes_application_palette() {
        let (tx, _rx) = mpsc::channel(4);
        let terminal = crate::ghostty::Terminal::new(20, 5, 0).unwrap();
        let pane = GhosttyPaneTerminal::new(terminal, tx.clone()).unwrap();
        let pane_id = PaneId::from_raw(1);
        let original_theme = crate::terminal_theme::TerminalTheme {
            background: Some(crate::terminal_theme::RgbColor {
                r: 20,
                g: 20,
                b: 20,
        ...
        assert!(pane.apply_host_terminal_theme(original_theme));
        ...
        assert!(!changed);
        assert!(pane
            .update_terminal_appearance(appearance, changed)
            .is_none());
        ...
        assert_eq!(
            crate::terminal_theme::parse_default_color_response(reply)
                .unwrap()
                .1,
            original_theme.background.unwrap()
        );
    ...
    #[test]
    fn color_scheme_query_freezes_application_mode() {
        let (tx, mut rx) = mpsc::channel(4);
        let terminal = crate::ghostty::Terminal::new(20, 5, 0).unwrap();
        let pane = GhosttyPaneTerminal::new(terminal, tx.clone()).unwrap();
        let pane_id = PaneId::from_raw(1);
        ...
        let retained_query = pane.process_pty_bytes(pane_id, 0, b"\x1b[?996n", &tx);
        assert_eq!(
            retained_query.terminal_responses,
            vec![Bytes::from_static(b"\x1b[?997;1n")]
        );
        assert!(pane
            .apply_host_terminal_appearance(Some(crate::terminal_theme::HostAppearance::Dark))
    ...
    #[test]
    fn default_colour_query_freezes_application_theme() {
        let (tx, _rx) = mpsc::channel(4);
        let terminal = crate::ghostty::Terminal::new(20, 5, 0).unwrap();
        let pane = GhosttyPaneTerminal::new(terminal, tx.clone()).unwrap();
        let pane_id = PaneId::from_raw(1);
        ...
        assert!(pane.apply_host_terminal_theme(dark));
        let first = pane.process_pty_bytes(pane_id, 0, b"\x1b]11;?\x07", &tx);
        assert_eq!(
            crate::terminal_theme::parse_default_color_response(
                std::str::from_utf8(&first.terminal_responses[0]).unwrap()
            )
            .unwrap()
            .1,
            dark.background.unwrap()
        );

        assert!(!pane.apply_host_terminal_theme(light));
        let second = pane.process_pty_bytes(pane_id, 0, b"\x1b]11;?\x07", &tx);
        assert_eq!(
            crate::terminal_theme::parse_default_color_response(
                std::str::from_utf8(&second.terminal_responses[0]).unwrap()
            )
            .unwrap()
            .1,
            dark.background.unwrap()
