...
const TERMINAL_SESSION_OBSERVE_USAGE: &str =
    "usage: herden terminal session observe <target> [--cols N] [--rows N]";
const TERMINAL_SESSION_CONTROL_USAGE: &str =
    "usage: herden terminal session control <target> [--takeover] [--cols N] [--rows N]";
...
fn channel_set(args: &[String]) -> std::io::Result<i32> {
    let Some(channel) = parse_channel_set_arg(args) else {
        eprintln!("usage: herden channel set <stable|preview>");
        return Ok(2);
    };
    ...
    println!(
        "Herden update channel set to {channel} in {}.",
        path.display()
    );
    ...
    if let Err(err) = crate::update::self_update(crate::update::SelfUpdateOptions::default()) {
        ...
        eprintln!("Run `herden update` to retry.");
        return Ok(1);
    }
...
fn print_channel_help() {
    eprintln!("herden channel commands:");
    eprintln!("  herden channel show                  print the configured update channel");
    eprintln!("  herden channel set <stable|preview>  choose the update channel");
}
...
fn config_check(args: &[String]) -> std::io::Result<i32> {
    match args {
        ...
        [flag] if matches!(flag.as_str(), "help" | "--help" | "-h") => {
            eprintln!("usage: herden config check");
            return Ok(0);
        }
        _ => {
            eprintln!("usage: herden config check");
...
fn config_reset_keys(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: herden config reset-keys");
        return Ok(2);
    }
    ...
    println!("Built-in v2 keybindings will apply after Herden restarts or reloads config.");
    println!("If a Herden server is running, run `herden server reload-config` to apply this now.");
    println!(
        "To restore: cp {} {}",
        backup_path.display(),
        path.display()
...
fn session_attach_help(args: &[String]) -> std::io::Result<i32> {
    if matches!(
        ...
        eprintln!("usage: herden session attach <name>");
        return Ok(0);
    }
    eprintln!("usage: herden session attach <name>");
    Ok(2)
...
fn session_list(args: &[String]) -> std::io::Result<i32> {
    let json = match parse_session_json_only(args, "usage: herden session list [--json]") {
        Ok(json) => json,
...
fn session_stop(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: herden session stop <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn session_delete(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: herden session delete <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn terminal_attach(args: &[String]) -> std::io::Result<i32> {
    let (terminal_id, takeover) = match parse_attach_target(
        args,
        "usage: herden terminal attach <terminal_id> [--takeover]",
    ) {
        Ok(parsed) => parsed,
        Err(code) => return Ok(code),
    };
...
fn terminal_title(args: &[String]) -> std::io::Result<i32> {
    match args.first().map(|arg| arg.as_str()) {
        Some("set") => {
            if args.len() != 2 {
                eprintln!("usage: herden terminal title set <title>");
                return Ok(2);
            }
        ...
        Some("clear") => {
            if args.len() != 1 {
                eprintln!("usage: herden terminal title clear");
                return Ok(2);
            }
            print_response(&send_request(&Request {
        ...
        Some("help" | "--help" | "-h") => {
            eprintln!("usage: herden terminal title set <title>");
            eprintln!("       herden terminal title clear");
            Ok(0)
        }
        _ => {
            eprintln!("usage: herden terminal title set <title>");
...
fn print_config_help() {
    eprintln!("herden config commands:");
    eprintln!("  herden config check  validate config.toml and print diagnostics");
    eprintln!("  herden config reset-keys  back up config.toml and remove custom keybindings");
}

fn print_terminal_help() {
    eprintln!("herden terminal commands:");
    eprintln!("  herden terminal attach <terminal_id> [--takeover]");
    eprintln!("  herden terminal session control <target> [--takeover] [--cols N] [--rows N]");
    eprintln!("  herden terminal session observe <target> [--cols N] [--rows N]");
    eprintln!("  herden terminal title set <title>");
    eprintln!("  herden terminal title clear");
    eprintln!("  detach from direct attach with ctrl+b q; send literal ctrl+b with ctrl+b ctrl+b");
}

fn print_session_help() {
    eprintln!("herden session commands:");
    eprintln!("  herden session list [--json]");
    eprintln!("  herden session attach <name>");
    eprintln!("  herden session stop <name> [--json]");
    eprintln!("  herden session delete <name> [--json]");
