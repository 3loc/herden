...
pub(crate) fn begin_cli_output() {
    // Unit tests exercise CLI handlers in-process. SIGPIPE disposition is
    // process-global, so changing it there can kill the test harness when an
    // unrelated test writes to a closed socket. Integration tests still run
    // the real executable and cover the production disposition.
    #[cfg(test)]
    return;
...
pub(crate) fn create_remote_ssh_config_dir(control_socket_name: &str) -> std::io::Result<PathBuf> {
    ...
    let message = if path_fits {
        "failed to create private herden ssh config directory"
    } else {
