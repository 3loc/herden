...
fn show_desktop_notification_on_thread(
    ...
    let window_name = wide_null("Herden notifications");
    let hwnd = unsafe {
        CreateWindowExW(
            0,
            class_name.as_ptr(),
    ...
    if unsafe { Shell_NotifyIconW(NIM_ADD, &notification) } == 0 {
        let _ = ready_tx.send(Err(std::io::Error::other(
            "failed to add Herden notification-area icon",
        )));
        unsafe {
            DestroyWindow(hwnd);
        }
    ...
    if unsafe { Shell_NotifyIconW(NIM_MODIFY, &notification) } == 0 {
        ...
        let _ = ready_tx.send(Err(std::io::Error::other(
            "failed to show Herden desktop notification",
        )));
        return;
    }
...
mod tests {
    ...
    fn scrollback_editor_argv_uses_editor_env_and_appends_path() {
        let path = std::path::Path::new(r"C:\Users\User\AppData\Local\Temp\herden scrollback.txt");
