...
const NESTED_HERDR_MESSAGES: [&str; 6] = [
    ...
    "recursive herdening is disabled. somewhere, a call stack breathes a sigh of relief.",
    "recursive descent denied. there is, in fact, such a thing as too much herden.",
...
const DEFAULT_CONFIG: &str = r##"# herden configuration
# Place this file at ~/.config/herden/config.toml
...
[update]
# Update channel used by background version checks and `herden update`.
...
# Check the Herden site for new versions in the background.
# version_check = true

# Check the Herden site for remote agent-detection manifest updates in the background.
# manifest_check = true
...
# remote_image_paste = "ctrl+v" # only active in herden --remote; empty disables raw-key image paste
# new_tab = "prefix+c"
...
# Hostname, datetime, and command entries resolve on the Herden server.
# tab_bar_right = []
...
# The title renders on the Herden server, so {hostname} names the host the panes
...
# a Herden server restart. Requires official integrations that report session refs.
# resume_agents_on_restore = true

[remote]
# Whether herden manages the ssh config used for `herden --remote`.
# When true (default), herden runs remote ssh through a generated config that
...
# force keepalive or multiplexing off, it only stops herden from adding its own.
# manage_ssh_config = true

[experimental]
# Allow launching herden from inside a herden-managed pane.
# allow_nested = false
...
fn should_block_nested_for_env(config: &config::Config, herden_env: Option<&str>) -> bool {
    !config.experimental.allow_nested && herden_env == Some(HERDR_ENV_VALUE)
}
...
fn exit_if_nested_disabled(config: &config::Config) {
    if should_block_nested(config) {
        eprintln!("\x1b[1merror:\x1b[0m nested herden is disabled by default.");
...
fn main() -> io::Result<()> {
    let raw_args: Vec<String> = match args_as_utf8(std::env::args_os()) {
        ...
        Err(err) => {
            eprintln!("error: {err}");
            eprintln!("run 'herden --help' for usage");
            std::process::exit(2);
        }
    };
    ...
    if args.get(1).map(|s| s.as_str()) == Some("update") {
        let options = match update::parse_self_update_args(&args[2..]) {
            ...
            Err(err) => {
                eprintln!("{err}");
                eprintln!("usage: herden update [--handoff]");
                std::process::exit(2);
            }
        };
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("herden — terminal workspace manager for AI coding agents");
        println!();
        println!("Usage: herden [options]");
        println!("       herden --session <name> [options]");
        println!("       herden --machine <label-or-id> <command>");
        println!("       herden --remote <ssh-target> [--session <name>]");
        println!("       herden session attach <name>");
        println!("       herden completion zsh");
        println!("       herden update [--handoff]");
        ...
        println!("       herden completion <shell>");
        println!("       herden config <subcommand> ...");
        println!("       herden channel <subcommand> ...");
        println!("       herden workspace <subcommand> ...");
        println!("       herden worktree <subcommand> ...");
        println!("       herden tab <subcommand> ...");
        println!("       herden notification <subcommand> ...");
        println!("       herden agent <subcommand> ...");
        println!("       herden pane <subcommand> ...");
        println!("       herden session <subcommand> ...");
        println!("       herden integration <subcommand> ...");
        println!();
        ...
        for (command, description) in [
            ...
            ("herden update", "Download and install the latest version"),
            (
                "herden completion zsh",
            ...
            (
                "herden channel set <stable|preview>",
                "Choose the stable or preview update channel",
            ),
            (
                "herden server reload-config",
                "Reload config.toml in the running server",
            ),
            (
                "herden config reset-keys",
                "Back up config.toml and remove custom keybindings",
            ),
            (
                "herden channel <subcommand>",
                "Manage the stable or preview update channel",
            ),
            ("herden machine <subcommand>", "Manage saved SSH machines"),
            (
                "herden api <subcommand>",
                "Inspect socket API metadata and live runtime state",
            ),
            (
                "herden workspace <subcommand>",
                "Workspace helpers over the socket API",
            ),
            (
                "herden worktree <subcommand>",
                "Git worktree helpers over the socket API",
            ),
            ("herden tab <subcommand>", "Tab helpers over the socket API"),
            (
                "herden notification <subcommand>",
                "Notification helpers over the socket API",
            ),
            (
                "herden agent <subcommand>",
                "Agent/terminal helpers over the socket API",
            ),
            (
                "herden pane <subcommand>",
                "Pane control helpers over the socket API",
            ),
            (
                "herden session <subcommand>",
                "Manage named persistent sessions",
            ),
            (
                "herden integration <subcommand>",
        ...
        println!("  {:<32} Run as headless server", "herden server");
        println!();
        println!("Options:");
        println!("  --session <name>    Use or create a named persistent session");
        println!("  --machine <label-or-id>  Run an API command on a saved SSH machine");
        println!("  --remote <target>   Attach through SSH to a remote Herden server");
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        platform::begin_cli_output();
        println!("herden {}", crate::build_info::version());
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn nested_herden_blocks_when_env_is_set() {
        let config = config::Config::default();
        assert!(should_block_nested_for_env(&config, Some(HERDR_ENV_VALUE)));
    }

    #[test]
    fn nested_herden_does_not_block_when_allowed() {
    ...
    #[test]
    fn nested_herden_does_not_block_without_env() {
        let config = config::Config::default();
    ...
    #[test]
    fn nested_message_strings_no_longer_repeat_herden_prefix() {
        assert!(NESTED_HERDR_MESSAGES
            .iter()
            .all(|message| !message.starts_with("herden:")));
    }
    ...
    fn args_as_utf8_passes_through_valid_arguments() {
        let args = ["herden", "pane", "get", "pane-1"].map(std::ffi::OsString::from);
        assert_eq!(
            args_as_utf8(args).unwrap(),
            ["herden", "pane", "get", "pane-1"]
        );
    }
