...
if ($channelWasExplicit -and $Channel -notin @("stable", "preview")) {
    Write-Error "Invalid Herden channel '$Channel'. Use 'stable' or 'preview'."
    exit 1
}
...
if ($useLocalPackage -and $LocalPackageFormat -notin @("zip", "exe")) {
    throw "Local Herden package has unsupported format '$LocalPackageFormat'."
}
...
function Get-HerdenCommandSource {
    $existing = Get-Command herden -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-HerdenExecutableKind {
    param(
        [string]$Path,
        [string]$ReleasesDir,
        [string]$CurrentDir,
    ...
    if ([string]::IsNullOrWhiteSpace($Path) -or
        -not [System.IO.Path]::GetFileName($Path).Equals("herden.exe", [System.StringComparison]::OrdinalIgnoreCase)) {
        return $null
    }
    ...
    try {
        ...
        foreach ($alias in @($CurrentDir, $VisibleBinDir)) {
            $aliasHerden = [System.IO.Path]::GetFullPath((Join-Path $alias "herden.exe"))
            if ($fullPath.Equals($aliasHerden, [System.StringComparison]::OrdinalIgnoreCase)) {
                return "alias"
            }
        }
...
function Publish-EnvironmentChange {
    if (-not ("HerdenInstaller.EnvironmentNativeMethods" -as [type])) {
        Add-Type -Namespace HerdenInstaller -Name EnvironmentNativeMethods -MemberDefinition @'
...
'@
    ...
    $result = [UIntPtr]::Zero
    [HerdenInstaller.EnvironmentNativeMethods]::SendMessageTimeout(
        [IntPtr]0xffff,
        0x1a,
        [UIntPtr]::Zero,
        "Environment",
...
function Invoke-CurlDownload {
    ...
    if (-not [System.Uri]::TryCreate($Uri, [System.UriKind]::Absolute, [ref]$parsedUri) -or
        ...
        throw "Herden download URL must use HTTP or HTTPS: $Uri"
    }

    $curl = Get-Command curl.exe -CommandType Application -ErrorAction SilentlyContinue |
        Select-Object -First 1
    if ($null -eq $curl) {
        throw "Herden installation requires curl.exe, which is included with supported Windows versions."
    }
...
function Get-RemoteManifest {
    ...
    $manifestPath = Join-Path ([System.IO.Path]::GetTempPath()) ("herden-manifest-" + [System.Guid]::NewGuid().ToString("N") + ".json")
    try {
...
function Test-FileDigest {
    ...
    if ($actual -ne $ExpectedDigest.ToLowerInvariant()) {
        throw "Downloaded Herden checksum did not match. Expected $ExpectedDigest but got $actual."
    }
}
...
function Test-HerdenReleaseComplete {
    param(
        [string]$ReleaseDir,
        [string]$Format
    )
    ...
    $herdenExe = Join-Path $ReleaseDir "herden.exe"
    if (-not (Test-RegularFile -Path $herdenExe)) {
        return $false
    }
    if ($Format -eq "exe") {
        return $true
    ...
    $markerPath = Join-Path $conptyRoot "herden-conpty.json"
    $required = @(
        "conpty/conpty.dll",
    ...
    try {
        ...
        $expectedBundleFiles = @($expectedConptyFiles) + "conpty/herden-conpty.json"
    ...
    while (Test-Path -LiteralPath $Path) {
        try {
            ...
            if ([DateTime]::UtcNow -ge $deadline) {
                Write-WarningStep "Herden installed successfully but could not remove a temporary release backup at $Path."
                return
            }
            Start-Sleep -Milliseconds 100
        }
...
function Set-ManagedJunction {
    param(
        ...
        [bool]$AllowLegacyHerdenBinMigration = $false
    )
    ...
    if (Test-Path -LiteralPath $LinkPath) {
        ...
        if (Test-IsJunction -Path $LinkPath) {
            ...
            if ((Get-ChildItem -LiteralPath $LinkPath -Force | Select-Object -First 1) -ne $null) {
                if (-not (Move-LegacyHerdenBinDirectory -Path $LinkPath -AllowMigration $AllowLegacyHerdenBinMigration)) {
...
function Move-LegacyHerdenBinDirectory {
    param(
        [string]$Path,
        [bool]$AllowMigration
    )
    ...
    if (($entries | Where-Object { $_.Name -ieq "herden.exe" } | Select-Object -First 1) -eq $null) {
        return $false
    }

    $legacyPath = "$Path.legacy.$([System.Guid]::NewGuid().ToString("N"))"
    Move-Item -LiteralPath $Path -Destination $legacyPath
    Write-Step "Moved legacy Herden bin directory to $legacyPath."
    return $true
}
...
function Resolve-HerdenVersion {
    param(
        [object]$Manifest,
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "Herden requires 64-bit Windows."
    exit 1
}
...
$herdenHome = if ([string]::IsNullOrWhiteSpace($env:HERDR_HOME)) {
    Join-Path $env:USERPROFILE ".herden"
} else {
    $env:HERDR_HOME
}
$herdenHome = [System.IO.Path]::GetFullPath($herdenHome)
$standaloneRoot = Join-Path $herdenHome "packages\standalone"
$releasesDir = Join-Path $standaloneRoot "releases"
$currentDir = Join-Path $standaloneRoot "current"
$lockPath = Join-Path $standaloneRoot "install.lock"

$defaultVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\Herden\bin"
...
$commandHerden = Get-HerdenCommandSource
$channelHerden = $commandHerden
if ([string]::IsNullOrWhiteSpace($channelHerden)) { $channelHerden = Get-HerdenMigrationFallback -CurrentDir $currentDir }
$existingHerdenKind = Get-HerdenExecutableKind `
    -Path $channelHerden `
    -ReleasesDir $releasesDir `
    -CurrentDir $currentDir `
    -VisibleBinDir $visibleBinDir
if (-not [string]::IsNullOrWhiteSpace($commandHerden) -and $null -eq $existingHerdenKind) {
    Write-Step "Detected existing Herden command at $commandHerden"
    Write-WarningStep "PATH order decides which Herden runs. This installer will put the active versioned release first for future and current PowerShell sessions."
}
...
if ($useLocalPackage) {
    ...
    if (-not $channelWasExplicit) {
        if (-not [string]::IsNullOrWhiteSpace($channelHerden) -and $null -eq $existingHerdenKind) {
            throw "Refusing to run unrecognized Herden command at $channelHerden to detect its update channel. Rerun with -Channel stable or -Channel preview."
        }
        if (-not [string]::IsNullOrWhiteSpace($channelHerden)) {
            $detectedChannel = [string](& $channelHerden channel show 2>$null | Select-Object -Last 1)
            $detectedChannel = $detectedChannel.Trim()
            if ($LASTEXITCODE -ne 0 -or $detectedChannel -notin @("stable", "preview")) {
                throw "Could not determine the existing Herden update channel. Rerun with -Channel stable or -Channel preview."
            }
            $Channel = $detectedChannel
            Write-Step "Preserving existing Herden $Channel channel"
    ...
    if ([string]::IsNullOrWhiteSpace($ManifestUrl)) {
        $ManifestUrl = if ($Channel -eq "preview") {
            "https://herden.app/preview.json"
        } else {
            "https://herden.app/latest.json"
        }
    }

    Write-Step "Fetching Herden $Channel manifest"
    ...
    if ($null -eq $assetProperty -and
        ...
        Write-Step "Fetching Herden preview manifest"
        $manifest = Get-RemoteManifest -Uri $ManifestUrl
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($ExpectedBuildId) -and [string]$manifest.build_id -ne $ExpectedBuildId) {
        throw "Preview manifest changed while updating. Expected build $ExpectedBuildId but found $($manifest.build_id). Run herden update again."
    }
    $versionIdentity = Resolve-HerdenVersion -Manifest $manifest -SelectedChannel $Channel
}
$safeVersionIdentity = $versionIdentity -replace '[^0-9A-Za-z._-]', '-'
$releaseName = "$safeVersionIdentity-$targetTriple"
$releaseDir = Join-Path $releasesDir $releaseName

Write-Step "Installing Herden $versionIdentity for $targetTriple"
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("herden-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    $userPathChanged = Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-HerdenReleaseComplete -ReleaseDir $releaseDir -Format $asset.Format)) {
            $downloadPath = if ($useLocalPackage) {
                $LocalPackagePath
            } else {
                Join-Path $tempDir "herden-download.$($asset.Format)"
            }
            $stagingDir = Join-Path $releasesDir ".staging.$releaseName.$PID"
            if (-not $useLocalPackage) {
                Write-Step "Downloading Herden"
            ...
            if ($asset.Format -eq "zip") {
                ...
                Copy-Item -LiteralPath $downloadPath -Destination (Join-Path $stagingDir "herden.exe")
            }
            if (-not (Test-HerdenReleaseComplete -ReleaseDir $stagingDir -Format $asset.Format)) {
                throw "Downloaded Herden package is incomplete or failed ConPTY verification."
            }
            $stagedHerden = Join-Path $stagingDir "herden.exe"
            & $stagedHerden --version *> $null
            if ($LASTEXITCODE -ne 0) {
                throw "Downloaded Herden command failed verification: $stagedHerden --version"
            }
            $backupDir = $null
            ...
            try {
                ...
                Write-WarningStep "Windows could not activate the downloaded release. Another process may have a package file open, such as antivirus or indexing. No incomplete release was activated. Run herden update again."
                throw
            }
        }

        $releaseHerden = Join-Path $releaseDir "herden.exe"
        & $releaseHerden --version *> $null
        if ($LASTEXITCODE -ne 0) {
            throw "Installed Herden command failed verification: $releaseHerden --version"
        }
        ...
        Set-ManagedJunction -LinkPath $visibleBinDir -TargetPath $releaseDir -ManagedTargetPrefix $standaloneRoot -AllowLegacyHerdenBinMigration $allowLegacyVisibleBinMigration
...
$resolvedHerden = Get-HerdenCommandSource
$resolvedHerdenKind = Get-HerdenExecutableKind `
    -Path $resolvedHerden `
    -ReleasesDir $releasesDir `
    -CurrentDir $currentDir `
    -VisibleBinDir $visibleBinDir
$releaseHerden = Join-Path $releaseDir "herden.exe"
if ($resolvedHerdenKind -ne "release" -or
    -not [System.IO.Path]::GetFullPath($resolvedHerden).Equals(
        [System.IO.Path]::GetFullPath($releaseHerden),
        [System.StringComparison]::OrdinalIgnoreCase
    )) {
    Write-WarningStep "PowerShell still resolves herden to $resolvedHerden. Open a new PowerShell window or inspect PATH order manually."
}

Write-Step "Current PowerShell session: herden"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: herden"
Write-Host "Herden $versionIdentity installed successfully."
