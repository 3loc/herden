...
use support::{
    cleanup_test_base, client_shell_handshake, register_runtime_dir, register_spawned_herden_pid,
    send_client_shell_shift_enter, unregister_spawned_herden_pid, wait_for_client_shell_bootstrap,
...
impl Drop for SpawnedHerden {
    fn drop(&mut self) {
        ...
        unregister_spawned_herden_pid(pid);
    }
}
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket: &Path) -> SpawnedHerden {
...
fn spawn_server_with_env(
    ...
    cmd.env(
        ...
        runtime_dir.join("herden-client.sock"),
    );
...
fn spawn_named_session_server(
    ...
    fs::create_dir_all(config_home.join("herden-dev")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    fs::write(
        config_home.join("herden-dev/config.toml"),
        "onboarding = false\n",
    )
    .unwrap();
...
fn spawn_default_session_server(config_home: &Path, runtime_dir: &Path) -> SpawnedHerden {
    fs::create_dir_all(config_home.join("herden-dev")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    fs::write(
        config_home.join("herden-dev/config.toml"),
...
fn wait_for_replacement_server_pid(runtime_dir: &Path, old_pid: u32, timeout: Duration) -> u32 {
    ...
    while Instant::now() < deadline {
        last_pids = support::herden_server_pids_for_runtime_dir(runtime_dir).unwrap_or_default();
...
fn live_handoff_preserves_named_session_socket_paths() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let session_dir = config_home.join("herden-dev/sessions/work");
    let api_socket = session_dir.join("herden.sock");
    let client_socket = session_dir.join("herden-client.sock");
    ...
    assert!(
        !config_home.join("herden-dev/herden.sock").exists(),
...
fn live_handoff_ignores_leaked_default_socket_env_for_named_session() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let default_session_dir = config_home.join("herden-dev");
    let default_api_socket = default_session_dir.join("herden.sock");
    let default_client_socket = default_session_dir.join("herden-client.sock");
    let work_session_dir = config_home.join("herden-dev/sessions/work");
    let work_api_socket = work_session_dir.join("herden.sock");
    let work_client_socket = work_session_dir.join("herden-client.sock");
...
fn live_handoff_preserves_client_socket_env_without_api_socket_env() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let api_socket = config_home.join("herden-dev/herden.sock");
...
fn live_handoff_preserves_installed_plugins() {
    ...
    let runtime_dir = base.join("runtime");
    let api_socket = config_home.join("herden-dev/herden.sock");
    let registry_path = config_home.join("herden-dev/plugins.json");
...
fn live_handoff_preserves_http_servers_across_multiple_sessions() {
    ...
    let runtime_dir = base.join("runtime");
    let sessions = [
        (None, config_home.join("herden-dev/herden.sock")),
        (
            Some("work"),
            config_home.join("herden-dev/sessions/work/herden.sock"),
        ),
    ];
