...
use support::{
    ...
    register_spawned_herden_pid, send_detach, unregister_spawned_herden_pid, wait_for_disconnect,
...
fn spawn_server_with_config(
    ...
    fs::write(config_home.join("herden/config.toml"), config).unwrap();
    ...
    cmd.env("HERDR_CONFIG_PATH", config_home.join("herden/config.toml"));
