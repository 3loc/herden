...
if [ "$last" = 'command -v herden' ]; then
    echo /home/remote/.local/bin/herden
    exit 0
fi
case "$last" in
...
case "$script" in
    ...
    *'version='*) echo /home/remote/.local/bin/herden ;;
    *'status client --json'*)
    ...
    *'mkdir -p'*) printf '/fake/tmp\000/fake/herden\000' ;;
    *'chmod 755'*) echo install >>"$FAKE_ROOT/actions"; touch "$FAKE_ROOT/installed" ;;
    *'command -v herden'*) echo /home/remote/.local/bin/herden ;;
...
fn setup_with_strict_host_key_failure(
    ...
    .unwrap();
    let status = Command::new(env!("CARGO_BIN_EXE_herden"))
    ...
    let mut command = CommandBuilder::new(env!("CARGO_BIN_EXE_herden"));
    if handoff {
...
fn machine_add_accepts_help_argument_order() {
    ...
    let root = std::env::temp_dir().join(format!(
        "herden-machine-add-order-{}-{nonce}",
        std::process::id()
    ));
