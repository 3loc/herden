...
use support::{
    ...
    register_spawned_herden_pid, unregister_spawned_herden_pid, wait_for_client_shell_bootstrap,
...
fn direct_attach_initial_mouse_capture_follows_config() {
    ...
    drop(spawned_server);
    cleanup_spawned_herden(attach, base);
}
...
fn client_sees_headless_startup_config_diagnostic() {
    ...
    assert!(
        wait_until(Duration::from_secs(8), Duration::from_millis(20), || {
            ...
            output.contains("config.toml") && output.contains("herden config check")
        }),
    ...
    drop(spawned);
    cleanup_spawned_herden(client, base);
}
...
fn server_unreachable_shows_clear_error() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_herden"))
        .arg("client")
    ...
    assert!(
        stderr.contains("Is the Herden server running?"),
        ...
        while Instant::now() < deadline {
            match thin_reader.read(&mut buf) {
                Ok(n) if n > 0 => {
                    ...
                    if output.to_lowercase().contains("herden:") {
                        break;
                    }
                }
...
) -> (SpawnedHerden, SpawnedHerden, SharedOutput) {
...
fn attach_thin_client_with_config(
    ...
    while Instant::now() < deadline {
        ...
        if out.to_lowercase().contains("herden:") {
            break;
        }
        thread::sleep(Duration::from_millis(30));
    }
...
fn federated_client_starts_without_local_and_survives_its_restart() {
    ...
    let remote_api = remote_runtime.join("herden.sock");
    let remote_client = remote_runtime.join("herden-client.sock");
    ...
    std::os::unix::fs::symlink(env!("CARGO_BIN_EXE_herden"), bin.join("herden")).unwrap();
    let quote =
    ...
    input.write_all(b"\x1b[<0;7;8M\x1b[<0;7;8m").unwrap();
    assert!(
        wait_until(Duration::from_secs(10), Duration::from_millis(20), || {
...
fn client_shell_detaches_restores_and_freshly_reattaches_to_current_state() {
    ...
    drop(server);
    cleanup_spawned_herden(client_b, base);
}
...
fn assert_client_restores_terminal(trigger: impl FnOnce(&mut SpawnedHerden, &mut SpawnedHerden)) {
    let _lock = test_lock();
    let base = unique_test_dir();
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    ...
    // SpawnedHerden::Drop kills and reaps both processes with a bounded wait.
    drop(spawned_server);
    cleanup_spawned_herden(thin_client, base);
}
...
fn read_until_client_attaches(client: &SpawnedHerden) -> String {
...
fn pane_spawn_cwd_fallback_in_server() {
    ...
    drop(spawned);
    cleanup_spawned_herden(client_shell, base);
}
