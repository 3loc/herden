...
pub(super) use crate::support::{
    ...
    unregister_spawned_herden_pid, CURRENT_PROTOCOL,
};
...
pub(super) fn create_committed_repo(path: &Path) {
    ...
    run_git(path, &["config", "user.email", "herden@example.invalid"]);
    run_git(path, &["config", "user.name", "Herden Test"]);
    fs::write(path.join("README.md"), "test\n").unwrap();
    run_git(path, &["add", "README.md"]);
    run_git(path, &["commit", "--quiet", "-m", "initial"]);
}

pub(super) struct SpawnedHerden {
...
pub(super) fn cleanup_spawned_herden(spawned: SpawnedHerden, base: PathBuf) {
    drop(spawned);
...
pub(super) fn spawn_herden(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedHerden {
    spawn_herden_with_config(
        config_home,
        runtime_dir,
        socket_path,
        None,
...
pub(super) fn spawn_herden_with_pane_history(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedHerden {
...
pub(super) fn spawn_named_server(
    ...
    register_spawned_herden_pid(Some(child.id()));
...
pub(super) fn spawn_herden_with_path(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
...
pub(super) fn spawn_herden_with_config(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
...
pub(super) fn parse_cli_json_output(
    ...
    serde_json::from_slice(&output.stdout).unwrap_or_else(|err| {
        panic!(
            "failed to parse JSON response for `herden {}`: {}\nstdout: {}\nstderr: {}",
            args.join(" "),
            err,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
