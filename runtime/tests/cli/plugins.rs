...
fn plugin_link_works_offline_and_is_global() {
    ...
    fs::write(
        plugin_dir.join("herden-plugin.toml"),
        r#"
id = "example.offline"
name = "Offline Plugin"
version = "0.1.0"
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "global-plugin/herden-plugin.toml"]);
...
version = "0.1.0"
min_herdr_version = "0.6.10"
description = "Apply a preferred Herden layout"
...
"#,
    ...
    run_git(
        &source_repo,
        &["add", "worktree-bootstrap/herden-plugin.toml"],
    );
    run_git(&source_repo, &["commit", "--quiet", "-m", "add plugin"]);
    ...
    fs::write(
        &git_config,
        format!(
            "[url \"file://{}\"]\n    insteadOf = https://github.com/ogulcancelik/herden-plugin-examples.git\n",
            source_repo.display()
        ),
    )
    .unwrap();
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ...
            "plugin",
            "install",
            "ogulcancelik/herden-plugin-examples/worktree-bootstrap",
            "--yes",
        ],
        &[
            ("GIT_CONFIG_GLOBAL", &git_config),
    ...
    assert_eq!(plugin["source"]["repo"], "herden-plugin-examples");
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "build-fail/herden-plugin.toml"]);
    run_git(
        &source_repo,
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ...
            "plugin",
            "install",
            "ogulcancelik/herden-plugin-examples/build-fail",
            "--yes",
        ],
        &[("GIT_CONFIG_GLOBAL", &git_config)],
    );
...
[[build]]
command = ["definitely-missing-herden-build-tool-xyz"]
...
"#,
    ...
    .unwrap();
    run_git(&source_repo, &["add", "missing-tool/herden-plugin.toml"]);
    run_git(
        &source_repo,
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ...
            "plugin",
            "install",
            "ogulcancelik/herden-plugin-examples/missing-tool",
            "--yes",
        ],
        &[("GIT_CONFIG_GLOBAL", &git_config)],
    );
    ...
    assert!(stderr.contains("  build: 1/1"), "{stderr}");
    assert!(
        stderr.contains("  command: definitely-missing-herden-build-tool-xyz"),
        "{stderr}"
    );
...
"#,
    ...
    let install = run_named_cli_with_env(
        ...
        &[
            ...
            "plugin",
            "install",
            "ogulcancelik/herden-plugin-examples/manifest-mutator",
            "--yes",
        ],
        &[("GIT_CONFIG_GLOBAL", &git_config)],
    );
    ...
    let stderr = String::from_utf8_lossy(&install.stderr);
    assert!(
        stderr.contains("plugin build changed herden-plugin.toml after install preview"),
        "{stderr}"
    );
...
fn plugin_install_restores_previous_checkout_when_registration_fails() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let socket_path = runtime_dir.join("fake-herden.sock");
    let source_repo = base.join("source-repo");
    let plugin_dir = source_repo.join("worktree-bootstrap");
    fs::create_dir_all(&plugin_dir).unwrap();
    create_committed_repo(&source_repo);
            ...
            serde_json::json!({
                ...
                "result": {
                    ...
                    "plugins": [{
                        ...
                        "version": "0.1.0",
                        "min_herdr_version": "0.6.10",
                        "manifest_path": managed_checkout_for_server.join("herden-plugin.toml").display().to_string(),
                        "plugin_root": managed_checkout_for_server.display().to_string(),
                        "enabled": true,
                        "source": {
                            "kind": "github",
                            "owner": "ogulcancelik",
                            "repo": "herden-plugin-examples",
