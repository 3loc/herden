...
fn run_claude_hook(action: &str, hook_input: &str) -> Option<serde_json::Value> {
    run_shell_hook(
        "src/integration/assets/claude/herden-agent-state.sh",
        &[action],
        hook_input,
    )
}
...
fn run_codex_hook(action: &str, hook_input: &str) -> Option<serde_json::Value> {
    run_shell_hook(
        "src/integration/assets/codex/herden-agent-state.sh",
        &[action],
        hook_input,
    )
}
...
fn run_copilot_hook(hook_input: &str) -> Option<serde_json::Value> {
    run_shell_hook(
        "src/integration/assets/copilot/herden-agent-state.sh",
        &[],
        hook_input,
    )
}
...
fn run_devin_hook(
    ...
    run_shell_hook_with_env(
        "src/integration/assets/devin/herden-agent-state.sh",
        &[action],
        hook_input,
        envs,
    )
...
fn run_grok_hook(hook_input: &str, envs: &[(&str, &str)]) -> Option<serde_json::Value> {
    run_shell_hook_with_env(
        "src/integration/assets/grok/herden-agent-state.sh",
        &["session"],
        hook_input,
        envs,
    )
