...
use support::{
    ...
    register_spawned_herden_pid, send_client_shell_focus, send_detach,
    unregister_spawned_herden_pid, wait_for_client_shell_bootstrap, wait_for_message_variant,
    wait_for_message_variants, CURRENT_ENDPOINT_PROTOCOL_GENERATION as CURRENT_PROTOCOL,
...
fn spawn_server(config: &Path, runtime: &Path, api: &Path) -> SpawnedHerden {
    fs::create_dir_all(config.join("herden")).unwrap();
    fs::create_dir_all(runtime).unwrap();
    register_runtime_dir(runtime);
    fs::write(config.join("herden/config.toml"), "onboarding = false\n").unwrap();
...
fn spawn_client(config: &Path, runtime: &Path, api: &Path) -> SpawnedHerden {
    register_runtime_dir(runtime);
    let pair = native_pty_system()
        .openpty(PtySize {
            rows: 24,
...
fn cleanup(server: SpawnedHerden, base: PathBuf) {
    drop(server);
...
fn same_tab_geometry_follows_meaningful_client_activity() {
    ...
    let api = runtime.join("herden.sock");
    let clients = runtime.join("herden-client.sock");
