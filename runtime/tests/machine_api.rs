...
case "$last" in
    *'command -v herden') printf 'login banner\nherdr-remote-output-ready:1\n%s\n' "$TEST_REMOTE_HERDR" ;;
...
impl Harness {
    fn new() -> Self {
        ...
        std::os::unix::fs::symlink(env!("CARGO_BIN_EXE_herden"), root.join("remote herden"))
            .unwrap();
        ...
        let remote = UnixListener::bind(session.join("herden.sock")).unwrap();
    ...
    fn command(&self, args: &[&str]) -> Command {
        ...
        command
            ...
            .env("TEST_REMOTE_HERDR", self.root.join("remote herden"))
