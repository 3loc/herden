...
use support::{
    cleanup_test_base, register_runtime_dir, register_spawned_herden_pid,
    unregister_spawned_herden_pid,
};
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/herden-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}

struct SpawnedHerden {
    _master: Box<dyn MasterPty + Send>,
    child: Box<dyn Child + Send + Sync>,
}

impl Drop for SpawnedHerden {
    fn drop(&mut self) {
        ...
        if let Some(pid) = pid {
            ...
            unregister_spawned_herden_pid(Some(pid));
        }
    }
}

fn cleanup_spawned_herden(spawned: SpawnedHerden, base: PathBuf) {
    drop(spawned);
...
fn spawn_server(
    ...
    fs::create_dir_all(config_home.join("herden")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    register_runtime_dir(runtime_dir);
    fs::write(
        config_home.join("herden/config.toml"),
        "onboarding = false\n",
    )
    .unwrap();
    ...
    let mut cmd = CommandBuilder::new(env!("CARGO_BIN_EXE_herden"));
    cmd.arg("server");
    ...
    register_spawned_herden_pid(child.process_id());
    drop(pair.slave);
...
/// Spawn `herden` (no subcommand) — the auto-detect launch path.
fn spawn_herden_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
    _client_socket_path: &Path,
...
fn run_cli(socket_path: &Path, args: &[&str]) -> std::process::Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_herden"));
    command.args(args);
...
/// Running `herden` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    let api_socket = runtime_dir.join("herden.sock");
    let client_socket = runtime_dir.join("herden-client.sock");
    ...
    // Run `herden` (no subcommand) — should auto-detect, spawn server, attach as client.
    let herden = spawn_herden_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = herden.child.process_id().expect("client should have PID");
    assert!(
        process_exists(client_pid),
        "client process should be running"
    );

    cleanup_spawned_herden(herden, base);
}

/// Running `herden` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `herden` (no subcommand) — should detect the running server and attach.
    let client = spawn_herden_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_herden(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_herden(server, base);
}
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `herden` with custom socket paths.
    let herden = spawn_herden_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `herden workspace list` through the server's API socket.
    ...
    // Test `herden pane list` through the server's API socket.
...
/// persists after the client exits, and a new `herden` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `herden` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_herden_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `herden` again — should detect the running server and reattach.
    let client2 = spawn_herden_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_herden(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    let api_socket = config_home.join(app_dir_name).join("herden.sock");
    let client_socket = config_home.join(app_dir_name).join("herden-client.sock");
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_herden_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("herden-client.log");
    let server_log = log_dir.join("herden-server.log");
    let monolith_log = log_dir.join("herden.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in herden.log: {monolith_content}"
    );

    cleanup_spawned_herden(spawned, base);
}
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_herden"))
    ...
    assert!(
        stderr.contains("nested herden is disabled by default"),
