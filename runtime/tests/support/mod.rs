...
const RUNTIME_OWNER_MARKER: &str = ".herden-test-owner-pid";
...
pub fn register_spawned_herden_pid(pid: Option<u32>) {
...
pub fn unregister_spawned_herden_pid(pid: Option<u32>) {
    let Some(pid) = pid else {
        return;
    };
...
#[cfg(target_os = "linux")]
pub fn herden_server_pids_for_runtime_dir(runtime_dir: &Path) -> std::io::Result<Vec<u32>> {
...
pub fn cleanup_registered_herden_pids() {
    let pids: Vec<u32> = {
        let mut registry = pid_registry_lock();
...
fn ensure_cleanup_hooks() {
    INIT.call_once(|| {
        ...
        std::panic::set_hook(Box::new(move |panic_info| {
            cleanup_registered_herden_pids();
...
fn start_global_watchdog() {
    thread::spawn(|| loop {
        ...
        if let Err(err) = cleanup_servers_with_missing_runtime_dir() {
            eprintln!("herden test cleanup watchdog error: {err}");
        }
    });
}
...
fn iter_worktree_server_pids() -> std::io::Result<Vec<u32>> {
    ...
    for entry in proc_entries {
        ...
        if is_test_herden_server_process(pid) {
            pids.push(pid);
        }
    }
...
fn is_test_herden_server_process(pid: u32) -> bool {
    let Some(exe_path) = proc_link_target(pid, "exe") else {
        return false;
    };

    if !is_test_herden_binary(&exe_path) {
        return false;
    }
...
mod tests {
    ...
    fn unique_missing_runtime_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "herden-watchdog-scoping-{label}-{}-{unique}",
            std::process::id()
        ))
    }
