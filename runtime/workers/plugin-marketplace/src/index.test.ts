  ...
  return (async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (treeMatch) {
      ...
      const fixtures = options.trees?.[fullName] ?? [
        { path: "herden-plugin.toml", content: manifest() },
      ];
      return Response.json({
        sha: "tree-sha",
...
describe("refreshPlugins", () => {
  test("publishes one backward-compatible repository card with multiple manifests", async () => {
    ...
    const fetch = repositoryFetch({
      ...
      trees: {
        [fullName]: [
          { path: "herden-plugin.toml", content: manifest() },
          {
            path: "plugins/second/herden-plugin.toml",
    ...
    expect(snapshot.plugins[0]).toMatchObject({
      ...
      manifests: [
        { path: "herden-plugin.toml", id: "example.plugin" },
        { path: "plugins/second/herden-plugin.toml", id: "example.second" },
      ],
    });
  ...
  test("rescans a repository when its pushed timestamp and head change", async () => {
    ...
    const trees = { [fullName]: [{ path: "herden-plugin.toml", content: manifest() }] };
  ...
  test("counts manifests separately from repository cards and omits empty cards", async () => {
    ...
    const result = await refreshPlugins(env(), {
      fetch: repositoryFetch({
        ...
        trees: {
          "ogulcancelik/herdr-plugin-example": [
            { path: "one/herden-plugin.toml", content: manifest() },
            { path: "two/herden-plugin.toml", content: manifest().replace("example.plugin", "example.two") },
          ],
          "example/empty": [{ path: "README.md", content: "empty" }],
          "example/invalid": [{ path: "herden-plugin.toml", content: "id = [broken" }],
        },
      }),
      logger: { error() {} },
    });
  ...
  test("ignores manifest symlinks and indexes their regular target", async () => {
    ...
    const result = await refreshPlugins(env(), {
      fetch: repositoryFetch({
        ...
        trees: {
          [fullName]: [
            { path: "herden-plugin.toml", content: "plugin/herden-plugin.toml", mode: "120000" },
            { path: "plugin/herden-plugin.toml", content: manifest() },
          ],
        },
      }),
      logger: { error() {} },
    ...
    const result = await refreshPlugins(env(), {
      fetch: repositoryFetch({
        ...
        trees: {
          [fullName]: [
            { path: "herden-plugin.toml", content: manifest() },
            { path: "platform/herden-plugin.toml", content: manifest() },
            {
              path: "tests/fixtures/helper/herden-plugin.toml",
  ...
  test("pins tree and manifest reads to the resolved commit", async () => {
    ...
    expect(manifestQuery).toContain(`${HEAD_COMMIT}:herden-plugin.toml`);
    expect(manifestQuery).not.toContain("main:herden-plugin.toml");
  });
    ...
    const trees = Object.fromEntries(
      repositories.map((repository) => [
        ...
        Array.from({ length: 100 }, (_, index) => ({
          path: `plugins/${index}/herden-plugin.toml`,
          content: manifest(),
        })),
      ]),
    );
