...
SDK_ROOT = Path.home() / ".local/share/herden/windows-cross"
...
def setup(accept_license: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="herden-windows-sdk-", dir=temp_parent) as cache:
...
def lint() -> None:
    ...
    subprocess.run(
        ["cargo", "clippy", "--bin", "herden", "--locked", "--target", TARGET, "--", "-D", "warnings"],
        env=env,
        check=True,
    )
