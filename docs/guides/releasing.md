...
| `Herden.xcodeproj/project.pbxproj` | derived — regenerated by `xcodegen generate` |
| `Sources/HerdenNotificationService/Info.plist` | derived — its version keys are `$(MARKETING_VERSION)` / `$(CURRENT_PROJECT_VERSION)` references |

The Host runtime is **not** part of an app release. `runtime/Cargo.toml`
carries its own `host-vX.Y.Z` line, cut by
[releasing-host.md](releasing-host.md) — App 0.1.x and Host 0.9.x move
independently, and `make publish` leaves the runtime version alone.
