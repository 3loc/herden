...
Herden when the Mac is behind NAT and has no stable public address. A
...
+------------------+    +-----------------------------+    +------------------+
| iOS              |    | VPS                         |    | macOS            |
| Herden     |    | public TCP 22               |    | Remote Login    |
|                  |    |                             |    | TCP 22           |
...
1. Herden authenticates to the VPS Jump Host with its Device Key.
2. The Jump Host opens only `127.0.0.1:2222`.
3. Herden performs a second SSH handshake with the Mac through that
...
Herden verifies the Jump Host and Host independently. Confirm a new VPS
...
## 2. Obtain the Herden Device Key

Copy the public-key line shown by Herden. The private half remains in the
iOS Keychain.
...
On the Mac, keep PTY, exec, and SFTP available for Herden while disabling
...
The desktop client identity is only for command-line validation. Herden
...
Herden, including exec channels, SFTP, and PTY access. Losing an
...
5. Generate a new tunnel key for the new VPS. Do not reuse the old VPS tunnel
   key.
6. Authorize the existing Herden Device Key on the new Jump account. The
...
- [ ] Herden verifies both VPS and Mac host keys.
- [ ] Device Key is authorized on both hops.
- [ ] `ssh` or an equivalent real end-to-end test reaches the Mac.
- [ ] Herden preflight and Attach work through the Jump Host.
