...
- The Even renderer consumes HUD protocol v1 fields, removes public build-time
  ...
  font-size property, so the view uses its fixed proportional firmware face in
  one zero-padding 576×288 text container. This preserves the firmware Unicode
  glyph set and avoids full-screen image transfers.
...
On a private Linux Host, the pinned Zig 0.16.0 toolchain built the optimized Host and the
...
match, `herden glasses enable` selected the Host's Tailnet address automatically, and both
...
renderer; an authenticated SSE frame was checked to contain the expected Space
label. This was a private test build, not a public or fleet release.
...
An active Wayland desktop ran the updated native simulator through a
localhost-only SSH forward and Vite development proxy. The first screen is a
compact Space overview with a stable selection marker; it enters the read-only
status view only after a click. Private captures and a development `.ehpk`
confirmed the renderer flow; the package is not hardware-verified or
distributable.
...
`host:agent:space:state`; the development session aggregated two private Hosts.
Each Host remains independently authenticated; this package is
read-only.
