...
Once a child queries the terminal defaults or palette, queries the light/dark
mode, or enables colour-scheme reporting, that PTY's application-facing colour
context becomes immutable. Later client reports remain useful to the client and
Host chrome but cannot recolour the running child or its retained cells. Live
handoff imports are frozen immediately because the existing process may already
have cached its answers before the new Host took ownership. Application OSC
set/reset commands remain application-owned and are not filtered.
...
Remaining work includes fully client-relative output or an explicitly selected
lossy presentation mode for applications that paint fixed RGB. Freezing the
application-facing context prevents one viewer from changing another viewer's
